...
SOLVER_SOURCE = Path(inspect.getfile(core.evolve_adaptive)).resolve()
...
def _solver_commit() -> str:
    """Identify the loaded solver, including a standalone release snapshot."""
    provenance = SOLVER_SOURCE.with_name("PROVENANCE.json")
    if provenance.exists():
        manifest = json.loads(provenance.read_text())
        if manifest["sha256"].get(SOLVER_SOURCE.name) != _sha256(SOLVER_SOURCE):
            raise RuntimeError("bundled solver does not match its source provenance")
        return manifest["canonical_commit"]
    repository = SOLVER_SOURCE.parents[2]
    commit = _git_head(repository)
    recorded = subprocess.run(
        ["git", "-C", str(repository), "show", f"{commit}:solvers/navier/navier_spectral_core.py"],
        check=True, capture_output=True,
    ).stdout
    if hashlib.sha256(recorded).hexdigest() != _sha256(SOLVER_SOURCE):
        raise RuntimeError("loaded solver has uncommitted changes; verify and commit before export")
    return commit
...
def export_dataset(
    ...
    *,
    ...
    dataset = {
        ...
        "canonical_solver": {
            "repository_commit": _solver_commit(),
            "source_path": "solvers/navier/navier_spectral_core.py",
            "source_sha256": _sha256(SOLVER_SOURCE),
