...
`scripts/proof_graph.py` over this tree: 965 modules, 2,662 local
...
fixed-point assembly is wired into the build-time receipt pipeline through
`Navier/ConditionalAudit.lean`). 619 modules are consumed by at least one
crown; 344 are intermediate-unconsumed research infrastructure feeding future
endpoints.
