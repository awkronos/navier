...
| F-023 | RED | Exact divergence-free triad energy cancellation forces every ordered interaction to vanish or have a favorable sign | At `37d41c1`, `Navier.Routes.R7.witness_nontermwise_cancellation` proves one admissible ordered transfer is `1` while the complete six-transfer sum is `0` | Global cancellation is collective and cannot be promoted to termwise depletion | Propose a weighted/grouped shell identity and test it on the preserved triad witness |
| F-024 | RED | An individual exact triad transfer with fixed amplitudes admits a frequency-uniform unweighted bound | At `00426ea`, `Navier.Routes.R7.orderedTransfer_unbounded_across_scale` proves that every real upper bound is exceeded by a common positive wavevector scale | Any viable cascade estimate must supply a compensating weight, derivative placement, or collective cancellation | State the shell weights and prove their scaling before testing summability |
