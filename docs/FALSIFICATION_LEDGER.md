...
| F-023 | ROUTE NOTE (static witness; not kernel-checked) | A restart engine for `RestartPaste.DatumHorizonIndependentRestart bkmVorticityControl` can be driven by the restart slice `u T₀` alone: a smooth divergence-free finite-energy slice with bounded vorticity has finite Wiener norm `‖𝓕(u T₀)‖_{L¹}`, so the Wiener local theorem (`WienerLocalClassical`, horizon `ν/(10⁶(‖a₀‖²+1))`) restarts it with a step fixed by `(ν, u₀, M)` | A continuation leaf · `RestartPaste.DatumHorizonIndependentRestart`, `WienerLocalClassical` | 2026-09-23 · branch `m/ns-0922` at `f258f5c` · static witness (paper): disjoint unit bumps at separated centres `x_k` carrying `k⁻¹ curl(φ(x-x_k) cos(k x₁) e₃)`: smooth, divergence free, `Σ k⁻² < ∞` energy, vorticity `O(1)`, Fourier `L¹` mass `≳ Σ k⁻¹ = ∞` | M (static carrier only) | A slice-only restart engine fails; the witness is a static field, not a solution slice reached from a Schwartz datum, so it does NOT refute the dynamical restart leaf | Restart must use the solution history: (U) uniqueness inside the `SolvesBefore` class, (P) a T-uniform subcritical bound `‖u t‖_{H^s}`, `s > 3/2`, from `(ν, u₀, M)`, (L) local existence from `H^s` data landing in `SolvesFrom`; or restate the continuation/a-priori leaves over a regular solution class (the crown itself is existential and unaffected) | F-022 |
| F-024 | FAITHFULNESS FINDING (decomposition, not crown) | The critical-control decomposition of crown A (`CriticalControlDecomposition.wholeSpaceGlobalRegularity_of_local_datumContinuation_apriori`, `ContinuationScaleSelfImprovement.wholeSpaceGlobalRegularity_of_localAtOne_bkmAtOne_datumRestart`) asks of its leaves only what the existential crown needs | A consumer · `APrioriCriticalControl`, `DatumContinuationFromCriticalControl`, `NSBKMUniformVorticityAprioriAtViscosityOne`, `RestartPaste.DatumHorizonIndependentRestart` | 2026-09-23 · `m/ns-0922` · audit: both leaves quantify over EVERY `SolvesBefore` solution from a Schwartz datum; that class (smooth, `L²` slices, energy ≤ initial, no dissipation integral, no decay of `∇u`/`p`) is `L∞_t L²_x`, supercritical at spatial infinity, and uniqueness in it is not known (the Fourier energy flux need not vanish at infinite frequency; weak–strong needs the dissipation integral). A pathological class member with infinite vorticity supremum would make the a priori leaf false while the existential crown `WholeSpaceGlobalRegularity` is untouched | I (quantifier audit; no witness constructed) | The universal leaves carry a hidden uniqueness burden (U) absent from the crown | Primary route restated over the regular class `ClassDecomposition.RegularOnCompacts`: `ClassDecomposition.wholeSpaceGlobalRegularity_of_local_datumContinuation_apriori_R`, local existence discharged (`WienerRegularity.localClassicalExistenceIn_R`), crown reduction `WienerRegularity.wholeSpaceGlobalRegularity_of_R_leaves`; `ClassDecomposition.aprioriR_of_apriori` (old a priori ⇒ R a priori). The R continuation leaf is NOT implied by the old one (it must return an R-solution): incomparable, weaker input and stronger output | Old consumers retained; F-023 |
| F-025 | ROUTE NOTE (static witness; paper-level, not kernel-checked) | Fourier-side (Wiener) log-Sobolev bound `∫|ξ||û(ξ)| dξ ≤ C y (1 + log(1 + Y/y) + log(1 + E/y))` with `y = ‖ω‖∞`, `Y = ‖Λ³u‖₂`, `E = ‖u‖₂²`, proposed as the rate bound feeding `DampedThresholdGronwall.threshold_gronwall_uniform` on the R-restart route | A continuation leaf · `RegularRestart.DatumHorizonIndependentRestartR bkmVorticityControl`, item 2 of the (P) plan | 2026-09-23 · branch `m/ns-0922` at `b112376` · static witness (paper): `N` well-separated translates of one divergence-free bump whose Fourier support lies in a fixed annulus, with phases chosen so that `∫|Σₖ e^{-2πiξ·xₖ}| |ξ||φ̂(ξ)| dξ ≳ √N`: `y ≍ 1`, `E ≍ N`, `Y ≍ √N`, so the right side is `O(log N)` and the left side `≳ √N` | M (static carrier only) | The Wiener-norm form of the log inequality is false: `‖ω‖∞` does not control Fourier `L¹` mass in a fixed frequency band. The witness is a static field, not a solution slice; it does NOT refute the physical-side bound on `‖∇u‖∞` | Use the physical-side inequality `‖∇u‖∞ ≤ C y (1 + log(1 + Y/y) + log(1 + E/y))` via Littlewood–Paley block kernels (the multiplier `ξξ/|ξ|²·ψ(ξ/2ʲ)` has a Schwartz kernel whose `L¹` norm does not depend on `j`), proved together with the damped `H³` inequality on the item-6 constructed solutions (every Sobolev order) and transferred to `u` by `RestartUniqueness.uniqueness_from` (orchestrator decision (b), 2026-09-23) | F-023 |
