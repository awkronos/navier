...
| Strong evolving time derivative | [CriticalMildTimeJetInterchange](../Navier/Analysis/CriticalMildTimeJetInterchange.lean) | Constructs the actual derivative in the completed weighted Fourier carrier from spatial smoothing, mode evolution and a summable uniform envelope. Its decoded coefficients are the literal mild time derivative. Higher time derivatives and joint classical reconstruction remain to be constructed. |
