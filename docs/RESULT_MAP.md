...
| Strong evolving time derivative | [CriticalMildTimeJetInterchange](../Navier/Analysis/CriticalMildTimeJetInterchange.lean), [CriticalMildSecondTimeDerivative](../Navier/Analysis/CriticalMildSecondTimeDerivative.lean) | Constructs the actual derivative in the completed weighted Fourier carrier from spatial smoothing, mode evolution and a summable uniform envelope. Differentiating the literal bilinear forcing then gives every mode's second time derivative. The second completed-carrier derivative, all-order time induction and joint classical reconstruction remain to be constructed. |
