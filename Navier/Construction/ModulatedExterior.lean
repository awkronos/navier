...
Upstream source: NavierStokes/ModulatedExterior.lean
...
namespace Navier.Construction.ModulatedExterior
...
theorem completedVelocity_before (C h : ℝ) (u : VelocityField) {z : SpaceTime}
    (ht : z.1 < 1) : completedVelocity C h u z = u z := if_pos ht

theorem completedPressure_before (C h : ℝ) (p : PressureField) {z : SpaceTime}
    (ht : z.1 < 1) : completedPressure C h p z = p z := if_pos ht
...
end Navier.Construction.ModulatedExterior
