...
Upstream source: NavierStokes/PulseCovariance.lean
...
-/
import Navier.Construction.SmoothCovariance
import Navier.Construction.FlatCovariance
import Navier.Construction.GaussianEnvelope
...
namespace Navier.Construction.PulseCovariance
...
theorem moment_integrable :
  ...
  apply ((integrable_first_gaussian_scaled (b := 2 * b) (by linarith [h.decay_pos]) h.radius_pos
    (r ^ 2 / 2)).const_mul (A ^ 2)).mono'
      (((continuous_id.sub continuous_const).abs.fun_mul h.weight_continuous).aestronglyMeasurable)
  filter_upwards [] with v
  rw [Real.norm_eq_abs, abs_of_nonneg
    (mul_nonneg (abs_nonneg _) (weight_nonneg v))]
  change |v - r ^ 2 / 2| * weight ψ x v ≤
    A ^ 2 * (|v - r ^ 2 / 2| * gaussian (2 * b) (r ^ 2 / 2) r v)
...
end Navier.Construction.PulseCovariance
