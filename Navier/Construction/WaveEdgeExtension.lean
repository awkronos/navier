...
Upstream source: NavierStokes/WaveEdgeExtension.lean
...
-/
import Navier.Construction.PrimaryCopyBounds
import Navier.Construction.PhysicalClassBounds
import Navier.Construction.FlatZeroExtension
...
namespace Navier.Construction.WaveEdgeExtension
...
theorem extension_inside (ρ : D → ℝ) (a b : ℝ) (f : D → E) {x : D}
    (hx : x ∈ window ρ a b) : extension ρ a b f x = f x := by classical exact if_pos hx

omit [NormedAddCommGroup D] [NormedSpace ℝ D] [NormedSpace ℝ E] in
theorem extension_outside (ρ : D → ℝ) (a b : ℝ) (f : D → E) {x : D}
    (hx : x ∉ window ρ a b) : extension ρ a b f x = 0 := by classical exact if_neg hx
...
theorem extendedJets_hasFDerivAt {Ω : Set D} (hΩ : IsOpen Ω) {ρ : D → ℝ}
  ...
  · have hn : x ∉ window ρ a b := by simp only [window, Set.mem_setOf_eq, heq, lt_self_iff_false, false_and, not_false_eq_true]
    rw [extension_outside ρ a b _ hn, hcurry]
...
theorem hasFTaylorSeriesUpToOn_extension {Ω : Set D} (hΩ : IsOpen Ω) {ρ : D → ℝ}
  ...
  · intro x hx
    ...
    · simp only [extendedJets, extension, if_pos hi]
      rfl
    · simp only [extendedJets, extension, if_neg hi]
      rfl
  · intro n _ x hx
...
end Navier.Construction.WaveEdgeExtension
