...
Upstream source: NavierStokes/SlowRecursion.lean
...
-/
import Navier.Construction.PositiveAxisExistence
import Navier.Construction.AxisSourceRegularity
import Navier.Construction.BoundaryAxisJets
...
namespace Navier.Construction.SlowRecursion
...
theorem priorDiffusion_value {R : ℝ} {U : Set ℂ} {h : ℝ} (c : Domain R U h)
  ...
  · simp only [priorDiffusion, PositiveAxisSystem.precedingDiffusion, if_neg hn]
...
theorem lowerHistory_apply {core buffer : ℝ} {U : Set ℂ} (hbuffer : 0 < buffer) (n : ℕ)
  ...
  simp only [lowerHistory, dif_pos hj, restrict_apply]
...
theorem sequence_expanded {core buffer : ℝ} {U : Set ℂ} {h : ℝ}
  ...
  have hS := PositiveAxisSystem.actualLowerSource_congr h
    (AxisSourceRegularity.previousOmegaDivX h (fun j => profile (H j 1)) (fun j => profile (H j 4)) (m + 1))
...
end Navier.Construction.SlowRecursion
