...
Upstream source: NavierStokes/SmoothLoop.lean
...
-/
import Navier.Construction.LoopMoments
...
namespace Navier.Construction.SmoothLoop
...
theorem extendedExpTilt_contDiff (m d μ p : ℝ) :
  ...
  by_cases hp : p = 0
  · simpa only [extendedExpTilt, if_pos hp] using cosineTilt_contDiff m (d * μ)
  · simpa only [extendedExpTilt, if_neg hp] using expTilt_contDiff m d μ p

theorem extendedExpTilt_periodic (m d μ p : ℝ) :
    Function.Periodic (extendedExpTilt m d μ p) (2 * Real.pi) := by
  by_cases hp : p = 0
  · simpa only [extendedExpTilt, if_pos hp] using cosineTilt_periodic m (d * μ)
  · simpa only [extendedExpTilt, if_neg hp] using expTilt_periodic m d μ p

theorem extendedExpTilt_mean (m d μ p : ℝ) :
    angularMean (extendedExpTilt m d μ p) = m := by
  by_cases hp : p = 0
  · simpa only [extendedExpTilt, if_pos hp] using cosineTilt_mean m (d * μ)
  · simpa only [extendedExpTilt, if_neg hp] using expTilt_mean m d μ p
...
theorem extendedExpTilt_projection_positive (p₁ p₂ m d μ : ℝ)
  ...
  · simp only [hp, zero_mul, add_zero] at hmargin ⊢
    linarith
  · simp only [extendedExpTilt, if_neg hp]
...
end Navier.Construction.SmoothLoop
