...
Upstream source: NavierStokes/PastExtension.lean
...
import Navier.Construction.SpacetimeEndpoint
...
namespace Navier.Construction.PastExtension
...
theorem zeroBefore_of_nonneg (g : SpaceTime → V) {t : ℝ} (ht : 0 ≤ t) (x : Space) :
  ...
  simp only [zeroBefore, ht, if_pos]
...
theorem zeroBefore_eventually_zero_at_zero {g : SpaceTime → V} (x : Space)
  ...
  by_cases ht : 0 ≤ z.1
  · simpa only [zeroBefore, if_pos ht] using hz
  · simp only [zeroBefore, if_neg ht]
...
theorem zeroBefore_periodic {g : SpaceTime → V}
  ...
  · simp only [zeroBefore, if_neg hnonneg]
...
end Navier.Construction.PastExtension
