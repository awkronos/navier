...
Upstream source: NavierStokes/LocalPhysicalCopyBounds.lean
...
-/
import Navier.Construction.PhysicalCopyBounds
...
namespace Navier.Construction.LocalPhysicalCopyBounds
...
theorem jet_bound_at_closure {f : E → F} {U : Set E} {x : E} {B : ℝ}
  ...
  have hcont : ContinuousAt (iteratedFDeriv ℝ j f) x :=
    (hf.iteratedFDeriv_right (m := 0) (i := j)
      (by simpa using ENat.natCast_le_of_coe_top_le_withTop le_rfl j)).continuousAt
...
end Navier.Construction.LocalPhysicalCopyBounds
