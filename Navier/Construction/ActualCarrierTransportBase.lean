...
Upstream source: NavierStokes/ActualCarrierTransportBase.lean
...
namespace Navier.Construction.ActualCarrierTransportBase
...
theorem mem_sourceRegion (l : Index B N0) (n : ℕ) (p : Parameter) (Y : Plane) :
  ...
  simp only [sourceRegion, ActualGaussianCoverage.sourceRegion, mem_inter_iff, mem_preimage,
    HarmonicSourceSupport.nativeUnion, mem_iUnion, PeriodizedWaveBounds.nativeCell, Set.mem_setOf_eq]
...
theorem labelCarrier_iff_canonicalSourceRegion
  ...
  · simpa only [canonicalSourceRegion, activeSlowCore, if_pos hn, sourceRegion] using
  ...
  · have hnot : associatedPoint p Y ∉ labelCarrier l n :=
    ...
    simp only [hnot, canonicalSourceRegion, ActualGaussianCoverage.sourceRegion,
      activeSlowCore, if_neg hn, mem_inter_iff, mem_preimage, mem_empty_iff_false, false_and]
...
end Navier.Construction.ActualCarrierTransportBase
