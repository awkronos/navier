...
Upstream source: NavierStokes/ActivationHolomorphic.lean
...
-/
import Navier.Construction.AxisHolomorphicJoint
import Navier.Construction.HolomorphicFamily
import Navier.Construction.ReferenceJetBounds
import Navier.Construction.StressActivation
...
namespace Navier.Construction.ActivationHolomorphic
...
theorem attach_logPoint (N : ReferencePath.Input) {F G : CField} {Ω : Set ℂ}
  ...
  · exact (if_pos hx).trans (heq p hp hx hz).symm
  · exact if_neg hx
...
theorem regular_attach (N : ReferencePath.Input) {R : ℝ} (hR : 0 < R) (hXR : N.endpoint < R)
  ...
  · intro x hx
    ...
    · have hf := hF.holomorphic x ⟨hx,hxe.trans_lt hXR⟩
      simpa only [attach,if_pos hxe] using hf
    · have hg := hG.holomorphic (N.logTime x) (mem_univ _)
      simpa only [attach,if_neg hxe,logPoint] using hg
...
theorem refF_real {δ : ℝ} (hδ : 0 < δ) (hδT : 2*δ < ReferencePath.rampLimit)
  ...
  · simp only [refF,attach,if_pos hx,ReferencePath.Input.refF,E.f_real]
  · simp only [refF,attach,if_neg hx,ReferencePath.Input.refF,logPoint]
...
theorem refU_real {δ : ℝ} (hδ : 0 < δ) (hδT : 2*δ < ReferencePath.rampLimit)
  ...
  by_cases hx : x ≤ N.endpoint
  · simp only [refU,attach,if_pos hx,ReferencePath.Input.refU,E.U_real]
  · simp only [refU,attach,if_neg hx,ReferencePath.Input.refU,logPoint]
...
theorem actF_real {δ : ℝ} (hδ : 0 < δ) (hδT : 2*δ < ReferencePath.rampLimit)
  ...
  by_cases hx : x ≤ N.endpoint
  · simp only [actF,attach,if_pos hx,StressActivation.FromReference.f,ReferencePath.Input.refF,E.f_real]
  · simp only [actF,attach,if_neg hx,StressActivation.FromReference.f,logPoint,StressActivation.activatedAngular]
...
theorem actU_real {δ : ℝ} (hδ : 0 < δ) (hδT : 2*δ < ReferencePath.rampLimit)
  ...
  by_cases hx : x ≤ N.endpoint
  · simp only [actU,attach,if_pos hx,StressActivation.FromReference.U,ReferencePath.Input.refU,E.U_real]
  · simp only [actU,attach,if_neg hx,StressActivation.FromReference.U,logPoint]
...
theorem actF_ne_zero (T κ δ : ℝ) {p : CPoint} (hx : 0 ≤ p.1) (hz : p.2 ∈ Ω) :
  ...
  · have hp := E.initial_half p.1 ⟨hx,he⟩ p.2 hz
    rw [actF,attach,if_pos he]
    intro hn
    simp only [hn,Complex.zero_re,lt_self_iff_false] at hp
  · simp only [actF,attach,if_neg he]
...
end Navier.Construction.ActivationHolomorphic
