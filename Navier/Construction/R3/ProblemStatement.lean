...
Upstream source: NavierStokes/R3/ProblemStatement.lean
...
operators are those of `Navier.Construction.ProblemStatement`; no periodicity assumption
...
namespace Navier.ConstructionR3.ProblemStatement

/-- R³ with its ordinary Euclidean norm. -/
abbrev Space := Navier.Construction.ProblemStatement.Space

/-- Spacetime, with time first. -/
abbrev SpaceTime := Navier.Construction.ProblemStatement.SpaceTime

abbrev VelocityField := Navier.Construction.ProblemStatement.VelocityField
abbrev PressureField := Navier.Construction.ProblemStatement.PressureField

/-- The physical domain before the asserted singular time. -/
abbrev preSingularDomain := Navier.Construction.ProblemStatement.preSingularDomain

/-- The physical domain for a global competing solution. -/
abbrev futureDomain := Navier.Construction.ProblemStatement.futureDomain
...
def navierStokesResidual (ν : ℝ) (u : VelocityField) (p : PressureField)
    (t : ℝ) (x : Space) : Space :=
  Navier.Construction.ProblemStatement.temporalDerivative u t x +
    Navier.Construction.ProblemStatement.advection u t x -
    ν • Navier.Construction.ProblemStatement.spatialLaplacian u t x +
    Navier.Construction.ProblemStatement.pressureGradient p t x
...
abbrev SpeedUnboundedAtOne := Navier.Construction.ProblemStatement.SpeedUnboundedAtOne
...
structure CandidateProperties (ν : ℝ) (u : VelocityField) (p : PressureField)
  ...
  divergence_free : ∀ t ∈ Ico (0 : ℝ) 1, ∀ x : Space,
    Navier.Construction.ProblemStatement.spatialDivergence u t x = 0
...
structure GlobalFiniteEnergySolution (ν : ℝ) (f : VelocityField) where
  ...
  divergence_free : ∀ t ∈ Ici (0 : ℝ), ∀ x : Space,
    Navier.Construction.ProblemStatement.spatialDivergence velocity t x = 0
...
@[simp] theorem residual_at_viscosity_one (u : VelocityField) (p : PressureField)
    ...
    navierStokesResidual 1 u p t x =
      Navier.Construction.ProblemStatement.navierStokesResidual u p t x := by
  simp [navierStokesResidual, Navier.Construction.ProblemStatement.navierStokesResidual]
...
theorem zero_force_has_global_solution (ν : ℝ) :
  ...
  · intro t ht x
    simp [navierStokesResidual, Navier.Construction.ProblemStatement.temporalDerivative,
...
end Navier.ConstructionR3.ProblemStatement
