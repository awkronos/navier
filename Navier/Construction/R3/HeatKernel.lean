...
Upstream source: NavierStokes/R3/HeatKernel.lean
...
theorem partial_heatKernel {s : ℝ} (hs : 0 < s) (i : Fin 3) (z : Space) :
  ...
  simp [Navier.Construction.ProblemStatement.coordinateVector,
...
theorem heatKernelSecond_eq_partial {s : ℝ} (hs : 0 < s)
  ...
  have hc : HasFDerivAt (fun x : Space => x j)
      (innerSL ℝ (Navier.Construction.ProblemStatement.coordinateVector j)) z := by
    convert! (innerSL ℝ (Navier.Construction.ProblemStatement.coordinateVector j)).hasFDerivAt
      (x := z) using 1
    ext x
    simp [Navier.Construction.ProblemStatement.coordinateVector,
      EuclideanSpace.inner_single_left]
  have hg : HasFDerivAt (fun x : Space => -(x j / (2 * s)))
      ((-((2 * s)⁻¹)) • innerSL ℝ (Navier.Construction.ProblemStatement.coordinateVector j)) z := by
    convert! hc.const_mul (-((2 * s)⁻¹)) using 1
    ext x
    ring
  rw [hfirst]
  unfold partialD Navier.Construction.PeriodicIntegration.spatialPartial
  change heatKernelSecond s i j z = fderiv ℝ ((fun x : Space => -(x j / (2 * s))) * heatKernel s) z (Navier.Construction.ProblemStatement.coordinateVector i)
  rw [(hg.mul (hasFDerivAt_heatKernel hs z)).fderiv]
  by_cases hij : i = j
  · subst i
    simp [heatKernelSecond, Navier.Construction.ProblemStatement.coordinateVector, EuclideanSpace.inner_single_right]
    ; field_simp
    ; ring
  · simp [heatKernelSecond, hij,
      Navier.Construction.ProblemStatement.coordinateVector, EuclideanSpace.inner_single_right]
    ; field_simp
    ; ring
