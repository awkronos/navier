...
Upstream source: NavierStokes/R3/LocalizedFluxEstimates.lean
...
import Navier.Construction.R3.WeightedSobolev
...
namespace Navier.ConstructionR3.LocalizedFluxEstimates

open ProblemStatement Comparison
open Navier.Construction.ProblemStatement (spatialDerivative coordinateVector)
...
theorem neg_coupling_le_weightedEnergy {χ : Space → ℝ} {u w : VelocityField} {t G : ℝ}
  ...
  change -(χ x * ⟪w (t, x), spatialDerivative u t x (w (t, x))⟫_ℝ) ≤
    G * (χ x * ‖w (t, x)‖ ^ 2)
  have h := Navier.Construction.PeriodicUniqueness.nonlinear_energy_bound
...
theorem continuous_laplacian {χ : Space → ℝ} (hχ : ContDiff ℝ ∞ χ) :
  ...
  exact continuous_finsetSum _ fun i _ =>
    (Navier.Construction.PeriodicUniqueness.spatial_partial_contDiff
      (Navier.Construction.PeriodicUniqueness.spatial_partial_contDiff hχ i) i).continuous
...
end Navier.ConstructionR3.LocalizedFluxEstimates
