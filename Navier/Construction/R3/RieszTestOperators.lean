...
Upstream source: NavierStokes/R3/RieszTestOperators.lean
...
-/
import Navier.Construction.R3.RieszSymbolRegularity
import Navier.Construction.R3.RieszL2Bounds
import Navier.Construction.R3.SmoothSobolevL6
import Navier.Construction.R3.LpNormTools
import Navier.Construction.R3.FourierTestDerivatives
...
namespace Navier.ConstructionR3.RieszTestOperators
...
abbrev partialTest (k : Fin 3) (ψ : ComplexTest) : ComplexTest :=
  LineDeriv.lineDerivOpCLM ℂ ComplexTest (Navier.Construction.ProblemStatement.coordinateVector k) ψ
...
theorem fourier_partialTest (ψ : ComplexTest) (k : Fin 3) (ξ : Space) :
    (FourierTransform.fourierCLE ℂ ComplexTest (partialTest k ψ)) ξ =
      (2 * Real.pi * Complex.I) * (ξ k : ℂ) *
        (FourierTransform.fourierCLE ℂ ComplexTest ψ) ξ := by
  simpa only [partialTest, Navier.Construction.ProblemStatement.coordinateVector,
    EuclideanSpace.inner_single_right, RCLike.conj_to_real, one_mul] using
    fourier_pderivTest ψ (Navier.Construction.ProblemStatement.coordinateVector k) ξ
...
theorem partial_rieszTest (i j : Fin 3) (ψ : ComplexTest) (k : Fin 3) (x : Space) :
  ...
  pderiv_rieszTest i j ψ (Navier.Construction.ProblemStatement.coordinateVector k) x
...
theorem norm_clm_le_sum_coordinate_norms {E : Type*} [NormedAddCommGroup E]
    ...
    ‖A‖ ≤ ∑ k : Fin 3, ‖A (Navier.Construction.ProblemStatement.coordinateVector k)‖ := by
  apply A.opNorm_le_bound (Finset.sum_nonneg fun _ _ => norm_nonneg _)
  intro v
  have hsplit : A v = ∑ k : Fin 3,
      v k • A (Navier.Construction.ProblemStatement.coordinateVector k) := by
    conv_lhs => rw [← Navier.Construction.PeriodicUniqueness.sum_coordinates v]
    simp only [map_sum, map_smul]
  rw [hsplit]
  calc
    ‖∑ k : Fin 3, v k • A (Navier.Construction.ProblemStatement.coordinateVector k)‖ ≤
        ∑ k : Fin 3, ‖v k • A (Navier.Construction.ProblemStatement.coordinateVector k)‖ :=
      norm_sum_le _ _
    _ ≤ ∑ k : Fin 3, ‖v‖ * ‖A (Navier.Construction.ProblemStatement.coordinateVector k)‖ := by
      apply Finset.sum_le_sum
      intro k _
      rw [norm_smul]
      exact mul_le_mul_of_nonneg_right (PiLp.norm_apply_le v k) (norm_nonneg _)
    _ = (∑ k : Fin 3, ‖A (Navier.Construction.ProblemStatement.coordinateVector k)‖) * ‖v‖ := by
...
theorem lpNorm_two_fderiv_rieszTest_le (i j : Fin 3) (ψ : ComplexTest) :
  ...
  have hcol (k : Fin 3) (x : Space) :
    ...
    simpa only [Comparison.partialD, Navier.Construction.PeriodicIntegration.spatialPartial,
      Navier.Construction.ProblemStatement.coordinateVector, PiLp.norm_single,
      norm_one, mul_one] using
      (fderiv ℝ (fun y => ψ y) x).le_opNorm
        (Navier.Construction.ProblemStatement.coordinateVector k)
  calc
    comparisonLpNorm 2 (fderiv ℝ (rieszTest i j ψ)) ≤
...
end Navier.ConstructionR3.RieszTestOperators
