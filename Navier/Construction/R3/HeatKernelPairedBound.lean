...
Upstream source: NavierStokes/R3/HeatKernelPairedBound.lean
...
-/
import Navier.Construction.R3.HeatKernelCancellation
import Navier.Construction.R3.HeatKernelTimeBound
import Navier.Construction.R3.RadialKernelBounds
import Navier.Construction.R3.PairedKernelBound
...
theorem heatKernelSecond_joint_measurable (i j : Fin 3) :
  ...
  have hcoord (a : Fin 3) : Measurable (fun p : ℝ × Space => p.2 a) := by
    have hc : Continuous (fun z : Space => z a) := by
      convert! (innerSL ℝ (Navier.Construction.ProblemStatement.coordinateVector a)).continuous using 1
      ext z
