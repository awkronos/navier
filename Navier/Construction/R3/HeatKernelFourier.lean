...
Upstream source: NavierStokes/R3/HeatKernelFourier.lean
...
-/
import Navier.Construction.R3.HeatKernel
import Navier.Construction.R3.GaussianMoments
import Navier.Construction.R3.RieszTestOperators
import Navier.Construction.R3.RieszHeatRepresentation
...
theorem fourierIntegral_gaussian_real {a : ℝ} (ha : 0 < a) (z : Space) :
  ...
  have hdim : Module.finrank ℝ Space = 3 := by
    simp [Space, Navier.Construction.ProblemStatement.Space]
...
private theorem partialD_ofReal {f : Space → ℝ} (hf : Differentiable ℝ f)
  ...
  unfold partialD Navier.Construction.PeriodicIntegration.spatialPartial
  change fderiv ℝ (Complex.ofRealCLM ∘ f) z
    (Navier.Construction.ProblemStatement.coordinateVector i) = _
...
private theorem differentiable_partial_heatKernel {s : ℝ} (hs : 0 < s) (j : Fin 3) :
  ...
  have hc : Differentiable ℝ (fun x : Space => x j) := by
    convert! (innerSL ℝ (Navier.Construction.ProblemStatement.coordinateVector j)).differentiable
      using 1
    ext x
    simp [Navier.Construction.ProblemStatement.coordinateVector, EuclideanSpace.inner_single_left]
...
private theorem heatKernelFourierData {s : ℝ} (hs : 0 < s) (i j : Fin 3) :
  ...
  have hd1 : partialD j (FourierTransform.fourierInv g) = FourierTransform.fourierInv f1 := by
    funext x
    unfold partialD Navier.Construction.PeriodicIntegration.spatialPartial
    simpa [f1, c, Navier.Construction.ProblemStatement.coordinateVector,
      EuclideanSpace.inner_single_right] using
      RieszTestOperators.fderiv_fourierInv_apply hg hg1 x
        (Navier.Construction.ProblemStatement.coordinateVector j)
  have hd2 : partialD i (FourierTransform.fourierInv f1) = FourierTransform.fourierInv f2 := by
    funext x
    unfold partialD Navier.Construction.PeriodicIntegration.spatialPartial
    simpa [f2, c, Navier.Construction.ProblemStatement.coordinateVector,
      EuclideanSpace.inner_single_right] using
      RieszTestOperators.fderiv_fourierInv_apply hf1 hf1moment x
        (Navier.Construction.ProblemStatement.coordinateVector i)
  constructor
  · rw [← hf2eq]
    exact hf2
  · intro z
