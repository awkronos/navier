...
Upstream source: NavierStokes/R3/CompactComparisonBounds.lean
...
namespace Navier.ConstructionR3.CompactComparisonBounds

open ProblemStatement Comparison
open Navier.Construction.ProblemStatement (spatialDerivative)
...
theorem exists_gradient_bound {T : ℝ} (hT : 0 < T) {u : VelocityField}
  ...
  obtain ⟨G, hG, hbound⟩ := Navier.Construction.PeriodicUniqueness.exists_gradient_bound hT hu hK
  refine ⟨G, hG.le, ?_⟩
  intro t ht x
  by_cases hx : x ∈ K
...
end Navier.ConstructionR3.CompactComparisonBounds
