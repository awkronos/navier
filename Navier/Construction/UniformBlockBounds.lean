...
Upstream source: NavierStokes/UniformBlockBounds.lean
...
namespace Navier.Construction.UniformBlockBounds
...
theorem pair_uniform {a : ι → ℕ → D → ℂ} (ha : UniformClass s w α a) (j m : ℤ) :
  ...
  · simpa only [ParticularWaveAssembly.pair_apply, if_pos hm, if_pos hm'] using hp.add hn
  · simpa only [ParticularWaveAssembly.pair_apply, if_pos hm, if_neg hm', map_zero, add_zero] using hp
  · simpa only [ParticularWaveAssembly.pair_apply, if_neg hm, if_pos hm', zero_add] using hn
  · simpa only [ParticularWaveAssembly.pair_apply, if_neg hm, if_neg hm', map_zero, add_zero] using hz
...
end Navier.Construction.UniformBlockBounds
