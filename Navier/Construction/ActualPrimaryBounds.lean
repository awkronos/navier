...
Upstream source: NavierStokes/ActualPrimaryBounds.lean
...
import Navier.Construction.ActualPhaseDefect
...
namespace Navier.Construction.ActualPrimaryBounds
...
theorem copied_uniformLocalJets {E : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E]
  ...
  · intro l n k x hx hk
    ...
    · rw [show copied a f l n k = fun y => coefficientScale a l n • f l (copyPoint l n k y) from
        funext (fun y => if_pos hn)]
      exact (hs l n k x hx).const_smul (coefficientScale a l n)
    · rw [show copied a f l n k = fun _ => 0 from funext (fun _ => if_neg hn)]
  ...
  · intro m
    ...
    · rw [show copied a f l n k = fun _ => 0 from funext (fun _ => if_neg hn), iteratedFDeriv_fun_zero]
...
theorem periodized_velocity_eq (l : SignedLabel B N0) (n : ℕ) {x : Native}
  ...
  · rw [ActualPrimary.chartCoefficients_amplitude_copies l.1 l.2 n (CommonWindow.index_le hn.2)]
    simp only [periodized, PeriodizedWaveBounds.copySum, copied, if_pos hn,
...
theorem periodized_cut_eq {E : Type*} [NormedAddCommGroup E] [NormedSpace ℝ E]
  ...
  · simp only [periodized, PeriodizedWaveBounds.copySum, copied, if_neg hn, tsum_zero, smul_zero]
...
theorem envelope_le_one (l : SignedLabel B N0) (n : ℕ) (x : Native) : envelope l n x ≤ 1 := by
  ...
  · have hz : ∀ k : TorusInverse.Frequency,
      ...
      exact hk
    simp only [envelope, WaveEnvelopeTransport.copyEnvelope, if_neg (hz _), tsum_zero, zero_le_one]
...
theorem actual_input_cover (l : SignedLabel B N0) (n : ℕ) {x : ActualPrimary.FullPoint}
  ...
  · right
    ...
    · filter_upwards [ha] with y hy
      simpa only [periodized, PeriodizedWaveBounds.copySum, copied, if_neg hn, tsum_zero] using hy
...
end Navier.Construction.ActualPrimaryBounds
