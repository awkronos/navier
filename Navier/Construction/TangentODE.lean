...
Upstream source: NavierStokes/TangentODE.lean
...
namespace Navier.Construction.TangentODE
...
theorem exists_linear_solution {a b : ℝ} (hab : a ≤ b)
  ...
  let v : IntervalSystem E := {
    ...
    lipschitz := hL
    continuous := by
      have hcA : Continuous (fun t : Icc a b => A t) := hA.restrict
      have hcg : Continuous (fun t : Icc a b => g t) := hg.restrict
...
end Navier.Construction.TangentODE
