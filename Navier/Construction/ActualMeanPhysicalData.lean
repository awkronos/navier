...
Upstream source: NavierStokes/ActualMeanPhysicalData.lean
...
import Navier.Construction.CycleStateCoherence
import Navier.Construction.ActualMeanPotentialRealization
import Navier.Construction.PhysicalStageBounds
import Navier.Construction.LocalMeanPhysicalBounds
...
namespace Navier.Construction.ActualMeanPhysicalData
...
theorem Atlas.physical_add {h : ℝ} {N Δ : ℕ} (A : Atlas h N Δ)
  ...
  · simp only [Atlas.physical, dif_pos hz, Pi.add_apply, mul_add]
  · simp only [Atlas.physical, dif_neg hz, Pi.add_apply, zero_add]
...
theorem Atlas.physical_sub {h : ℝ} {N Δ : ℕ} (A : Atlas h N Δ)
  ...
  funext z
  by_cases hz : ∃ n, A.Valid U z n
  · simp only [Atlas.physical, dif_pos hz, Pi.sub_apply, mul_sub]
  · simp only [Atlas.physical, dif_neg hz, Pi.sub_apply, sub_self]
...
end Navier.Construction.ActualMeanPhysicalData
