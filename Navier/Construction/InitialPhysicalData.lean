...
Upstream source: NavierStokes/InitialPhysicalData.lean
...
-/
import Navier.Construction.ActualPrimaryBounds
import Navier.Construction.ActualPrimaryDynamics
import Navier.Construction.ActualPrimaryCovariance
import Navier.Construction.ActualCarrierGeometry
import Navier.Construction.ActualSignedPhysicalData
import Navier.Construction.CartesianCopySource
import Navier.Construction.PhysicalStageBounds
import Navier.Construction.MixedAxisPreservation
import Navier.Construction.InitialNativeRegularity
...
namespace Navier.Construction.InitialPhysicalData
...
theorem carrier_bandLabel (l : SignedLabel B N0) (k : Frequency) :
  ...
  simp only [carrier, dif_pos hl, selected_eq]
...
theorem sourceFamily_eq_some {E : Type} [Zero E]
  ...
  funext x
  simp only [sourceFamily, if_pos hn, hl]
...
theorem sourceFamily_eq_zero {E : Type} [Zero E]
  ...
  · simp only [sourceFamily, if_neg hn]
...
theorem copied_self {E : Type} [NormedAddCommGroup E] [NormedSpace ℝ E]
  ...
  simp only [copied, if_pos (near_self l), coefficientScale, PhysicalParticularWave.ratioPower,
  ...
  · by_cases hL : I.1.1 ∈ active (B := B) (N0 := N0)
    · by_cases hI : I.1.2.val = 1
      · refine ⟨selected I.1.1 hL, selected_label _ _, hI, hn, ?_⟩
        simpa only [sourceFamily, if_pos hn, sourceChoice, dif_pos hL, if_pos hI] using hne
      · exact (hne (by simp only [sourceFamily, if_pos hn, sourceChoice, dif_pos hL, if_neg hI])).elim
    · exact (hne (by simp only [sourceFamily, if_pos hn, sourceChoice, dif_neg hL])).elim
  · exact (hne (by simp only [sourceFamily, if_neg hn])).elim
...
theorem potential_amplitude_data (i : Fin 3) (k : Frequency) (I : PhysicalWaveSum.WaveIndex 1)
  ...
  have ht : 0 < x.1.1 := by
    by_contra ht
    exact hx (by simp only [potentialFamily, if_neg ht])
  have hs : nativePotentialSource B N0 (I, k) I.1.val.1 (PhysicalClassBounds.cylindricalMap x) ≠ 0 := by
    intro hz
    exact hx (by simp only [potentialFamily, if_pos ht, CartesianCopySource.rotatedSource,
...
theorem pressure_amplitude_data (k : Frequency) (I : PhysicalWaveSum.WaveIndex 1)
  ...
  have ht : 0 < x.1.1 := by
    by_contra ht
    exact hx (by simp only [pressureFamily, if_neg ht])
  have hs : nativePressureSource B N0 (I, k) I.1.val.1 (PhysicalClassBounds.cylindricalMap x) ≠ 0 := by
    intro hz
    exact hx (by simp only [pressureFamily, if_pos ht, hz, smul_zero])
  obtain ⟨l, hl, hI, _, hn⟩ := sourceFamily_nonzero _ (I, k) I.1.val.1
    (PhysicalClassBounds.cylindricalMap x) hs
  have hm : I.1.val.1 = BaseChartJets.cellBand l.2 :=
    (congrArg (fun L : PhysicalWaveSum.BandLabel => L.val.1) hl).symm
...
theorem cells_bandLabel (l : SignedLabel B N0) (n : ℕ) (k : Frequency) (x : LiftPoint) :
  ...
  simp only [cells, baseCells, PeriodizedWaveBounds.nativeCells, PeriodizedWaveBounds.nativeCell,
    slowCell, dif_pos hl, selected_eq, mem_inter_iff, Set.mem_setOf_eq, mem_preimage,
    nativeAt]
  rfl
...
theorem carrier_center (L : PhysicalWaveSum.BandLabel) (k : Frequency) :
  ...
  · rw [carrier, dif_pos hL, selectedCarrier_center]
    have he : spatialLabel (selected (B := B) (N0 := N0) L hL) = L.val :=
      congrArg Subtype.val (selected_label (B := B) (N0 := N0) L hL)
    rw [he]
  · simp only [carrier, dif_neg hL, ActualSignedPhysicalData.carrier]
...
theorem carrier_integer (L : PhysicalWaveSum.BandLabel) (k : Frequency) :
  ...
  · refine ⟨PrimaryGeometryAssembly.angularMode ActualPrimary.certificate ActualPrimary.modulation
    ...
    rw [carrier, dif_pos hL, ← selected_band L hL]
...
theorem carrierProfiles :
  ...
  · intro i
    ...
    · change ContDiffOn ℝ ∞ (fun p => ((carrier (B := B) (N0 := N0) i.2 i.1).F p,
      ...
      simp only [carrier, profileRegion, dif_pos hL]
      exact hfg.smooth (selected i.2 hL).2
    · intro p hp
      have hfalse : False := by
        simp only [PhysicalCopyBounds.copyBandDomain, profileRegion, dif_neg hL, mem_empty_iff_false] at hp
      exact hfalse.elim
  · intro m
    ...
    · change x ∈ profileRegion (B := B) (N0 := N0) i.2 at hx
      simp only [profileRegion, dif_pos hL] at hx
      change ‖iteratedFDeriv ℝ j (fun p => ((carrier (B := B) (N0 := N0) i.2 i.1).F p,
        (carrier (B := B) (N0 := N0) i.2 i.1).G p)) x‖ ≤
        C * ChartScales.S i.2.val.1 ^ p
      rw [carrier, dif_pos hL, ← selected_band i.2 hL]
      exact hb (selected i.2 hL).2 j hj x hx
    · have hfalse : False := by
        simp only [PhysicalCopyBounds.copyBandDomain, profileRegion, dif_neg hL, mem_empty_iff_false] at hx
      exact hfalse.elim
...
theorem profileRegion_contains (L : PhysicalWaveSum.BandLabel) (k : Frequency)
  ...
  · change _ ∈ slowCell (B := B) (N0 := N0) L at hs
    simp only [slowCell, dif_pos hL, mem_preimage] at hs
    ...
    simp only [profileRegion, dif_pos hL]
  ...
  · simp only [slowCell, dif_neg hL, mem_empty_iff_false] at hs
...
theorem potential_amplitude_source (B N0 : ℕ) (i : Fin 3) (k : Frequency)
  ...
  intro x hx
  simp only [potentialFamily, if_pos (sourceStrip_time hx)]
  rfl
...
theorem pressure_amplitude_source (B N0 : ℕ) (k : Frequency)
  ...
  intro x hx
  simp only [pressureFamily, if_pos (sourceStrip_time hx)]
  rfl
...
theorem carrier_frequencies (L : PhysicalWaveSum.BandLabel) (k : Frequency) :
  ...
  by_cases hL : L ∈ active (B := B) (N0 := N0)
  · simp only [carrier, dif_pos hL, selectedCarrier, ActualSignedPhysicalData.carrier]
    exact ActualPhaseJetBounds.phase_constants_bound (selected L hL)
  · simp only [carrier, dif_neg hL, ActualSignedPhysicalData.carrier, abs_zero]
...
theorem potential_amplitude_smooth (B N0 : ℕ) (i : Fin 3) (k : Frequency)
  ...
  simp only [potentialFamily, if_pos ht]
  rfl
...
theorem pressure_amplitude_smooth (B N0 : ℕ) (k : Frequency)
  ...
  intro x hx
  have ht : 0 < x.1.1 := hx.2
  simp only [pressureFamily, if_pos ht, Function.comp_apply]
...
theorem cut_amplitude_self (l : SignedLabel B N0) (x : Point) :
  ...
  have hu : periodized (CoordinateAlgebra.A ActualPrimary.h)
    ...
    simp only [periodized, PeriodizedWaveBounds.copySum, copied, if_pos hn, ← tsum_const_smul'']
...
theorem nativePotentialSource_active (l : SignedLabel B N0) (k : Frequency) :
  ...
  simp only [sourceChoice, positiveIndex, ActualSignedPhysicalData.positiveIndex,
    dif_pos hl, selected_eq, ite_true]
...
theorem potential_amplitude_active (l : SignedLabel B N0) (k : Frequency) (i : Fin 3)
  ...
  simp only [potentialFamily, if_pos ht, CartesianCopySource.rotatedSource,
...
theorem pressure_amplitude_active (l : SignedLabel B N0) (k : Frequency)
  ...
  simp only [pressureFamily, if_pos ht, positiveIndex_band, nativePressureSource_active]
...
end Navier.Construction.InitialPhysicalData
