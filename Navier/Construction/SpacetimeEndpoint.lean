...
Upstream source: NavierStokes/SpacetimeEndpoint.lean
...
namespace Navier.Construction.SpacetimeEndpoint
...
theorem extendTrace_of_lt {V : Type*} {T : ℝ} {f : SpaceTime → V} {L : Space → V}
  ...
  simp only [extendTrace, if_pos hz]
...
theorem extendTrace_periodic {T : ℝ} {f : SpaceTime → V} {L : Space → V} (v : Space)
  ...
  by_cases ht : t < T
  · simp only [extendTrace, if_pos ht]
    exact hperiod t ht x
  · simp only [extendTrace, if_neg ht]
...
theorem hasFTaylorSeriesUpToOn_extension {T : ℝ} {f : SpaceTime → V}
  ...
  · intro z hz
    by_cases hzt : z.1 < T
    · simpa only [extendJets, extendTrace, if_pos hzt] using hzero z hzt
    · simp only [extendJets, extendTrace, if_neg hzt]
  · intro n hn z hz
...
end Navier.Construction.SpacetimeEndpoint
