...
Upstream source: NavierStokes/HarmonicResidual.lean
...
namespace Navier.Construction.HarmonicResidual
...
@[simp] theorem realCoefficients_apply (c : Coefficients D) (j : ℤ) (x : D) :
  ...
  rw [hmul]
  simp only [neg_zero, zero_add, Pi.mul_apply]
  rfl
...
theorem realCoefficients_eq_self {c : Coefficients D} (hc : ConjugateSymmetric c) :
  ...
  ext j x
  change (realCoefficients c).coeff j x = c.coeff j x
...
theorem coefficients_unique {c d : Coefficients D} (k : ℝ) (Φ : D → ℝ)
  ...
  ext j x
  change c.coeff j x = d.coeff j x
...
end Navier.Construction.HarmonicResidual
