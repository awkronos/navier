...
Upstream source: NavierStokes/HarmonicWaveInteraction.lean
...
namespace Navier.Construction.HarmonicWaveInteraction
...
theorem nonconstant_real_sub_axisymmetric (a b : Coefficients D) (hb : BandLimited b 0) :
  ...
  ext j x
  change (HarmonicResidual.nonconstant (HarmonicResidual.realCoefficients (a - b))).coeff j x =
    (HarmonicResidual.nonconstant (HarmonicResidual.realCoefficients a)).coeff j x
  by_cases hj : j = 0
  · subst j
    simp [HarmonicResidual.nonconstant]
...
@[simp] theorem differentiate_single_apply (V : D → D) (k : ℝ) (Φ : D → ℝ)
    (j m : ℤ) (f : D → ℂ) (x : D) :
    differentiate V k Φ (AddMonoidAlgebra.single j f) m x =
      if m = j then derivativeCoefficient V k Φ j f x else 0 := by
  rw [differentiate_apply, single_coeff]
  by_cases hm : m = j
  · subst m
    simp
  · rw [if_neg (Ne.symm hm), derivativeCoefficient_zero, if_neg hm]
    rfl

omit [NormedAddCommGroup D] [NormedSpace ℝ D] in
@[simp] theorem angularDifferentiate_single_apply (kp : ℤ) (j m : ℤ)
    (f : D → ℂ) (x : D) :
    angularDifferentiate kp (AddMonoidAlgebra.single j f) m x =
      if m = j then (Complex.I * ((j * kp : ℤ) : ℂ)) * f x else 0 := by
  rw [angularDifferentiate_apply, single_coeff]
  by_cases hm : m = j
  · subst m
...
theorem divergenceCoefficients_single (g : HarmonicResidual.Frame D)
  ...
  ext m x
  change (divergenceCoefficients g k Φ kp
      (fun i => AddMonoidAlgebra.single j (a i j))).coeff m x =
    (AddMonoidAlgebra.single j (divergenceCoefficients g k Φ kp a j) :
      Coefficients D).coeff m x
  simp only [divergenceCoefficients, HarmonicMeanInteraction.coeff_add]
  rw [differentiate_single_apply, HarmonicMeanInteraction.constant_mul,
    HarmonicMeanInteraction.constant_mul, angularDifferentiate_single_apply,
    differentiate_single_apply]
  simp_rw [single_coeff]
  by_cases hm : m = j
  · subst m
    simp only [if_pos]
    simp_rw [HarmonicMeanInteraction.coeff_add]
    rw [differentiate_apply, HarmonicMeanInteraction.constant_mul,
      HarmonicMeanInteraction.constant_mul, angularDifferentiate_apply,
      differentiate_apply]
    ring
  · simp [hm, Ne.symm hm]
...
end Navier.Construction.HarmonicWaveInteraction
