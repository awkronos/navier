...
Upstream source: NavierStokes/EndpointExtension.lean
...
namespace Navier.Construction.EndpointExtension
...
theorem glue_eqOn_right {V : Type*} {a : ℝ} {left right : ℝ → V}
  ...
  · have hEq : x = a := le_antisymm hxa hx
    subst x
    simpa only [glue, if_pos le_rfl] using hvalue
  · simp only [glue, if_neg hxa]
...
theorem hasDerivAt_glue {a : ℝ} {left right leftDeriv rightDeriv : ℝ → V}
  ...
  · simpa only [glue, if_pos hlt.le] using (hL x hlt.le).hasDerivAt (Iic_mem_nhds hlt)
  · subst x
    have hright' : HasDerivWithinAt (glue a left right) (leftDeriv a) (Ici a) a := by
      rw [hderiv]
      exact hR a le_rfl
    have h := (hL a le_rfl).union hright'
    simpa only [Iic_union_Ici, hasDerivWithinAt_univ, glue, if_pos le_rfl] using h
...
end Navier.Construction.EndpointExtension
