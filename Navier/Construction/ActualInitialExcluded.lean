...
Upstream source: NavierStokes/ActualInitialExcluded.lean
...
-/
import Navier.Construction.GaugeExcludedBounds
import Navier.Construction.ActualBaseResidual
import Navier.Construction.GlobalBaseError
import Navier.Construction.SlowBaseEndpoint
import Navier.Construction.ActualPrimaryBounds
import Navier.Construction.ActualPrimaryDynamics
import Navier.Construction.ActualGaussianCoverage
import Navier.Construction.ActualPrimaryCovariance
import Navier.Construction.ActualInitialMean
import Navier.Construction.ActualInitialCoherence
import Navier.Construction.ActualPhaseJetBounds
...
namespace Navier.Construction.ActualInitialExcluded
...
theorem normalizedBase_smooth (n : ℕ) :
  ...
  have he : ContDiffOn ℝ ∞
      (fun p => FinalSlowBase.error H v upper B
        (BaseChartJets.bandPoint F.data.h (ChartScales.Q n) p))
      (BaseContextAssembly.slowCarrier W U) := by
...
theorem copiedGaussian_all_gains (a β : ℝ) :
  ...
  have hlarge : LocalClass nativeStrip
    ...
    · exact Or.inl ⟨hn, hi⟩
    · exact Or.inr (Filter.Eventually.of_forall (fun _ => if_neg hn))
...
theorem periodizedGaussian_all_gains (a β : ℝ) :
  ...
  · intro l n k x hx
    ...
    · have hf : nativeGaussian l (copyPoint l n k x) ≠ 0 := by
        intro hz
        exact hx (by simp only [copied, if_pos hn, hz, smul_zero])
      exact nativeGaussian_core l _ hf
    · exact (hx (by simp only [copied, if_neg hn])).elim
...
theorem labelCarrier_closed (l : SignedLabel B N0) (n : ℕ) : IsClosed (labelCarrier l n) := by
  ...
  have he : labelCarrier l n = q ⁻¹' ActualGaussianCoverage.sourceRegion
    ...
    ext x
    simp only [labelCarrier, Set.mem_setOf_eq, mem_preimage, ActualGaussianCoverage.sourceRegion,
...
theorem periodizedGaussian_eq (l : SignedLabel B N0) (n : ℕ) {x : Native}
  ...
  have hs (k : TorusInverse.Frequency) : support (copied a nativeGaussian l n k) ⊆
    ...
    · have hg : nativeGaussian l (copyPoint l n k y) ≠ 0 := by
        intro hz
        exact hy (by simp only [copied, if_pos hn, hz, smul_zero])
      exact nativeGaussian_core l _ hg
    · exact (hy (by simp only [copied, if_neg hn])).elim
  by_cases hn : near l n
  · by_cases hc : ∃ k, x ∈ (copyCells l).carrier n k
    · obtain ⟨k, hk⟩ := hc
      ...
      exact (PeriodizedWaveBounds.copySum_germ (copyCells l) n
        (copied a nativeGaussian l n) hs hk).eq_of_nhds.trans (if_pos hn)
  ...
  · have hzero : (ActualPrimary.chartCoefficients l.1 l.2).amplitude n
      ...
      simp only [periodized, PeriodizedWaveBounds.copySum, copied, if_neg hn, tsum_zero]
    simp only [periodized, PeriodizedWaveBounds.copySum, copied, if_neg hn, tsum_zero,
...
end Navier.Construction.ActualInitialExcluded
