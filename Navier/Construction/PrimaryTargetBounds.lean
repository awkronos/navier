...
Upstream source: NavierStokes/PrimaryTargetBounds.lean
...
-/
import Navier.Construction.BasePhaseGeometry
import Navier.Construction.AlignedProfileSpectralCone
import Navier.Construction.PrimaryGeometryAssembly
...
namespace Navier.Construction.PrimaryTargetBounds
...
theorem modelSet_compact (M u eta : ℝ) : IsCompact (modelSet M u eta) := by
  ...
  ext p
  simp only [modelSet, Set.mem_setOf_eq, mem_inter_iff, mem_prod, Metric.mem_sphere,
    dist_zero_right]
  tauto
...
theorem stripWeight_eq (r eta : ℝ) (hr : 0 < r) :
  ...
  rw [if_pos (by positivity : 0 < r^2/2)]
...
end Navier.Construction.PrimaryTargetBounds
