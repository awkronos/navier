...
Upstream source: NavierStokes/NaturalEntrance.lean
...
namespace Navier.Construction.NaturalEntrance
...
theorem compact_absorption {K : Type*} [TopologicalSpace K] [CompactSpace K]
  ...
  have hc : ∃ a ∈ s, b < a * χ p + R p := by
    simpa only [U, mem_iUnion, Set.mem_setOf_eq, exists_prop] using hs (mem_univ p)
  obtain ⟨a, ha, hpa⟩ := hc
  have ham : a ≤ M := by
    have hterm : |a| ≤ ∑ a ∈ s, |a| :=
...
end Navier.Construction.NaturalEntrance
