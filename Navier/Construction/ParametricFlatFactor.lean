...
Upstream source: NavierStokes/ParametricFlatFactor.lean
...
namespace Navier.Construction.ParametricFlatFactor
...
theorem bounded_coefficient_jets {b : E × ℝ → ℝ} (hb : ContDiff ℝ ∞ b)
  ...
  have hkbound (k : ℕ) : ∃ B : ℝ, 0 ≤ B ∧ ∀ z : E × ℝ, ‖z‖ ≤ R →
    ...
    have hcont := hb.continuous_iteratedFDeriv
      (m := k) (le_of_lt (WithTop.coe_lt_coe.mpr (ENat.coe_lt_top k)))
...
end Navier.Construction.ParametricFlatFactor
