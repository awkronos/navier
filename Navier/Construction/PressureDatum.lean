...
Upstream source: NavierStokes/PressureDatum.lean
...
namespace Navier.Construction.PressureDatum
...
theorem exponent_mass_pos_of_active {g a : ℝ → ℝ} {A : ℝ}
  ...
  have heq : Function.support (fun y => g y * a y) = {y | 0 < g y ∧ 0 < a y} := by
    ext y
    simp only [Function.mem_support, Set.mem_setOf_eq, ne_eq, mul_eq_zero, not_or]
    constructor
    · rintro ⟨hg, ha⟩
...
end Navier.Construction.PressureDatum
