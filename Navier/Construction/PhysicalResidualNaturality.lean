...
Upstream source: NavierStokes/PhysicalResidualNaturality.lean
...
namespace Navier.Construction.PhysicalResidualNaturality
...
theorem constant {U : Set D} {e : D ≃L[ℝ] E} {a : ℝ}
  ...
  by_cases hj : j = 0
  · subst j
    simpa only [constantCoefficient_coeff, if_pos] using h x hx
  · simp only [constantCoefficient_coeff, if_neg hj, Pi.zero_apply, smul_zero]
...
theorem linearResidual (H : FrameOn U e c l g r) (hU : IsOpen U) (hl : l ≠ 0)
  ...
  have hv : CoefficientsOn U e (c*c*l)
    ...
    simp only [constant_mul, he, Complex.real_smul, Complex.ofReal_mul]
...
theorem scalarInvRadius (hl : l ≠ 0) :
  ...
  simp only [constantCoefficient_coeff, if_pos, Complex.real_smul,
...
end Navier.Construction.PhysicalResidualNaturality
