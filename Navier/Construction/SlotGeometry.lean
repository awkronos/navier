...
Upstream source: NavierStokes/SlotGeometry.lean
...
namespace Navier.Construction.SlotGeometry
...
private theorem zero_mem_safeOffsets (m D : ℕ) : (0 : Plane × Plane) ∈ safeOffsets m D := by
  ...
  · apply mem_iInter.mpr
    ...
    · rw [separationOffsets, if_pos hh]
      simpa only [Set.mem_setOf_eq, Prod.fst_zero, Prod.snd_zero, add_zero] using
        centers_avoid_covering m D i j n.val (Nat.le_of_lt_succ n.isLt) hh
    · rw [separationOffsets, if_neg hh]
      exact mem_univ _
...
theorem exists_common_radius (m D : ℕ) :
  ...
  · intro n hn i j hij x hx y hy
    ...
    simpa only [separationOffsets, n', hij, if_pos, Set.mem_setOf_eq, hcx, hcy] using hs
...
end Navier.Construction.SlotGeometry
