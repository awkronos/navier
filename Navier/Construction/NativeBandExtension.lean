...
Upstream source: NavierStokes/NativeBandExtension.lean
...
-/
import Navier.Construction.WaveEdgeExtension
...
namespace Navier.Construction.NativeBandExtension
...
theorem jets_zero_at_closure {f : D → E} {s : Set D} {x : D}
  ...
  have hc : ContinuousAt (iteratedFDeriv ℝ n f) x :=
    (hf.iteratedFDeriv_right (m := 0) (i := n)
      (by simpa using nat_le_infty n)).continuousAt
...
end Navier.Construction.NativeBandExtension
