...
Upstream source: NavierStokes/AnalyticCoefficientBounds.lean
...
-/
import Navier.Construction.AxisCoefficientSpace
...
namespace Navier.Construction.AnalyticCoefficientBounds
...
theorem UnitHolomorphic.degreeZeroJet_continuous {I : Window} {ρ : ℝ} {f : ℂ → ℂ}
  ...
  · simpa only [degreeZeroJet, if_neg hn] using
...
theorem UnitHolomorphic.degreeZeroJet_bound {I : Window} {ε ρ : ℝ} {f : ℂ → ℂ}
  ...
  · simp only [degreeZeroJet, if_neg hn, abs_zero]
...
theorem uniform_normalizedExp_axisData
  ...
  · intro x hx
    rw [hf.coefficient_toAxisSpace hε hερ 0 hx, if_pos rfl]
    exact normalizedExp_re_of_real (hreal x hx)
  · intro n hn x hx
    rw [hf.coefficient_toAxisSpace hε hερ n hx, if_neg hn]
...
end Navier.Construction.AnalyticCoefficientBounds
