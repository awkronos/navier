...
Upstream source: NavierStokes/SlowStressSupport.lean
...
namespace Navier.Construction.SlowStressSupport
...
theorem stress_of_pos (m : ℕ) (F : Field) {w : ℝ × ℝ} (hw : 0 < w.1) :
    stress m F w = -ProfileHistories.primitive F w / w.1 ^ m := if_pos hw
...
end Navier.Construction.SlowStressSupport
