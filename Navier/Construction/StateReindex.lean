...
Upstream source: NavierStokes/StateReindex.lean
...
namespace Navier.Construction.StateReindex
...
theorem coefficients_mul (e : D ≃ₗᵢ[ℝ] E) (a b : HarmonicFields.Coefficients E) :
    coefficients e (a * b) = coefficients e a * coefficients e b := by
  ext j x
  change (coefficients e (a * b)).coeff j x = (coefficients e a * coefficients e b).coeff j x
...
theorem coefficients_constant (e : D ≃ₗᵢ[ℝ] E) (a : E → ℂ) :
  ...
  ext j x
  change (coefficients e (HarmonicFields.constantCoefficient a)).coeff j x =
    (HarmonicFields.constantCoefficient (fun x => a (e x))).coeff j x
  rw [coefficients_apply]
  change (Finsupp.single (0 : ℤ) a) j (e x) =
    (Finsupp.single (0 : ℤ) (fun x => a (e x))) j x
  by_cases hj : j = 0
  · subst j
    simp
  · simp [hj]
...
theorem differentiate_pull (e : D ≃ₗᵢ[ℝ] E) (V : E → E) (k : ℝ)
  ...
  ext j x
  change (HarmonicFields.differentiate (vector e V) k (fun x => Phi (e x))
    (coefficients e a)).coeff j x = (coefficients e (HarmonicFields.differentiate V k Phi a)).coeff j x
...
theorem realCoefficients_pull (e : D ≃ₗᵢ[ℝ] E) (a : HarmonicFields.Coefficients E) :
  ...
  ext j x
  change (HarmonicResidual.realCoefficients (coefficients e a)).coeff j x =
    (coefficients e (HarmonicResidual.realCoefficients a)).coeff j x
...
theorem nonconstant_pull (e : D ≃ₗᵢ[ℝ] E) (a : HarmonicFields.Coefficients E) :
  ...
  ext j x
  change (HarmonicResidual.nonconstant (coefficients e a)).coeff j x =
    (coefficients e (HarmonicResidual.nonconstant a)).coeff j x
  by_cases hj : j = 0
  · subst j
...
theorem coefficients_roundtrip (e : D ≃ₗᵢ[ℝ] E) (a : HarmonicFields.Coefficients D) :
  ...
  ext j x
  change (coefficients e (coefficients e.symm a)).coeff j x = a.coeff j x
...
end Navier.Construction.StateReindex
