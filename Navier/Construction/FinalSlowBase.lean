...
Upstream source: NavierStokes/FinalSlowBase.lean
...
import Navier.Construction.ModulatedExterior
import Navier.Construction.AlignedProfileSpectralCone
...
theorem leading_lowerBound (hcone : LeadingStressWeights.FullTrueCone v) :
  ...
  simp only [weight, BaseResidual.activeZeta, ActiveAnnulusWeight.radialWeight, if_pos hx,
...
theorem completedVelocity_before (upper : ℝ) (B : ℕ) {z : ProblemStatement.SpaceTime} (ht : z.1 < 1) :
    completedVelocity H v upper B z = velocity H v upper B z := if_pos ht

theorem completedPressure_before (upper : ℝ) (B : ℕ) {z : ProblemStatement.SpaceTime} (ht : z.1 < 1) :
    completedPressure H v upper B z = pressure H v upper B z := if_pos ht
