...
Upstream source: NavierStokes/HarmonicFields.lean
...
-/
import Navier.Construction.HarmonicCalculus
import Navier.Construction.MeanResidual
...
namespace Navier.Construction.HarmonicFields
...
instance (priority := 2000) {α : Type*} : CoeFun (Coefficients α) (fun _ => ℤ → α → ℂ) :=
  ⟨fun c => c.coeff⟩
...
theorem evaluate_single {α : Type*} (j : ℤ) (a : α → ℂ) (x : α) (φ : ℝ) :
  ...
  simp [evaluate_eq_hom, evaluateHom, AddMonoidAlgebra.liftNCRingHom_single]
  left
  rfl
...
theorem convolution_apply {α : Type*} (c d : Coefficients α) (m : ℤ) (x : α) :
  ...
  have hmul := congrFun (AddMonoidAlgebra.mul_apply c d m) x
  change (c * d).coeff m x = _ at hmul
  rw [hmul]
  ...
  change (if m - j ∈ d.coeff.support then
      c.coeff j x * d.coeff (m - j) x else 0) =
    c.coeff j x * d.coeff (m - j) x
  by_cases hm : m - j ∈ d.coeff.support
  · rw [if_pos hm]
  · have hd : d (m - j) = 0 := Finsupp.notMem_support_iff.mp hm
    rw [if_neg hm]
    change 0 = c.coeff j x * d.coeff (m - j) x
    rw [hd]
    simp
...
theorem conjugateReverse_eq_self {α : Type*} {c : Coefficients α}
  ...
  intro j
  funext x
  change conj (c.coeff (-j) x) = c.coeff j x
  rw [hc j x]
  simp
...
end Navier.Construction.HarmonicFields
