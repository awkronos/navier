...
Upstream source: NavierStokes/StressActivation.lean
...
namespace Navier.Construction.StressActivation
...
theorem f_eq_reference (T κ δ : ℝ) {p : Point} (hp : p.1 ≤ N.endpoint) :
    f N T κ δ p = N.refF δ p := if_pos hp

theorem U_eq_reference (T κ δ : ℝ) {p : Point} (hp : p.1 ≤ N.endpoint) :
    U N T κ δ p = N.refU δ p := if_pos hp
...
end Navier.Construction.StressActivation
