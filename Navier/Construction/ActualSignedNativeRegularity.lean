...
Upstream source: NavierStokes/ActualSignedNativeRegularity.lean
...
import Navier.Construction.ActualSignedPhysicalBinding
import Navier.Construction.FlatDyadicExtension
import Navier.Construction.ActualSignedUnmaskedBounds
import Navier.Construction.ActualSignedUnmaskedBinding
...
namespace Navier.Construction.ActualSignedNativeRegularity
...
theorem norm_iteratedFDeriv_linear_pull {D F V : Type}
  ...
  have he := e.iteratedFDerivWithin_comp_right hg hΩ.uniqueDiffOn
    (hΩ.preimage e.continuous).uniqueDiffOn hx
      (show (m : WithTop ℕ∞) ≤ ∞ from le_of_lt (WithTop.coe_lt_coe.mpr (ENat.coe_lt_top m)))
...
theorem own_native_smooth {f : Label B N0 → TorusInverse.Frequency → ℕ → FullPoint → E} {α : ℝ}
  ...
  simpa only [ownField, if_pos rfl, ite_true] using hh
...
theorem native_potential_source_factor (L : NativeLabel B N0)
  ...
  · have hm : I.1.1 ∈ (branch P u H hp L).active := by
    ...
    erw [ActualSignedPhysicalData.nativePotentialSource, dif_pos hm]
  ...
  · have hm : I.1.1 ∉ (branch P u H hp L).active := by
    ...
    simp only [ActualSignedPhysicalData.nativePotentialSource, dif_neg hm, hL, false_and, ite_false]
...
theorem native_pressure_source_factor (L : NativeLabel B N0)
  ...
  · have hm : I.1.1 ∈ (branch P u H hp L).active := by
      ...
      exact mem_singleton_iff.mpr hL
    have hv : I.1.1.val = L.val := congrArg Subtype.val hL
    erw [ActualSignedPhysicalData.nativePressureSource, dif_pos hm]
    simp only [hL, true_and]
    by_cases hi : I.1.2.val = 1 ∧ n = L.val.1
    · simp only [if_pos hi]
  ...
  · have hm : I.1.1 ∉ (branch P u H hp L).active := by
      change I.1.1 ∉ {(L : PhysicalWaveSum.BandLabel)}
      exact fun h => hL (mem_singleton_iff.mp h)
    simp only [ActualSignedPhysicalData.nativePressureSource, dif_neg hm, hL, false_and, ite_false]
...
end Navier.Construction.ActualSignedNativeRegularity
