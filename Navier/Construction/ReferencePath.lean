...
Upstream source: NavierStokes/ReferencePath.lean
...
-/
import Navier.Construction.ProfileHistories
import Navier.Construction.NaturalProfile
...
namespace Navier.Construction.ReferencePath
...
theorem refF_eq_natural {δ : ℝ} (hδ : 0 < δ) (hδT : 2 * δ < rampLimit)
  ...
  · have hxpos : 0 < p.1 := N.endpoint_pos.trans (lt_of_not_ge hx)
    ...
    rw [refF, if_neg hx, continuation_eq_natural rampLimit_pos hδ hδT
...
theorem refU_eq_natural {δ : ℝ} (hδ : 0 < δ) (hδT : 2 * δ < rampLimit)
  ...
  · have hxpos : 0 < p.1 := N.endpoint_pos.trans (lt_of_not_ge hx)
    rw [refU, if_neg hx, continuation_eq_natural rampLimit_pos hδ hδT
...
theorem refF_eq_logtime {δ : ℝ} (hδ : 0 < δ) (hδT : 2 * δ < rampLimit)
  ...
  · have ht : N.logTime p.1 ≤ 0 := (N.logTime_le_iff hX).2 (by simpa only [Real.exp_zero, mul_one] using hx)
    ...
    rw [refF, if_pos hx, continuation_eq_natural rampLimit_pos hδ hδT
...
theorem refU_eq_logtime {δ : ℝ} (hδ : 0 < δ) (hδT : 2 * δ < rampLimit)
  ...
  by_cases hx : p.1 ≤ N.endpoint
  · have ht : N.logTime p.1 ≤ 0 := (N.logTime_le_iff hX).2 (by simpa only [Real.exp_zero, mul_one] using hx)
    rw [refU, if_pos hx, continuation_eq_natural rampLimit_pos hδ hδT
...
end Navier.Construction.ReferencePath
