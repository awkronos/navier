...
Upstream source: NavierStokes/VolterraParity.lean
...
namespace Navier.Construction.VolterraParity
...
theorem glue_jointly_continuous {R : ℝ} {U : Set ℂ} {Wp Wm : Field}
  ...
  · apply hp.mono
    intro p h
    have hh : 0 ≤ p.1 := by
      simpa only [(isClosed_le continuous_const continuous_fst).closure_eq, Set.mem_setOf_eq] using h.2
...
end Navier.Construction.VolterraParity
