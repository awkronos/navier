...
Upstream source: NavierStokes/AlignedProfileSpectralCone.lean
...
-/
import Navier.Construction.ProfileSpectralCone
import Navier.Construction.EntranceAlignedBase
...
namespace Navier.Construction.AlignedProfileSpectralCone
...
private theorem gluedDirection_left_germ {J : Set ℝ} {a b cL cR : ℝ}
  ...
  by_cases hqa : q.2 ≤ a
  · simp only [gluedDirection, if_pos hqa]
  · simp only [gluedDirection, if_neg hqa, if_neg (not_le.mpr hqb)]
...
private theorem gluedDirection_right_germ {J : Set ℝ} {a b cL cR : ℝ}
  ...
  simp only [gluedDirection, if_neg (not_le.mpr hqa)]
  by_cases hqb : b ≤ q.2
  · simp only [if_pos hqb]
  · simp only [if_neg hqb]
...
theorem exists_closed_direction (hcone : LeadingStressWeights.FullTrueCone v) :
  ...
  · subst y
    ...
    simpa only [D, gluedDirection, if_pos le_rfl, sub_self, L, planeEdgeFactor,
      logShear] using ht
  ...
  · subst y
    ...
    simpa only [D, gluedDirection, if_neg (not_le.mpr hab), if_pos le_rfl,
...
end Navier.Construction.AlignedProfileSpectralCone
