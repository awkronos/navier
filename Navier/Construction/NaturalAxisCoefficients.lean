...
Upstream source: NavierStokes/NaturalAxisCoefficients.lean
...
import Navier.Construction.AnalyticPrimitive
import Navier.Construction.NaturalAxisData
import Navier.Construction.NaturalAxisBridge
...
namespace Navier.Construction.NaturalAxisCoefficients
...
theorem boundedAxisElement_coefficient {I : Window} {ε ρ B : ℝ} {f : ℂ → ℂ}
  ...
  by_cases hn : n = 0
  · rw [if_pos hn, if_pos hn, Complex.div_ofReal_re]
...
theorem CoefficientFamily.radiallyConstant {h j σ : ℝ} {P : ℝ → ℝ}
  ...
  intro n hn x hx
  rw [v.coefficient_eq k n x hx, if_neg hn]
...
theorem CoefficientFamily.value {h j σ : ℝ} {P : ℝ → ℝ}
  ...
  exact (v.coefficient_eq k 0 x hx).trans (if_pos rfl)
...
end Navier.Construction.NaturalAxisCoefficients
