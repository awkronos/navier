...
Upstream source: NavierStokes/PhysicalStageBounds.lean
...
namespace Navier.Construction.PhysicalStageBounds
...
theorem addBaseAtZero_pos (base : X → V) (increments : ℕ → X → V) {j : ℕ} (hj : 1 ≤ j) :
  ...
  funext x
  simp only [addBaseAtZero, if_neg (by omega : j ≠ 0)]
...
end Navier.Construction.PhysicalStageBounds
