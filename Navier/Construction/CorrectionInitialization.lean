...
Upstream source: NavierStokes/CorrectionInitialization.lean
...
import Navier.Construction.PartitionedCovariance
import Navier.Construction.LinearWaveBounds
import Navier.Construction.TemporalMeanUpdate
import Navier.Construction.MeanRankUpdate
...
import Navier.Construction.StateMomentBalances
import Navier.Construction.SignedWaveUpdate
import Navier.Construction.VariableGaugeMean
import Navier.Construction.PrimaryMaterialDefect
import Navier.Construction.PrimaryResidualClass
...
import Navier.Construction.BaseContextAssembly
import Navier.Construction.CommonBaseContext
import Navier.Construction.PeriodicPhaseAssembly
import Navier.Construction.BaseRankPatch
import Navier.Construction.ActualSignedGeometry
import Navier.Construction.NativeBandExtension
...
namespace Navier.Construction.CorrectionInitialization
...
theorem excluded_mean_zero (p : PrimaryPiece (D × ℝ))
  ...
  simpa only [hm, if_neg (by norm_num : (1 : ℤ) ≠ 0), Pi.zero_apply,
    Int.neg_zero, star_zero, zero_add, Complex.zero_re]
...
theorem conjugatePair_class {s : StripData D} {P : ℕ → D → ℝ} {α : ℝ}
  ...
  have hs (k : ℤ) : WaveClass s P α (fun n x =>
    ...
    by_cases hk : j = k
    · apply WaveInteractionBounds.class_congr hhalf
      intro n x _
      simpa only [if_pos hk] using (heval n x).symm
    · apply WaveInteractionBounds.class_congr
        (MemClass.zero ha.weight_nonneg : WaveClass s P α (fun _ _ => (0 : ℂ)))
      intro n x _
      simpa only [if_neg hk] using (heval n x).symm
...
theorem zeroMean_gr_supportedGauge {a b : ℝ} {V : Set S} (hV : IsOpen V)
  ...
  have hD (i j : Fin 3) (v : PressureStream.Lift S) :
    ...
    have he : iteratedFDeriv ℝ 1 (u.covariance i j n) x = 0 := by
      by_contra hn'
      exact hr (hs hn')
    have he' := congrArg (fun A : (PressureStream.Lift S) [×1]→L[ℝ] ℝ =>
      A (fun _ : Fin 1 => v)) he
    rw [iteratedFDeriv_one_apply] at he'
    change fderiv ℝ (u.covariance i j n) x v = 0 at he'
    exact he'
  apply hn
  rw [zeroMean_gr c u hm]
...
end Navier.Construction.CorrectionInitialization
