...
Upstream source: NavierStokes/FlatKernelBounds.lean
...
namespace Navier.Construction.FlatKernelBounds
  ...
  | jet k =>
      have hk : HasDerivAt (iteratedDeriv k b)
        ...
        exact ((hb.differentiable_iteratedDeriv k
          (WithTop.coe_lt_coe.mpr (ENat.coe_lt_top k))) _).hasDerivAt
...
theorem bounded_profile_jet {b : ℝ → ℝ} (hb : ContDiff ℝ ∞ b) (R : ℝ) (k : ℕ) :
  ...
  have hcont := hb.continuous_iteratedDeriv k
    (le_of_lt (WithTop.coe_lt_coe.mpr (ENat.coe_lt_top k)))
...
theorem Expr.continuousOn_eval_t (e : Expr) {b : ℝ → ℝ}
  ...
  | jet k =>
      exact (hb.continuous_iteratedDeriv k
        (le_of_lt (WithTop.coe_lt_coe.mpr (ENat.coe_lt_top k)))).comp_continuousOn
...
end Navier.Construction.FlatKernelBounds
