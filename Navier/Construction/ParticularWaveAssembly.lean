...
Upstream source: NavierStokes/ParticularWaveAssembly.lean
...
import Navier.Construction.CopyAngularInvariance
...
namespace Navier.Construction.ParticularWaveAssembly
...
theorem pair_apply {D : Type} (j m : ℤ) (a : D → ℂ) (x : D) :
  ...
  · simp only [Finsupp.single_apply, if_pos hm, if_pos hn, if_pos hm.symm, if_pos hn.symm]
  · simp only [Finsupp.single_apply, if_pos hm, if_neg hn, if_pos hm.symm,
      if_neg (Ne.symm hn), Pi.zero_apply]
  · simp only [Finsupp.single_apply, if_neg hm, if_pos hn, if_neg (Ne.symm hm),
      if_pos hn.symm, Pi.zero_apply]
  · simp only [Finsupp.single_apply, if_neg hm, if_neg hn, if_neg (Ne.symm hm),
      if_neg (Ne.symm hn), Pi.zero_apply]
...
theorem signed_pairs_reconstruct {D : Type} (c : Coefficients D) (N : ℕ)
  ...
  classical
  ext m x
  change (∑ j ∈ modes N, conjugatePair j (c j)).coeff m x = c.coeff m x
  ...
  · simp only [if_pos hm, hc m x, map_div₀, map_ofNat, starRingEnd_self_apply]
    ring
  · have hz : c m = 0 := by
      by_cases hm0 : m = 0
      · simpa [hm0] using h0
    ...
    simp only [if_neg hm, hz, Pi.zero_apply, map_zero, add_zero]
...
theorem modeBlock_zero {D : Type} {j : ℤ} (hj : j ≠ 0)
  ...
  · intro n i
    funext x
    simp only [modeBlock, pair_apply, neg_zero, if_neg (Ne.symm hj), map_zero, add_zero, Pi.zero_apply]
  · intro n
    funext x
    simp only [modeBlock, pair_apply, neg_zero, if_neg (Ne.symm hj), map_zero, add_zero, Pi.zero_apply]
...
theorem pair_class {D : Type} [NormedAddCommGroup D] [NormedSpace ℝ D]
  ...
  · have h := hp.add hn
    simp only [pair_apply, if_pos hm, if_pos hm', div_eq_mul_inv, one_mul, mul_comm] at h ⊢
    exact h
  · simpa only [pair_apply, if_pos hm, if_neg hm', div_eq_mul_inv, one_mul, mul_comm,
      map_zero, add_zero] using hp
  · simp only [pair_apply, if_neg hm, if_pos hm', div_eq_mul_inv, one_mul, mul_comm, zero_add] at hn ⊢
    exact hn
  · simpa only [pair_apply, if_neg hm, if_neg hm', map_zero, add_zero] using hz
...
theorem good_classes (W : ℕ → (P × ℝ) × Plane → ℝ)
  ...
  · exact hh.1
  · intro j hj
    simp only [goodFamily, dif_pos hj]
    apply nativeSlice_waveClass
...
theorem gaussian_classes (β : ℝ) (i : Fin 3) (m : ℤ) :
  ...
  have hj (j : ℤ) (h : j ∈ modes N) : UnweightedClass (sectionStrip D.strip) β
    ...
    simp only [gaussianFamily, dif_pos h]
...
theorem cancellation_sum
  ...
  intro j hj i
  simp only [goodFamily, gaussianFamily, dif_pos hj]
...
end Navier.Construction.ParticularWaveAssembly
