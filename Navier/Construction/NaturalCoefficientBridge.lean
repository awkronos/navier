...
Upstream source: NavierStokes/NaturalCoefficientBridge.lean
...
namespace Navier.Construction.NaturalCoefficientBridge
...
theorem secondX_eq_slice {f : InnerProfile} {w : InnerPoint}
  ...
  have hfx : DifferentiableAt ℝ (partialX f) w :=
    (SimilarityProfile.partialX_smoothAt hf (m := 1)
      (by decide)).differentiableAt (by norm_num)
...
end Navier.Construction.NaturalCoefficientBridge
