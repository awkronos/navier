...
Upstream source: NavierStokes/NominalProfile.lean
...
-/
import Navier.Construction.OutgoingDilation
import Navier.Construction.OutgoingHistories
import Navier.Construction.NaturalEntrance
import Navier.Construction.ReferencePath
import Navier.Construction.ShapeTransition
import Navier.Construction.FiveProfileMoments
import Navier.Construction.TransitionRamp
import Navier.Construction.HeatedOutgoing
import Navier.Construction.ActivationContinuation
import Navier.Construction.ExtendedHeatedOutgoing
...
open Navier.Construction.OutgoingProfile

namespace Navier.Construction.NominalProfile
...
theorem splice_inner {r : ℝ} (inner outer : Field) {p : Point} (hp : p.1 ≤ r) :
    splice r inner outer p = inner p := if_pos hp

theorem splice_outer {r : ℝ} (inner outer : Field) {p : Point} (hp : r < p.1) :
    splice r inner outer p = outer p := if_neg (not_le.mpr hp)
...
theorem f_before_Xi {p : Point} (hp : p.1 ≤ Xi) : c.f p = c.seedF p := if_pos hp
...
theorem E_eq_sqrt_mul_f
  ...
  · rw [f, if_neg hp, mul_div_cancel₀ _
...
theorem heated_seed_fields (coef : ℝ → HeatedOutgoing.Coeff)
  ...
  refine ⟨?_, hc.1, ?_⟩
  · rw [heatedf, if_pos hp, c.f_before_Xi hp]
    rfl
...
theorem extendedE_eq_sqrt_f (coef : ℝ → ExtendedHeatedOutgoing.Coeff)
  ...
  by_cases hp : p.1 ≤ Xi
  · rw [extendedf, if_pos hp, c.extendedE_before coef (hp.trans (c.Xi_lt_heatJoin hsep).le)]
    exact c.E_eq_sqrt_mul_f hsep hX
  · rw [extendedf, if_neg hp, mul_div_cancel₀ _
      (Real.sqrt_pos.2 (show 0 < 2 * p.1 by positivity)).ne']
...
theorem extended_seed_fields (coef : ℝ → ExtendedHeatedOutgoing.Coeff)
  ...
  rw [extendedf, if_pos hp, c.f_before_Xi hp]
...
end Navier.Construction.NominalProfile
