...
Upstream source: NavierStokes/SquaredPartition.lean
...
namespace Navier.Construction.SquaredPartition
...
theorem dyadicProfile_support : support dyadicProfile = Ioo (1 / 2 : ℝ) 2 := by
  ...
  · change (if 0 < q then lineMask 0 (logCoordinate q) else 0) ≠ 0 ↔ _
    rw [if_pos hq, ← mem_support, lineMask_support]
    simpa using logCoordinate_window hq
  · simp only [mem_support, dyadicProfile, if_neg hq, ne_eq, not_true_eq_false, mem_Ioo,
      false_iff, not_and]
    intro hl
...
theorem dyadicMask_eq_line (n : ℤ) {q : ℝ} (hq : 0 < q) :
  ...
  rw [if_pos (div_pos hq (integerQ_pos n)), hcoord, ← lineMask_eq_translate]
...
end Navier.Construction.SquaredPartition
