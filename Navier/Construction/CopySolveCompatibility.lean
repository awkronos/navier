...
Upstream source: NavierStokes/CopySolveCompatibility.lean
...
namespace Navier.Construction.CopySolveCompatibility
...
theorem pathFamily_smul (F : P × ℝ → E) (p : P) (c : ℝ) :
  ...
  · subst c
    simp only [zero_smul]
    simp only [SmoothPathFamily.pathFamily, dif_pos continuous_const]
    rfl
  by_cases hF : Continuous (fun s : Icc a b => F (p, s))
  ...
  · have hCF : ¬ Continuous (fun s : Icc a b => c • F (p, s)) := by
    ...
    simp only [SmoothPathFamily.pathFamily, dif_neg hF, dif_neg hCF, smul_zero]
...
end Navier.Construction.CopySolveCompatibility
