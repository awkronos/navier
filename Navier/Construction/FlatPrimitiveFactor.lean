...
Upstream source: NavierStokes/FlatPrimitiveFactor.lean
...
-/
import Navier.Construction.FlatPrimitive
import Navier.Construction.FlatKernelBounds
...
open Navier.Construction.FlatCutoff Navier.Construction.FlatPrimitive

namespace Navier.Construction.FlatPrimitiveFactor
...
theorem kernel_ae_contDiff (c : ℝ) (j : ℕ) {b : ℝ → ℝ} (hb : ContDiff ℝ ∞ b) :
  ...
  exact Navier.Construction.FlatKernelBounds.kernel_contDiff_x hb c j ht.le
...
theorem kernel_iteratedDeriv_measurable (c : ℝ) (j : ℕ) {b : ℝ → ℝ}
  ...
  have hcont : ContinuousOn
    ...
    (Navier.Construction.FlatKernelBounds.kernel_iteratedDeriv_continuousOn_t hb c j n x).mono
      Ioi_subset_Ici_self
...
theorem kernel_locallyDominated {c : ℝ} (hc : 0 < c) (j : ℕ) {b : ℝ → ℝ}
    (hb : ContDiff ℝ ∞ b) :
    Navier.Construction.SmoothParameterIntegral.LocallyDominatedDeriv
      (kernel c j b) (volume.restrict (Ioi 0)) := by
  intro n x
  have hR : 0 ≤ |x| + 1 := by positivity
  obtain ⟨C, N, _hC, hbound⟩ :=
    Navier.Construction.FlatKernelBounds.kernel_iteratedDeriv_bound hb c j n hR
...
theorem factor_contDiff {c : ℝ} (hc : 0 < c) (j : ℕ) {b : ℝ → ℝ}
  ...
  Navier.Construction.SmoothParameterIntegral.contDiff_integral_of_iteratedDeriv
...
theorem factor_iteratedDeriv {c : ℝ} (hc : 0 < c) (j : ℕ) {b : ℝ → ℝ}
  ...
  Navier.Construction.SmoothParameterIntegral.iteratedDeriv_integral
    (kernel_ae_contDiff c j hb) (kernel_iteratedDeriv_measurable c j hb)
...
end Navier.Construction.FlatPrimitiveFactor
