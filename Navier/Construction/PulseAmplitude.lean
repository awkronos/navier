...
Upstream source: NavierStokes/PulseAmplitude.lean
...
-/
import Navier.Construction.OutgoingSchedule
import Navier.Construction.OutgoingTail
import Navier.Construction.TailEnergyBounds
import Navier.Construction.OutgoingPulseBounds
import Navier.Construction.RadialSchedule
...
open Navier.Construction.OutgoingSchedule

namespace Navier.Construction.PulseAmplitude
...
theorem weightedSquare_integral (a l u : ℝ) :
  ...
  exact (energyWeight_continuous.fun_mul
    ((continuous_id.sub continuous_const).pow 2)).intervalIntegrable l u
...
theorem pulseConstant_upper : pulseConstant ≤ 1 / 4 := by
  have hm := intervalIntegral.integral_mono_on (μ := volume) (by norm_num : (0 : ℝ) ≤ 13)
    ((energyWeight_continuous.fun_mul
      (mainPulse_contDiff.continuous.pow 2)).intervalIntegrable 0 13)
    ((energyWeight_continuous.fun_mul (continuous_id.pow 2)).intervalIntegrable 0 13)
...
theorem pulseConstant_lower : 1 / 5 < pulseConstant := by
  have hw : Continuous (fun z : ℝ => Real.exp (-2 * z) * mainPulse z ^ 2) :=
    energyWeight_continuous.fun_mul (mainPulse_contDiff.continuous.pow 2)
  ...
  have hlo := intervalIntegral.integral_mono_on (μ := volume) (by norm_num : (1 / 50 : ℝ) ≤ 5)
    ((energyWeight_continuous.fun_mul
      ((continuous_id.sub continuous_const).pow 2)).intervalIntegrable (1 / 50) 5)
...
end Navier.Construction.PulseAmplitude
