...
Upstream source: NavierStokes/FlatZeroExtension.lean
...
namespace Navier.Construction.FlatZeroExtension
...
theorem zeroExtension_of_pos (f : E × ℝ → F) {p : E × ℝ} (hp : 0 < p.2) :
    zeroExtension f p = f p := if_pos hp

theorem zeroExtension_of_nonpos (f : E × ℝ → F) {p : E × ℝ} (hp : p.2 ≤ 0) :
    zeroExtension f p = 0 := if_neg (not_lt_of_ge hp)
...
theorem hasFTaylorSeriesUpToOn_zeroExtension {c : ℝ} (hc : 0 < c)
  ...
  · intro p _
    by_cases hp : 0 < p.2
    · simp only [extendedJets, zeroExtension, if_pos hp]
      rfl
    · simp only [extendedJets, zeroExtension, if_neg hp]
      rfl
  · intro n _ p hp
...
end Navier.Construction.FlatZeroExtension
