...
Upstream source: NavierStokes/ModulatedCone.lean
...
-/
import Navier.Construction.ParametricModulation
import Navier.Construction.StressActivation
import Navier.Construction.UniformCone
import Navier.Construction.ModulatedStockBounds
...
namespace Navier.Construction.ModulatedCone
...
theorem fields_uniform_bound (W : ModulatedHistories.Window)
  ...
  · have hX' : X ∈ Icc W.left W.right := ⟨hX.1.le, hX.2⟩
    ...
    simpa only [ModulatedHistories.localizedF, ModulatedHistories.localizedU,
      ModulatedHistories.splice, if_pos hX, ModulatedHistories.rawF, ModulatedHistories.rawU] using
  ...
  · simp only [ModulatedHistories.localizedF, ModulatedHistories.localizedU,
      ModulatedHistories.splice, if_neg hX, sub_self, abs_zero, and_self]
...
theorem exists_with_moment_repair {D D' : RadialDomain}
  ...
  have hcoeff : ∀ n : ℝ, N₀ ≤ n → 1 ≤ n → JetBounds.FiniteJetBound 1 (c n) J (Cc / n) := by
    intro n hn _
    simpa only [c, dif_pos hn] using hjet n hn
  ...
  · intro η hη X hX
    have hmom : FiveProfileMoments.physicalMoments patch b (A η) (G η) (c N η) =
      ...
      simpa only [c, dif_pos hrn] using (hsolve N hrn η (hJV N hrn hη)).1
...
end Navier.Construction.ModulatedCone
