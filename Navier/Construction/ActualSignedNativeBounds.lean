...
Upstream source: NavierStokes/ActualSignedNativeBounds.lean
...
namespace Navier.Construction.ActualSignedNativeBounds
...
theorem selection_on_label {E : Type*} [Zero E]
  ...
  simp only [selection, dif_pos hL, true_and]
...
theorem selection_inactive {E : Type*} [Zero E]
  ...
  classical
  simp only [selection, dif_neg hL]
...
theorem selection_active_function {E : Type*} [Zero E]
  ...
  classical
  funext y
  simp only [selection, dif_pos hL, if_pos hI]
...
theorem potentialSource_other_label (L : PhysicalWaveSum.BandLabel)
  ...
  · simp only [DependentSignedPhysicalFamily.Family.valueAt, dif_pos hL]
    ...
    erw [ActualSignedPhysicalData.nativePotentialSource, dif_neg hm]
  · rw [DependentSignedPhysicalFamily.Family.valueAt_inactive _ _ hL]
    rfl
...
theorem pressureSource_other_label (L : PhysicalWaveSum.BandLabel)
  ...
  · simp only [DependentSignedPhysicalFamily.Family.valueAt, dif_pos hL]
    have hm : I.1.1 ∉ ((family P u H hp).singleton ⟨L.val, L.property, hL⟩).active := by
      simpa only [DependentSignedPhysicalFamily.Family.singleton_active, Set.mem_singleton_iff] using hI
    erw [ActualSignedPhysicalData.nativePressureSource, dif_neg hm]
  · rw [DependentSignedPhysicalFamily.Family.valueAt_inactive _ _ hL]
    rfl
...
end Navier.Construction.ActualSignedNativeBounds
