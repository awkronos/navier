...
Upstream source: NavierStokes/SmoothParameterIntegral.lean
...
namespace Navier.Construction.SmoothParameterIntegral
  ...
  have hd : ∀ᵐ t ∂μ, ∀ y ∈ ball x ε,
    ...
    simpa only [jet, fderiv_iteratedFDeriv, Function.comp_apply] using
      (ht.differentiable_iteratedFDeriv (m := k)
        (WithTop.coe_lt_coe.mpr (ENat.coe_lt_top k)) y).hasFDerivAt
...
end Navier.Construction.SmoothParameterIntegral
