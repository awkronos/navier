...
Upstream source: NavierStokes/ActualSignedNativeProfiles.lean
...
namespace Navier.Construction.ActualSignedNativeProfiles
...
theorem region_active (L : NativeLabel B N0) :
  ...
  classical
  simp only [region, dif_pos L.mem]
...
theorem profile_jets : PolynomialJets (profileDomain B N0)
  ...
  · intro I
    ...
    · let L : NativeLabel B N0 := ⟨I.2.val,I.2.property,hI⟩
      ...
      simpa only [profileDomain, PhysicalCopyBounds.copyBandDomain, region, dif_pos hI,
    ...
    · intro p hp
      have hempty : p ∈ (∅ : Set Slow) := by
        simpa only [profileDomain, PhysicalCopyBounds.copyBandDomain, region, dif_neg hI] using hp
      exact hempty.elim
  · intro N
    ...
    · simp only [profileDomain, PhysicalCopyBounds.copyBandDomain, region, dif_neg hI] at hp
      exact hp.elim
...
theorem singleton_profiles (L : NativeLabel B N0) (K : BandLabel) (k : Frequency) (p : Slow) :
  ...
  · subst K
    ...
    simp only [ite_true, ActualSignedPhysicalData.extendedCarrier, dif_pos hm]
    have hb := ActualSignedExterior.actualLabel_reference L
  ...
  · have hm : K ∉ ((ActualSignedExterior.family s).singleton L).active := by
    ...
    simp only [if_neg he, ActualSignedPhysicalData.extendedCarrier, dif_neg hm,
...
theorem singleton_jets (L : NativeLabel B N0) :
  ...
  · intro I
    ...
    · simpa only [if_neg hI] using (contDiffOn_const :
  ...
  · intro N
    ...
    intro I j hj p hp
    by_cases hI : I.2 = (L : BandLabel)
    · simpa only [if_pos hI] using hm I j hj p hp
    · simp only [if_neg hI, iteratedFDeriv_fun_zero, Pi.zero_apply, norm_zero]
...
theorem potential_profiles_jets (i : Fin 3) :
  ...
  · have hactive : I.2 ∈ (ActualSignedExterior.family s).active := hI
    ...
    simp only [DependentSignedPhysicalFamily.Family.potentialCopies,
      ...
      DependentSignedPhysicalFamily.Family.copyAt, dif_pos hactive,
...
end Navier.Construction.ActualSignedNativeProfiles
