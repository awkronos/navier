...
Upstream source: NavierStokes/HeatTailEdit.lean
...
-/
import Navier.Construction.RadialHeatProfile
import Navier.Construction.OutgoingTail
import Navier.Construction.SmoothCutoffs
...
namespace Navier.Construction.HeatTailEdit
...
theorem powerTail_weighted_change {h ν K e M q : ℝ} (hh : 0 < h) (hν : 0 < ν)
  ...
  have hg := ((edit_contDiffOn hh hν hK hE).continuousOn.sub hE.continuousOn).mono
...
theorem powerTail_weighted_squareChange {h ν K e M q : ℝ} (hh : 0 < h) (hν : 0 < ν)
  ...
  have hE := powerTail_contDiffOn hK h e hf
  have hg := (((edit_contDiffOn hh hν hK hE).continuousOn.pow 2).sub
    (hE.continuousOn.pow 2)).mono (Ioi_subset_Ioi hK.le)
...
end Navier.Construction.HeatTailEdit
