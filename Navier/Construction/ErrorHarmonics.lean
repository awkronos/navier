...
Upstream source: NavierStokes/ErrorHarmonics.lean
...
import Navier.Construction.AxisymmetricResidual
...
namespace Navier.Construction.ErrorHarmonics
...
theorem zeroBlock_symmetric {D : Type} (k : ℕ → ℝ) (Φ : ℕ → D → ℝ) (kp : ℕ → ℤ)
  ...
  intro j x
  classical
  change (constantCoefficient (fun x => (a n x i : ℂ))).coeff (-j) x =
    conj ((constantCoefficient (fun x => (a n x i : ℂ))).coeff j x)
  have hcoeff (l : ℤ) :
  ...
  · subst j
    ...
    simp
  · rw [hcoeff, hcoeff]
    have hj' : (0 : ℤ) ≠ j := Ne.symm hj
    have hneg : (0 : ℤ) ≠ -j := by
      intro h
      apply hj
      omega
    rw [if_neg hneg, if_neg hj']
    simp
...
end Navier.Construction.ErrorHarmonics
