...
Upstream source: NavierStokes/HarmonicMeanInteraction.lean
...
namespace Navier.Construction.HarmonicMeanInteraction
...
theorem constant_mul_constant (f g : D → ℂ) :
  ...
  ext j x
  change (constantCoefficient f * constantCoefficient g).coeff j x =
    (constantCoefficient (fun x => f x * g x)).coeff j x
  rw [constant_mul]
  by_cases hj : j = 0
  · rw [constantCoefficient_coeff, constantCoefficient_coeff]
    simp [hj]
  · rw [constantCoefficient_coeff, constantCoefficient_coeff]
    simp [hj]
...
theorem constant_add (f g : D → ℂ) :
    constantCoefficient f + constantCoefficient g = constantCoefficient (fun x => f x + g x) := by
  change AddMonoidAlgebra.single 0 f + AddMonoidAlgebra.single 0 g =
    AddMonoidAlgebra.single 0 (f + g)
  exact (AddMonoidAlgebra.single_add 0 f g).symm
...
theorem differentiate_constant (V : D → D) (k : ℝ) (Φ : D → ℝ) (f : D → ℂ) :
  ...
  ext j x
  change (differentiate V k Φ (constantCoefficient f)).coeff j x =
    (constantCoefficient (along V f)).coeff j x
  by_cases hj : j = 0
  · subst j
    rw [differentiate_apply, constantCoefficient_coeff, constantCoefficient_coeff]
    simp [derivativeCoefficient, phaseFactor]
  · have hz : (constantCoefficient f : Coefficients D) j = 0 := by
      rw [constantCoefficient_coeff, if_neg hj]
    rw [differentiate_apply, hz, derivativeCoefficient_zero, constantCoefficient_coeff,
      if_neg hj]
...
theorem angularDifferentiate_constant (kp : ℤ) (f : D → ℂ) :
  ...
  ext j x
  change (angularDifferentiate kp (constantCoefficient f)).coeff j x = (0 : Coefficients D).coeff j x
  by_cases hj : j = 0
  · subst j
    rw [angularDifferentiate_apply, constantCoefficient_coeff]
    simp
  · rw [angularDifferentiate_apply, constantCoefficient_coeff, if_neg hj]
    simp
...
theorem rotate_constant (m : D → ComplexVector) (i : Fin 3) :
  ...
  ext j x
  change (HarmonicResidual.rotate (HarmonicResidual.constantVector m) i).coeff j x =
    (constantCoefficient (fun x => angularGenerator (m x) i)).coeff j x
  fin_cases i <;> by_cases hj : j = 0 <;>
    simp [HarmonicResidual.rotate, HarmonicResidual.constantVector, angularGenerator,
      constantCoefficient_coeff, hj]
...
theorem realCoefficients_sub (a b : Coefficients D) :
  ...
  ext j x
  change (HarmonicResidual.realCoefficients (a - b)).coeff j x =
    (HarmonicResidual.realCoefficients a - HarmonicResidual.realCoefficients b).coeff j x
...
theorem nonconstant_sub (a b : Coefficients D) :
  ...
  ext j x
  change (HarmonicResidual.nonconstant (a - b)).coeff j x =
    (HarmonicResidual.nonconstant a - HarmonicResidual.nonconstant b).coeff j x
  by_cases hj : j = 0
  · subst j
...
end Navier.Construction.HarmonicMeanInteraction
