...
Upstream source: NavierStokes/ActualSignedCommonDynamics.lean
...
import Navier.Construction.ClosedNativeWaveIdentities
...
namespace Navier.Construction.ActualSignedCommonDynamics
...
theorem linearGood_bounds {β : ℝ} {request : ℕ → FullPoint → SignedWaveUpdate.Vec2}
  ...
  · intro i j hj
    have hz (x : Point) :
        (HarmonicFields.constantCoefficient (fun _ : Point => (0 : ℂ))).coeff j x = 0 := by
      change (Finsupp.single (0 : ℤ) (0 : Point → ℂ)) j x = 0
      simp
...
end Navier.Construction.ActualSignedCommonDynamics
