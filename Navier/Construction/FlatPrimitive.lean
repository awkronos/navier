...
Upstream source: NavierStokes/FlatPrimitive.lean
...
-/
import Navier.Construction.FlatCutoff
...
open Navier.Construction.FlatCutoff

namespace Navier.Construction.FlatPrimitive
...
theorem iteratedDeriv_zero_on_nonpos {f : ℝ → ℝ} (hf : ContDiff ℝ ∞ f)
  ...
  | succ m ih =>
    ...
    apply (uniqueDiffOn_Iic x x (mem_Iic.mpr le_rfl)).eq_deriv (Iic x)
      ((hf.differentiable_iteratedDeriv m (by exact_mod_cast ENat.coe_lt_top m) x).hasDerivAt.hasDerivWithinAt)
...
theorem normalizedPrimitive_factorization (c : ℝ) (j : ℕ) (b : ℝ → ℝ)
  ...
  · have hs : scale c j x ≠ 0 := (scale_pos c j hpos).ne'
    simp only [normalizedPrimitive, if_neg hpos.ne']
    field_simp
...
end Navier.Construction.FlatPrimitive
