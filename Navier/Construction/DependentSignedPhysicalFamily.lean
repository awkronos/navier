...
Upstream source: NavierStokes/DependentSignedPhysicalFamily.lean
...
namespace Navier.Construction.DependentSignedPhysicalFamily
...
@[simp] theorem copyAt_active (copies : NativeLabel f.active → CopyFamily H K)
  ...
  classical
  simp only [copyAt, dif_pos L.mem]
...
theorem potential_amplitude_eq_source (i : Fin 3) (k : TorusInverse.Frequency)
  ...
  · simp only [copyAt, potentialSource, valueAt, dif_pos hL]
...
theorem pressure_amplitude_eq_source (k : TorusInverse.Frequency)
  ...
  by_cases hL : I.1 ∈ f.active
  · simp only [copyAt, pressureSource, valueAt, dif_pos hL]
...
end Navier.Construction.DependentSignedPhysicalFamily
