...
Upstream source: NavierStokes/GlobalSlowProfiles.lean
...
-/
import Navier.Construction.SlowRecursion
import Navier.Construction.PositiveOrderMoments
import Navier.Construction.TransportPrimitive
import Navier.Construction.SlowStressSupport
import Navier.Construction.SlowResidualMatching
...
namespace Navier.Construction.GlobalSlowProfiles
...
theorem exists_step {S : Set ℝ} {h C : ℝ} (s : Scheme S h C) {n : ℕ} (hn : 0 < n)
  ...
  have ho : Exterior s.B S o := by
    ...
    intro j hj
    simpa only [previousBeta, dif_pos hj] using (previous j hj).beta_exterior
  have hu0 : u 0 = s.base.axial := by
    dsimp [u, previousAxial]
    rw [dif_pos hn, (previous 0 hn).zero_data rfl]
  ...
  let out : Admissible s n := {
    ...
    inner_axial := by
      ...
      simpa only [u, previousAxial, dif_neg (lt_irrefl n)] using hi
    inner_phi := by
      intro _ eta R hR
      have hi := hpi eta R hR
      simpa only [p, previousPhi, dif_neg (lt_irrefl n)] using hi
    outer_axial := by
      intro _ eta R hR
      have ho := huo eta R hR
      simpa only [u, previousAxial, dif_neg (lt_irrefl n)] using ho
    outer_phi := by
      intro _ eta R hR
      have ho := hpo eta R hR
      simpa only [p, previousPhi, dif_neg (lt_irrefl n)] using ho }
...
theorem previousSource_sequence {S : Set ℝ} {h C : ℝ} (s : Scheme S h C) (n : ℕ) :
  ...
  · intro j hj
    simp only [previousAxial, dif_pos hj, profiles]
  · intro j hj
    simp only [previousBeta, dif_pos hj, profiles]
...
theorem updated_sequence_axial {S : Set ℝ} {h C : ℝ} (s : Scheme S h C) (n : ℕ)
  ...
  · rw [Function.update_of_ne he]
    ...
    simp only [previousAxial, dif_pos hjn, profiles]
...
theorem updated_sequence_phi {S : Set ℝ} {h C : ℝ} (s : Scheme S h C) (n : ℕ)
  ...
  · rw [Function.update_of_ne he]
    have hjn : j < n := lt_of_le_of_ne hj he
    simp only [previousPhi, dif_pos hjn, profiles]
...
end Navier.Construction.GlobalSlowProfiles
