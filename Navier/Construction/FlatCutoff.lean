...
Upstream source: NavierStokes/FlatCutoff.lean
...
namespace Navier.Construction.FlatCutoff
...
theorem edge_le_glue {c x : ℝ} (hxc : x ≤ c) :
  ...
  · have hx' : 0 < x := lt_of_not_ge hx
    rw [edge_of_pos c hx']
    simp only [expNegInvGlue, if_neg hx]
...
theorem edge_div_edge (c d : ℝ) :
  ...
  by_cases hx : x ≤ 0
  · simp [edge, hx]
  · simp only [edge, if_neg hx, ← Real.exp_sub]
    congr 1
    ring
...
end Navier.Construction.FlatCutoff
