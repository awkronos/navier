...
Upstream source: NavierStokes/TransitionRamp.lean
...
-/
import Navier.Construction.ActivationStocks
...
namespace Navier.Construction.TransitionRamp
...
theorem logField_positive_jet_error_bound {J : Set ℝ} (hJ : IsOpen J) {stock : Field}
  ...
  intro t ht
  rw [parameterJet_angularSlope hJ hs T κ b w₁ w₂ n hη, if_neg hn, sub_zero]
  apply controlled_product_bound _ (damping_mem T hκ t).1 hM (hb t ht)
...
theorem physicalF_before (T κ w₁ w₂ : ℝ) {p : Point} (hp : p.1 ≤ R.radius0) :
    R.physicalF T κ w₁ w₂ p = R.profiles.f p := if_pos hp

theorem physicalU_before (T κ w₁ : ℝ) {p : Point} (hp : p.1 ≤ R.radius0) :
    R.physicalU T κ w₁ p = R.profiles.U p := if_pos hp
...
theorem physicalU_held (hJ : IsOpen J) {T κ w₁ X η : ℝ}
  ...
  rw [physicalU, if_neg (not_le.mpr (hR.trans_le hX))]
...
theorem physicalF_held_log (hJ : IsOpen J) {T κ w₁ w₂ X η : ℝ}
  ...
  rw [physicalF, if_neg (not_le.mpr (hR.trans_le hX))]
...
end Navier.Construction.TransitionRamp
