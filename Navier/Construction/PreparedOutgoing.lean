...
Upstream source: NavierStokes/PreparedOutgoing.lean
...
-/
import Navier.Construction.ScheduledProfileChoice
...
namespace Navier.Construction.PreparedOutgoing
...
theorem exists_prepared : Nonempty PreparedProfile := by
  ...
  have hFlam : F.data.core.lam < Classical.choose (caps K hK) := by
    rw [hcore]
    simpa only [cap, dif_pos hK] using hlam
...
end Navier.Construction.PreparedOutgoing
