...
Upstream source: NavierStokes/CutStageEstimates.lean
...
namespace Navier.Construction.CutStageEstimates
...
theorem positiveStages_of_pos {A : ℕ → E → V} {j : ℕ} (hj : 1 ≤ j) :
  ...
  funext x
  simp only [positiveStages, if_neg (by omega : j ≠ 0)]
...
end Navier.Construction.CutStageEstimates
