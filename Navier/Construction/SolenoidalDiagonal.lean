...
Upstream source: NavierStokes/SolenoidalDiagonal.lean
...
import Navier.Construction.DiagonalScale
...
namespace Navier.Construction.SolenoidalDiagonal
...
theorem locallyFinite_cutStage_support_on {a : ℕ → ℝ} (ha : Tendsto a atTop atTop)
  ...
  exact locallyFinite_cutStage_support ha (continuousOn_iff_continuous_restrict.mp hq)
    (fun x => hqpos x x.property)
    (fun j x => A j x)
...
end Navier.Construction.SolenoidalDiagonal
