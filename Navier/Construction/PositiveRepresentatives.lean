...
Upstream source: NavierStokes/PositiveRepresentatives.lean
...
namespace Navier.Construction.PositiveRepresentatives
...
theorem positiveTime_convex : Convex ℝ positiveTime := by
  ...
  · have hb' : b = 1 := by linarith
    simpa only [zero_mul, zero_add, hb', one_mul, positiveTime, Set.mem_setOf_eq] using hy
...
theorem stableInverse_spec {a : ℝ} {p : ℝ × ℝ} (hp : p ∈ stableTarget a) :
  ...
  simp only [stableInverse, dif_pos hp]
...
end Navier.Construction.PositiveRepresentatives
