...
2. `hmM`, `hmXm1`, `hmX1`, `hmXm1Time`, `hmX1Time`, `hmX1Int` — the
   ...
   completed slots.  The only existing supplier
...
/-- The mild image of a box element, lifted to the completed linked carrier by
the leaf measurability fields and the self-map budget.  Assumes no fixed
point. -/
def mildLift (ν : ℝ≥0) (hν : 0 < ν) (T R : ℝ) (hT : 0 ≤ T)
  ...
  actualLinkedOfRaw ν T (2 * R)
    ...
    (L.hmM x) (L.hmXm1 x) (L.hmX1 x) (L.hmXm1Time x) (L.hmX1Time x)
    (selfMapEstimate ν hν T R hT a haM ha ha1 haR L x).1 (L.hmX1Int x)
...
theorem mildLift_mem (ν : ℝ≥0) (hν : 0 < ν) (T R : ℝ) (hT : 0 ≤ T)
  ...
  set u := mildImage ν hν a (everywhereRawRepresentative ν T x.1) with hu_def
  refine ⟨?_, ?_⟩
  · have key : (mildLift ν hν T R hT a haM ha ha1 haR L x).1.fst =
      toXm1TimeSlot u (L.hmM x) (L.hmXm1 x) T (2 * R) (L.hmXm1Time x) hb.1 := rfl
    rw [key]
    have hpoint : ∀ᵐ t ∂leiLinTimeMeasure T,
        ‖(toXm1TimeSlot u (L.hmM x) (L.hmXm1 x) T (2 * R) (L.hmXm1Time x) hb.1 :
          ℝ → Xm1Spatial) t‖ ≤ 2 * R := by
      filter_upwards [coeFn_toXm1TimeSlot u (L.hmM x) (L.hmXm1 x) T (2 * R)
          (L.hmXm1Time x) hb.1,
  ...
  · have key : (mildLift ν hν T R hT a haM ha ha1 haR L x).1.snd =
      toViscousX1TimeSlot u (L.hmM x) (L.hmX1 x) ν T (L.hmX1Time x)
        (L.hmX1Int x) := rfl
    rw [key, L1.norm_eq_integral_norm]
    have hrew : (∫ t, ‖(toViscousX1TimeSlot u (L.hmM x) (L.hmX1 x) ν T
          (L.hmX1Time x) (L.hmX1Int x) : ℝ → ViscousX1Spatial ν) t‖
          ∂leiLinTimeMeasure T) =
        ∫ t, (ν : ℝ) * coordinateX1Mass (u t) ∂leiLinTimeMeasure T := by
      apply integral_congr_ae
      filter_upwards [coeFn_toViscousX1TimeSlot u (L.hmM x) (L.hmX1 x) ν T
...
theorem mildLift_contraction (ν : ℝ≥0) (hν : 0 < ν) (T R : ℝ) (hT : 0 ≤ T)
  ...
  have hb := selfMapEstimate ν hν T R hT a haM ha ha1 haR L x
  have hb' := selfMapEstimate ν hν T R hT a haM ha ha1 haR L y
  ...
  exact (actualLinkedOfRaw_coordinate_sum_le ν T (2 * R) (2 * R)
      ...
      (mildImage ν hν a (everywhereRawRepresentative ν T x.1))
      (mildImage ν hν a (everywhereRawRepresentative ν T y.1))
      (L.hmM x) (L.hmM y) (L.hmXm1 x) (L.hmXm1 y) (L.hmX1 x) (L.hmX1 y)
      (L.hmXm1Time x) (L.hmXm1Time y) (L.hmX1Time x) (L.hmX1Time y)
      hb.1 hb'.1 (L.hmX1Int x) (L.hmX1Int y) hXm hX1).trans
    (le_of_eq (by ring))
