...
theorem galileanMeanRemovedPath_fixedPoint_on_horizon
  ...
  let Ry : ℝ := Ru + ‖rawMeanCarrier c‖
  have hRy : 0 ≤ Ry := add_nonneg hRu (norm_nonneg _)
  have hyR : ∀ s ∈ Ioc (0 : ℝ) T, ‖y s‖ ≤ Ry := by
    intro s hs
    change ‖galileanMeanRemovedPath c u s‖ ≤
      Ru + ‖rawMeanCarrier c‖
    exact (norm_galileanMeanRemovedPath_le c u s).trans
      (add_le_add (huR s hs) le_rfl)
  apply Subtype.ext
  funext k
  by_cases hk : k = 0
  · subst k
...
#print Navier.Analysis.CriticalMildGalileanVolterra.galileanMeanRemovedPath_fixedPoint_on_horizon
