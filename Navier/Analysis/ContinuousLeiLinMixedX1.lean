...
theorem integral_normX1_continuousDuhamel_le_source_ae
...
/-- The original all-time section form follows from the stronger
almost-everywhere mixed maximal-regularity estimate. -/
theorem integral_normX1_continuousDuhamel_le_source
  ...
  have hDξae : ∀ᵐ t ∂(volume.restrict (Icc (0 : ℝ) T)),
      Integrable (fun ξ : ES =>
        ‖ξ‖ * ‖continuousDuhamel ν a b t ξ i‖) := by
    filter_upwards [ae_restrict_mem hmT] with t ht
    exact hDξ t ht
  exact integral_normX1_continuousDuhamel_le_source_ae
    a b ν T i hν hD0 hDξae hW1 hW hb0 hg hJ
...
-/
theorem integral_coordinateX1Mass_continuousDuhamel_le_integral_X0_product_ae
  ...
  · intro i _
    exact integral_normX1_continuousDuhamel_le_source_ae a b ν T i hν (hD0 i) (hDξ i)
...
/-- The original all-time mixed `X¹` API is a corollary of the
almost-everywhere section theorem. -/
theorem integral_coordinateX1Mass_continuousDuhamel_le_integral_X0_product
  ...
  have hDξae : ∀ i : Fin 3, ∀ᵐ t ∂(volume.restrict (Icc (0 : ℝ) T)),
      Integrable (fun ξ : ES =>
        ‖ξ‖ * ‖continuousDuhamel ν a b t ξ i‖) := fun i => by
    filter_upwards [ae_restrict_mem hmT] with t ht
    exact hDξ i t ht
  exact integral_coordinateX1Mass_continuousDuhamel_le_integral_X0_product_ae
    a b ν T hν hD0 hDξae hW1 hW hb0 hg hJ hs1 hi ha hbs ha0 hb00 hprod

end Navier.Analysis.ContinuousLeiLinMixedX1

#print axioms Navier.Analysis.ContinuousLeiLinMixedX1.integral_normX1_continuousDuhamel_le_source_ae
