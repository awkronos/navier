...
open Navier.Analysis.PhysicalPeriodicCoerciveShellControl
...
theorem triadMode_frequency_norm_sq (i : Fin 3) :
    ‖officialEuclideanPoint (latticeFrequency (triadMode i))‖ ^ 2 =
      ![(9 : ℝ), 16, 25] i := by
  fin_cases i <;>
    simp [triadMode, triadP, triadQ, triadL, latticeFrequency, officialEuclideanPoint,
...
/-- Wavevector closure makes the product of translation phases exactly one. -/
theorem triadTranslation_phase_product (x : Space) :
    latticeCharacter (triadMode 0) x * latticeCharacter (triadMode 1) x *
      latticeCharacter (triadMode 2) x = 1 := by
  rw [← latticeCharacter_add, ← latticeCharacter_add, triadMode_sum_zero]
  simp [latticeCharacter_eq_exp, latticeFrequency, Fin.sum_univ_three]

/-- Physical translations furnish determinant-one diagonal unitary matrices.
This supplies a genuine restricted SU(3) action, not full SU(3) mode symmetry. -/
theorem triadTranslation_mem_specialUnitary (x : Space) :
    triadTranslation x ∈ Matrix.specialUnitaryGroup (Fin 3) ℂ := by
  rw [Matrix.mem_specialUnitaryGroup_iff]
  constructor
  · rw [Matrix.mem_unitaryGroup_iff]
    change Matrix.diagonal _ * star (Matrix.diagonal _) = 1
    rw [Matrix.star_eq_conjTranspose, Matrix.diagonal_conjTranspose,
      Matrix.diagonal_mul_diagonal]
    have hphase (i : Fin 3) :
        latticeCharacter (triadMode i) x * star (latticeCharacter (triadMode i) x) = 1 := by
      rw [star_latticeCharacter, ← latticeCharacter_add, add_neg_cancel]
      simp [latticeCharacter_eq_exp, latticeFrequency, Fin.sum_univ_three]
    simpa only [Pi.star_apply, hphase] using
      (Matrix.diagonal_one : Matrix.diagonal (fun _ : Fin 3 => (1 : ℂ)) = 1)
  · simpa [triadTranslation, Matrix.det_diagonal, Fin.prod_univ_three] using
      triadTranslation_phase_product x
