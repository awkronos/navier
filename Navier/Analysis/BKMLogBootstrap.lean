...
The file retains the proved log-Grönwall criterion, Fourier and kernel bounds,
and conditional transfer lemmas.  It makes no claim that an arbitrary
Schwartz-sliced Navier–Stokes solution supplies the missing weighted-energy
propagation or integer-order energy estimate.  The former admitted producers
and every theorem depending on them have been removed.
-/
