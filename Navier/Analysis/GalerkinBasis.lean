...
* `galerkinCoefficientFlow_timeEquicontinuous` itself is **closed** and lives in
  `Navier.Analysis.GalerkinModeData`; its consumers are
  `GalerkinEnergyBudget.timeEquicontinuous_modalApprox_of_flow` and, through
  it, the assembly `GalerkinEnergyBudget.galerkinModeData_of_modalFlow`.  It
  cannot live here: discharging the convective
  hypothesis needs
