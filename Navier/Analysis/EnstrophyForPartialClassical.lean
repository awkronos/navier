...
theorem vel_contDiff (sol : PartialClassicalSolution ν zeroForce u₀ T)
  ...
  have hCA : ContDiffAt ℝ ∞ (fun z : ℝ × Space => sol.velocity z.1 z.2) (t, x) :=
    contDiffAt_velocity_spacetime sol ht0 htT x
  have hembed : ContDiffAt ℝ ∞ (fun (y : Space) => (t, y)) x :=
    (contDiffAt_const (c := t)).prod contDiffAt_id
  have hslice : ContDiffAt ℝ ∞ (sol.velocity t) x :=
    hCA.comp x hembed
  exact hslice
