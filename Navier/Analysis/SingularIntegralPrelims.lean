...
* `bkm_shape_absorption` — the three-term version with `√M₂`, the exact algebraic
  step from the retired logBKMControl_of_schwartzSliced extracted as a named lemma.
...
the retired logBKMControl_of_schwartzSliced, formerly in `BKMLogBootstrap.lean`) that
