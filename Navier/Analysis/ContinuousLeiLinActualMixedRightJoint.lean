...
/-!
# Reverse-ordered joint mixed source on the actual carrier

This is the noncommuted source order `(z,x-y)`.  Its product-measure proof is
separate because the vector Navier source is ordered, even though the scalar
`X⁰` majorant is symmetric.
-/
...
namespace Navier.Analysis.ContinuousLeiLinActualMixedRightJoint

/-- The reverse ordered mixed source `(z,x-y)` is integrable on frequency
times a causal interval from its a.e. joint Euclidean realization. -/
theorem integrable_weightedContinuousNavierSource_right_sub_of_actualBox
  ...
  let F : ES × ℝ → ℝ := fun p => ‖p.1‖⁻¹ *
    complexEuclideanNorm (continuousNavierSource u w p.2 p.1)
  have hFmeas : AEStronglyMeasurable F (volume.prod μ) := by
    have hwgt : AEStronglyMeasurable
        (fun p : ES × ℝ => ‖p.1‖⁻¹ : ES × ℝ → ℝ) (volume.prod μ) := by fun_prop
    have hn : AEStronglyMeasurable (fun p : ES × ℝ =>
        complexEuclideanNorm (continuousNavierSource u w p.2 p.1))
        (volume.prod μ) := by
      simpa [u, w, μ, complexEuclideanNorm] using hjoint.norm
    exact hwgt.mul hn
  ...
  · have hsec : AEStronglyMeasurable (fun s : ℝ => ∫ ξ : ES, ‖F (ξ, s)‖) μ :=
    ...
    have hdom : Integrable (fun s : ℝ =>
        coordinateX0Mass (u s) * coordinateX0Mass (w s)) μ := by
      ...
      have hrestricted := hfull.mono_measure (Measure.restrict_mono hsub le_rfl)
    ...
    simpa [F] using normXm1_continuousNavierSource_le_coordinateX0Mass
      u w s (by simpa [u] using huM)
      (fun r i => by simpa [w, commonRepresentativeDifference, Pi.sub_apply] using hwM r i)
      (by simpa [u] using hu0) hw0

/-- Every weighted coordinate of the reverse ordered source is integrable on
the product domain. -/
theorem integrable_weightedContinuousNavierSource_right_sub_coord_of_actualBox
    (ν : ℝ≥0) (hν : 0 < ν) (T R t : ℝ)
    (ht : t ∈ Icc (0 : ℝ) T) (x y : ActualLinkedCarrier ν T)
    (z : ActualLinkedBox ν T (2 * R) (2 * R))
    (hjoint : AEStronglyMeasurable (fun p : ES × ℝ =>
    ...
    Integrable (fun p : ES × ℝ => ‖p.1‖⁻¹ *
      ...
      (volume.prod (volume.restrict (Icc (0 : ℝ) t))) := by
  have hvec := integrable_weightedContinuousNavierSource_right_sub_of_actualBox
    ν hν T R t ht x y z hjoint
  have hcoord := continuousNavierSource_coord_aestronglyMeasurable
    (everywhereRawRepresentative ν T z.1)
    (commonRepresentativeDifference ν T x y) 0 t hjoint i
  apply hvec.mono'
  ...
  change ‖‖p.1‖⁻¹ * ‖continuousNavierSource
    (everywhereRawRepresentative ν T z.1)
    (commonRepresentativeDifference ν T x y) p.2 p.1 i‖‖ ≤ _
...
/-- Forward heat multiplication preserves every reverse ordered weighted
coordinate on the causal product domain. -/
theorem integrable_weightedHeatContinuousNavierSource_right_sub_coord_of_actualBox
    (ν : ℝ≥0) (hν : 0 < ν) (T R t : ℝ)
    (ht : t ∈ Icc (0 : ℝ) T) (x y : ActualLinkedCarrier ν T)
    (z : ActualLinkedBox ν T (2 * R) (2 * R))
    (hjoint : AEStronglyMeasurable (fun p : ES × ℝ =>
    ...
    Integrable (fun p : ES × ℝ => (‖p.1‖⁻¹ : ℝ) •
      heatMode (ν : ℝ) (t - p.2) (fun ζ : ES => continuousNavierSource
        (everywhereRawRepresentative ν T z.1)
        (commonRepresentativeDifference ν T x y) p.2 ζ i) p.1)
      (volume.prod (volume.restrict (Icc (0 : ℝ) t))) := by
  let u := everywhereRawRepresentative ν T z.1
  let w := commonRepresentativeDifference ν T x y
  let μ := volume.restrict (Icc (0 : ℝ) t)
  have hsrc := integrable_weightedContinuousNavierSource_right_sub_coord_of_actualBox
    ν hν T R t ht x y z hjoint i
  have hcoord := continuousNavierSource_coord_aestronglyMeasurable
    u w 0 t (by simpa [u, w, μ] using hjoint) i
  have hLmeas : AEStronglyMeasurable (fun p : ES × ℝ => (‖p.1‖⁻¹ : ℝ) •
      heatMode (ν : ℝ) (t - p.2)
        (fun ζ : ES => continuousNavierSource u w p.2 ζ i) p.1)
      (volume.prod μ) := by
    unfold heatMode
  ...
  exact norm_weightedHeatMode_le (ν : ℝ) t p.2 (by exact_mod_cast hν) hp.2
    p.1 (fun ζ : ES => continuousNavierSource u w p.2 ζ i)

end Navier.Analysis.ContinuousLeiLinActualMixedRightJoint

set_option pp.fullNames true in
#print axioms Navier.Analysis.ContinuousLeiLinActualMixedRightJoint.integrable_weightedContinuousNavierSource_right_sub_of_actualBox
set_option pp.fullNames true in
#print axioms Navier.Analysis.ContinuousLeiLinActualMixedRightJoint.integrable_weightedContinuousNavierSource_right_sub_coord_of_actualBox
set_option pp.fullNames true in
#print axioms Navier.Analysis.ContinuousLeiLinActualMixedRightJoint.integrable_weightedHeatContinuousNavierSource_right_sub_coord_of_actualBox
