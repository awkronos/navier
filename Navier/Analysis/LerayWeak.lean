...
structure GalerkinApproximation (ν : ℝ) (u₀ : SchwartzVelocity) where
  ...
  bound_le : bound ≤ kineticEnergy u₀ 0
...
theorem exists_subseq_windowCauchy
  ...
  -- Diagonal extraction over cell-average Cauchy at each dyadic scale.
  -- We use `exists_diagonal_subseq` directly (not `exists_subseq_forall_windowCauchy`,
  -- which has a type mismatch with the fixed-n outer binder).
  let Q (l : ℕ) (τ' : ℕ → ℕ) : Prop :=
    let h := (l+1 : ℝ)⁻¹
    let S : Finset (ℤ × (Fin 3 → ℤ)) :=
      let hfinite := finite_prodGridIndices (by positivity : 0 < (l+1 : ℝ)⁻¹) (n : ℝ)
  ...
  have hQsub : ∀ (l : ℕ) (τ' ρ' : ℕ → ℕ), Q l τ' → StrictMono ρ' → Q l (τ' ∘ ρ') := by
    ...
    intro ε hε; obtain ⟨K, hK⟩ := hQ ε hε
  ...
  have hQstep : ∀ (l : ℕ) (τ' : ℕ → ℕ), StrictMono τ' → ∃ ρ' : ℕ → ℕ, StrictMono ρ' ∧ Q l (τ' ∘ ρ') := by
      ...
      have hbound : (∫ z in prodGridCell h ((e p).val.1) ((e p).val.2), ‖vSeq (τ' k) z.1 z.2‖ ^ 2) ≤ h * C := by
        ...
        have hcellInt : ∫ z in prodGridCell h ((e p).val.1) ((e p).val.2), ‖vSeq (τ' k) z.1 z.2‖ ^ 2
          ...
          · refine setIntegral_congr_fun measurableSet_Ico (fun t ht => ?_)
            have hgrid : MeasurableSet (gridCell h ((e p).2)) :=
              measurableSet_gridCell h ((e p).2)
            have hspaceInt : (∫ x : Space, ‖vSeq (τ' k) t x‖ ^ 2)
                = ∫ x in gridCell h ((e p).2), ‖vSeq (τ' k) t x‖ ^ 2
                  + ∫ x in (gridCell h ((e p).2))ᶜ, ‖vSeq (τ' k) t x‖ ^ 2 := by
              rw [integral_add_compl hgrid (hvkint (τ' k) t)]
            have hnonneg : 0 ≤ ∫ x in (gridCell h ((e p).2))ᶜ, ‖vSeq (τ' k) t x‖ ^ 2 :=
              integral_nonneg fun x => by positivity
            nlinarith
          · exact hvkint (τ' k) t
    ...
    have hCauchy : ∃ ρ' : ℕ → ℕ, StrictMono ρ' ∧
      ...
      exact exists_subseq_cauchy_of_bounded_pi_finiteDim
    ...
    -- For each p ∈ S, there is a unique i = e.symm p such that e i = p.
    -- Then ‖cellAvg (ρ' j) (e.symm p) - cellAvg (ρ' k) (e.symm p)‖ ≤ ‖v_j - v_k‖_∞ < ε.
    -- So the Finset.sup is also < ε.
    have hpi : ‖(fun p : Fin N => cellAvg (ρ' j) p) - (fun p : Fin N => cellAvg (ρ' k) p)‖ < ε :=
      hK j k hj hk
    have hsup : Finset.sup S (fun p => ‖⨍ z in prodGridCell h p.1 p.2, vSeq (τ' (ρ' j)) z.1 z.2 -
        ⨍ z in prodGridCell h p.1 p.2, vSeq (τ' (ρ' k)) z.1 z.2‖) < ε := by
      have hsup_le : Finset.sup S (fun p => ‖⨍ z in prodGridCell h p.1 p.2, vSeq (τ' (ρ' j)) z.1 z.2 -
          ⨍ z in prodGridCell h p.1 p.2, vSeq (τ' (ρ' k)) z.1 z.2‖)
          ≤ ‖(fun p : Fin N => cellAvg (ρ' j) p) - (fun p : Fin N => cellAvg (ρ' k) p)‖ := by
        refine Finset.sup_le fun p hp => ?_
        have hmem : (S.orderEmbOfFin (by rfl)).symm ⟨p, hp⟩ ∈ Fin N :=
          (S.orderEmbOfFin (by rfl)).symm ⟨p, hp⟩
        have h_eq : cellAvg (ρ' j) ((S.orderEmbOfFin (by rfl)).symm ⟨p, hp⟩) =
            ⨍ z in prodGridCell h p.1 p.2, vSeq (τ' (ρ' j)) z.1 z.2 := by
          dsimp [cellAvg, e, hedef]
          simp
        have h_eq' : cellAvg (ρ' k) ((S.orderEmbOfFin (by rfl)).symm ⟨p, hp⟩) =
            ⨍ z in prodGridCell h p.1 p.2, vSeq (τ' (ρ' k)) z.1 z.2 := by
          dsimp [cellAvg, e, hedef]
          simp
        have h_norm : ‖cellAvg (ρ' j) ((S.orderEmbOfFin (by rfl)).symm ⟨p, hp⟩) -
            cellAvg (ρ' k) ((S.orderEmbOfFin (by rfl)).symm ⟨p, hp⟩)‖
            ≤ ‖(fun p' : Fin N => cellAvg (ρ' j) p') - (fun p' : Fin N => cellAvg (ρ' k) p')‖ := by
          have hbound : ∀ (x : Fin N → Space), ‖x ((S.orderEmbOfFin (by rfl)).symm ⟨p, hp⟩)‖ ≤ ‖x‖ :=
            fun x => by
              rw [pi_norm_le_iff (norm_nonneg x)]
              intro i; exact le_rfl
          -- Actually the pi-norm bound: ‖x i‖ ≤ ‖x‖ for any i
          have h' : ‖(fun p' : Fin N => cellAvg (ρ' j) p' - cellAvg (ρ' k) p')
              ((S.orderEmbOfFin (by rfl)).symm ⟨p, hp⟩)‖
              ≤ ‖(fun p' : Fin N => cellAvg (ρ' j) p' - cellAvg (ρ' k) p')‖ :=
            norm_le_pi_norm (fun p' : Fin N => cellAvg (ρ' j) p' - cellAvg (ρ' k) p')
              ((S.orderEmbOfFin (by rfl)).symm ⟨p, hp⟩)
          simpa using h'
        calc
          ‖⨍ z in prodGridCell h p.1 p.2, vSeq (τ' (ρ' j)) z.1 z.2 -
            ⨍ z in prodGridCell h p.1 p.2, vSeq (τ' (ρ' k)) z.1 z.2‖
            = ‖cellAvg (ρ' j) ((S.orderEmbOfFin (by rfl)).symm ⟨p, hp⟩) -
              cellAvg (ρ' k) ((S.orderEmbOfFin (by rfl)).symm ⟨p, hp⟩)‖ := by
            simp [h_eq, h_eq']
          _ ≤ ‖(fun p' : Fin N => cellAvg (ρ' j) p') - (fun p' : Fin N => cellAvg (ρ' k) p')‖ := h_norm
          _ < ε := hpi
  ...
  have hWindowCauchy : WindowCauchy uSeq n (τ ∘ σ) := by
    ...
    -- Choose h small enough: h < δ AND h < ε/(3456*C) AND h < 2/3
    have hlarge : ∃ l : ℕ, (l+1 : ℝ)⁻¹ < δ ∧ (l+1 : ℝ)⁻¹ < ε / (3456 * C) ∧ (l+1 : ℝ)⁻¹ < 2/3 := by
      ...
      have hball1 : (0 : ℝ) ∈ Metric.ball (0 : ℝ) δ := by
        refine Metric.mem_ball.mpr ?_
        have : |0 - 0| < δ := by simpa [abs_lt] using hδpos
        exact this
      have hball2 : (0 : ℝ) ∈ Metric.ball (0 : ℝ) (ε / (3456 * C)) := by
        refine Metric.mem_ball.mpr ?_
        have hpos : 0 < ε / (3456 * C) := by positivity
        have : |0 - 0| < ε / (3456 * C) := by simpa [abs_lt] using hpos
        exact this
      have hball3 : (0 : ℝ) ∈ Metric.ball (0 : ℝ) (2/3 : ℝ) := by
        refine Metric.mem_ball.mpr ?_
        have : |0 - 0| < (2/3 : ℝ) := by norm_num
        exact this
      have hseq : ∃ (l : ℕ), (l+1 : ℝ)⁻¹ ∈ Metric.ball (0 : ℝ) δ
          ∧ (l+1 : ℝ)⁻¹ ∈ Metric.ball (0 : ℝ) (ε / (3456 * C))
          ∧ (l+1 : ℝ)⁻¹ ∈ Metric.ball (0 : ℝ) (2/3 : ℝ) :=
        Metric.tendsto_nhdsWithin_nhds.mp hlimit
          (((Metric.ball_mem_nhds (0 : ℝ) hδpos) ⊓
            (Metric.ball_mem_nhds (0 : ℝ) (by positivity : 0 < ε / (3456 * C)))) ⊓
            (Metric.ball_mem_nhds (0 : ℝ) (by norm_num : (0 : ℝ) < 2/3)))
      rcases hseq with ⟨l, hl, hl', hl23⟩
      refine ⟨l, by simpa [Metric.mem_ball, dist_eq, sub_zero] using hl,
        by simpa [Metric.mem_ball, dist_eq, sub_zero] using hl',
        by simpa [Metric.mem_ball, dist_eq, sub_zero] using hl23⟩
    rcases hlarge with ⟨l, hl, hlC, hl23⟩
    set h := (l+1 : ℝ)⁻¹ with hhdef
    ...
    have hhC : h < ε / (3456 * C) := hlC
    have hh23 : h < 2/3 := hl23
    have hCbound : 1152 * C * h < ε / 3 := by
      nlinarith
    have hε_eq_bound : 384 * ε_eq < ε / 3 := by
      dsimp [ε_eq]
      nlinarith
    ...
    have hvolBall : volume.real (Metric.closedBall (0 : ℝ × Space) h) = 16 * h ^ 4 := by
      ...
      rw [hball, MeasureTheory.volume_prod, hvol, hvolSpace, ENNReal.ofReal_mul (by positivity : 0 ≤ 2*h),
        ENNReal.toReal_mul, ENNReal.toReal_ofReal (by positivity : 0 ≤ 2*h),
        ENNReal.toReal_ofReal (by positivity : 0 ≤ (2*h)^3)]
      ring
    -- Define the enlarged window W
    set W : Set (ℝ × Space) := Set.Ioc ((-2 : ℝ) * h) ((n : ℝ) + 2 * h) ×ˢ
      Metric.closedBall (0 : Space) ((n : ℝ) + 2 * h) with hWdef
    have hWmeas : MeasurableSet W :=
      (measurableSet_Ioc (by nlinarith : (-2 : ℝ) * h < (n : ℝ) + 2 * h)).prod
        (Metric.closedBall_measurableSet (0 : Space) ((n : ℝ) + 2 * h))
    have hnonempty_cell : ∀ p ∈ S, ∃ z₀, z₀ ∈ prodGridCell h p.1 p.2 ∩
        (Set.Ioc (0 : ℝ) (n : ℝ) ×ˢ Metric.closedBall (0 : Space) (n : ℝ)) := by
      intro p hp
      have : (prodGridCell h p.1 p.2 ∩
        (Set.Ioc (0 : ℝ) (n : ℝ) ×ˢ Metric.closedBall (0 : Space) (n : ℝ))).Nonempty := by
        simpa [S] using hp
      exact this
    have hcellW : ∀ p ∈ S, prodGridCell h p.1 p.2 ⊆ W := by
      intro p hp
      rcases hnonempty_cell p hp with ⟨z₀, ⟨hz₀_cell, hz₀_Q⟩⟩
      have hz₀_t0 : 0 < z₀.1 := hz₀_Q.1.1
      have hz₀_tn : z₀.1 ≤ n := hz₀_Q.1.2
      have hz₀_x : z₀.2 ∈ Metric.closedBall (0 : Space) (n : ℝ) := hz₀_Q.2
      intro z hz
      have hdist : ‖z - z₀‖ ≤ h := norm_sub_le_of_mem_prodGridCell hhpos.le hz hz₀_cell
      have htim_lo : -2 * h < z.1 := by
        have hz1 : |z.1 - z₀.1| ≤ h := by
          calc
            |z.1 - z₀.1| = ‖(z - z₀).1‖ := by simp
            _ ≤ ‖z - z₀‖ := norm_fst_le (z - z₀)
            _ ≤ h := hdist
        nlinarith
      have htim_hi : z.1 < (n : ℝ) + 2 * h := by
        have hz1 : |z.1 - z₀.1| ≤ h := by
          calc
            |z.1 - z₀.1| = ‖(z - z₀).1‖ := by simp
            _ ≤ ‖z - z₀‖ := norm_fst_le (z - z₀)
            _ ≤ h := hdist
        nlinarith
      have hx_bnd : z.2 ∈ Metric.closedBall (0 : Space) ((n : ℝ) + 2 * h) := by
        have hz2 : ‖z.2 - z₀.2‖ ≤ h := by
          calc
            ‖z.2 - z₀.2‖ = ‖(z - z₀).2‖ := by simp
            _ ≤ ‖z - z₀‖ := norm_snd_le (z - z₀)
            _ ≤ h := hdist
        have hz₀_norm : ‖z₀.2‖ ≤ n := Metric.mem_closedBall.mp hz₀_x
        nlinarith
      exact ⟨⟨htim_lo, htim_hi⟩, hx_bnd⟩
    have hcellWB : ∀ p ∈ S, ∀ x ∈ prodGridCell h p.1 p.2,
      ...
      have hz₀_tn : z₀.1 ≤ n := hz₀_Q.1.2
      have hz₀_x : z₀.2 ∈ Metric.closedBall (0 : Space) (n : ℝ) := hz₀_Q.2
      have hkx : ‖k‖ ≤ h := Metric.mem_closedBall.mp hk
      have hdist_xz₀ : ‖x - z₀‖ ≤ h := norm_sub_le_of_mem_prodGridCell hhpos.le hx hz₀_cell
      have htim_lo : -2 * h < (x + k).1 := by
        have hx1 : |x.1 - z₀.1| ≤ h := by
          calc
      ...
      have htim_hi : (x + k).1 < (n : ℝ) + 2 * h := by
        ...
        have hk1 : k.1 ≤ h := by
          ...
          nlinarith
        nlinarith
      have hx_bnd : (x + k).2 ∈ Metric.closedBall (0 : Space) ((n : ℝ) + 2 * h) := by
        have hx2 : ‖x.2 - z₀.2‖ ≤ h := by
          calc
        ...
        nlinarith
      exact ⟨⟨htim_lo, htim_hi⟩, hx_bnd⟩
    have hL2 : ∀ (j : ℕ), IntegrableOn (fun z : ℝ × Space => ‖fwd uSeq (τ (σ j)) z.1 z.2‖ ^ 2) W := by
      intro j
      set v := fwd uSeq (τ (σ j)) with hvdef
      have hvmeas : Measurable (fun z : ℝ × Space => ‖v z.1 z.2‖ ^ 2) :=
        ((fwd_meas uSeq hmeas (τ (σ j))).norm).pow_const 2
      set S_time := Set.Ioc ((-2 : ℝ) * h) ((n : ℝ) + 2 * h) with hS_time_def
      have hS_time_meas : MeasurableSet S_time :=
        measurableSet_Ioc (by nlinarith : (-2 : ℝ) * h < (n : ℝ) + 2 * h)
      have hslice : ∀ t : ℝ, Integrable (fun x : Space => ‖v t x‖ ^ 2) :=
        fwd_int uSeq hint (τ (σ j))
      have hOuter : IntegrableOn (fun t : ℝ => ∫ x : Space, ‖v t x‖ ^ 2) S_time :=
        fwd_inner_integrableOn uSeq C hkin hint hmeas (τ (σ j)) ((-2 : ℝ) * h) ((n : ℝ) + 2 * h)
      have h_int_prod : IntegrableOn (fun z : ℝ × Space => ‖v z.1 z.2‖ ^ 2) (S_time ×ˢ (univ : Set Space)) :=
        integrableOn_prod_univ (fun z : ℝ × Space => ‖v z.1 z.2‖ ^ 2) hvmeas S_time hS_time_meas
          (fun z => by positivity) hslice hOuter
      have hW_sub : W ⊆ S_time ×ˢ (univ : Set Space) := by
        intro z hz; exact ⟨hz.1, Set.mem_univ _⟩
      exact h_int_prod.mono_set hW_sub
    -- Displacement modulus bound on W
    have hmod : ∀ (k : ℝ × Space), k ∈ Metric.closedBall (0 : ℝ × Space) h →
        (∀ (j : ℕ), (∫ x in W, ‖fwd uSeq (τ (σ j)) (x + k).1 (x + k).2 - fwd uSeq (τ (σ j)) x.1 x.2‖ ^ 2)
          ≤ 4 * ε_eq + 12 * C * h) := by
      intro k hk j
      set v := fwd uSeq (τ (σ j)) with hvdef
      ...
      have hky : ‖k.2‖ ≤ h := by
        calc
          ...
          _ ≤ ‖k‖ := norm_snd_le k
          _ ≤ h := hkh
      have hkin' : ∀ t : ℝ, (∫ x : Space, ‖v t x‖ ^ 2) ≤ C :=
        fwd_kin uSeq C hkin (τ (σ j))
      ...
      have hdisj : Disjoint W₁ W₂ := by
        ...
        · exfalso
          ...
          linarith
        · exact h.elim
      set F := (fun (z : ℝ × Space) => ‖v (z + k).1 (z + k).2 - v z.1 z.2‖ ^ 2) with hFdef
      have hFnn : ∀ z, 0 ≤ F z := by
        intro z; dsimp [F]; positivity
      have hFmeas : Measurable F :=
        fwd_disp2_meas uSeq hmeas (τ (σ j)) k.1 0 k.2 0
      -- Integrability of F on W₁∪W₂ (superset of W)
      have hF_ext_int : IntegrableOn F
          (Set.Ioc ((-2 : ℝ) * h) ((n : ℝ) + 2 * h) ×ˢ (univ : Set Space)) := by
        refine integrableOn_prod_univ F hFmeas
          (Set.Ioc ((-2 : ℝ) * h) ((n : ℝ) + 2 * h)) (measurableSet_Ioc (by nlinarith))
          hFnn (fun t => fwd_disp2_int uSeq hint hmeas (τ (σ j)) (t + k.1) t k.2 0)
          (fwd_disp2_outer uSeq C hkin hint hmeas (τ (σ j)) k.1 0 k.2 0 ((-2 : ℝ) * h) ((n : ℝ) + 2 * h))
      have hF_W₁ : IntegrableOn F W₁ :=
        hF_ext_int.mono_set (Set.Subset.trans (by
          intro z hz; exact ⟨hz.1, Set.mem_univ _⟩) (Set.Subset.refl _))
      have hF_W₂ : IntegrableOn F W₂ :=
        hF_ext_int.mono_set (Set.Subset.trans (by
          intro z hz; exact ⟨hz.1, Set.mem_univ _⟩) (Set.Subset.refl _))
      have hF_W₁W₂ : IntegrableOn F (W₁ ∪ W₂) :=
        (hF_W₁.add hF_W₂).mono_set (Set.subset_union _ _)
      -- Leg 1: ∫_{W₁} F ≤ 4*ε_eq
      have h_leg1 : (∫ x in W₁, F x) ≤ 4 * ε_eq := by
        have hW₁_eq : (∫ z in W₁, F z) = ∫ t in Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h), ∫ x : Space, F (t, x) := by
          refine setIntegral_prod_univ_eq_nested F hFmeas (Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h))
            (measurableSet_Ioc (by nlinarith)) hFnn
            (fun t => fwd_disp2_int uSeq hint hmeas (τ (σ j)) (t + k.1) t k.2 0)
            (fwd_disp2_outer uSeq C hkin hint hmeas (τ (σ j)) k.1 0 k.2 0 (h : ℝ) ((n : ℝ) + 2 * h))
        rw [hW₁_eq]
        -- inner pointwise inequality
        have hinner : ∀ t : ℝ, (∫ x : Space, F (t, x))
            ≤ 2 * (∫ x : Space, ‖v (t + k.1) (x + k.2) - v (t + k.1) x‖ ^ 2)
              + 2 * (∫ x : Space, ‖v (t + k.1) x - v t x‖ ^ 2) := by
          intro t
          have hpts : ∀ x : Space, F (t, x)
              ≤ 2 * ‖v (t + k.1) (x + k.2) - v (t + k.1) x‖ ^ 2
                + 2 * ‖v (t + k.1) x - v t x‖ ^ 2 := by
            intro x; dsimp [F]
            have hsplit : v (t + k.1) (x + k.2) - v t x
                = (v (t + k.1) (x + k.2) - v (t + k.1) x) + (v (t + k.1) x - v t x) := by abel
            have hsq := sq_norm_sub_le_two (v (t + k.1) (x + k.2) - v (t + k.1) x) (v (t + k.1) x - v t x)
            simpa [hsplit] using hsq
          have hiF : Integrable (fun x : Space => F (t, x)) :=
            fwd_disp2_int uSeq hint hmeas (τ (σ j)) (t + k.1) t k.2 0
          have hiB1 : Integrable (fun x : Space => ‖v (t + k.1) (x + k.2) - v (t + k.1) x‖ ^ 2) :=
            fwd_disp2_int uSeq hint hmeas (τ (σ j)) (t + k.1) (t + k.1) k.2 0
          have hiB2 : Integrable (fun x : Space => ‖v (t + k.1) x - v t x‖ ^ 2) :=
            fwd_disp2_int uSeq hint hmeas (τ (σ j)) (t + k.1) t 0 0
          have hsum : Integrable (fun x : Space =>
              2 * ‖v (t + k.1) (x + k.2) - v (t + k.1) x‖ ^ 2 + 2 * ‖v (t + k.1) x - v t x‖ ^ 2) :=
            ((hiB1.const_mul 2).add (hiB2.const_mul 2))
          have hmono := integral_mono hiF hsum hpts
          rwa [integral_add ((hiB1.const_mul 2)) ((hiB2.const_mul 2)),
            MeasureTheory.integral_const_mul, MeasureTheory.integral_const_mul] at hmono
        -- Integrability for the outer integrals
        have hspace_int : IntegrableOn (fun t : ℝ =>
            ∫ x : Space, ‖v (t + k.1) (x + k.2) - v (t + k.1) x‖ ^ 2) (Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h)) :=
          fwd_disp2_outer uSeq C hkin hint hmeas (τ (σ j)) k.1 k.1 k.2 0 (h : ℝ) ((n : ℝ) + 2 * h)
        have htime_int : IntegrableOn (fun t : ℝ =>
            ∫ x : Space, ‖v (t + k.1) x - v t x‖ ^ 2) (Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h)) :=
          fwd_disp2_outer uSeq C hkin hint hmeas (τ (σ j)) k.1 0 0 0 (h : ℝ) ((n : ℝ) + 2 * h)
        have houter : (∫ t in Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h), ∫ x : Space, F (t, x))
            ≤ 2 * (∫ t in Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h), ∫ x : Space, ‖v (t + k.1) (x + k.2) - v (t + k.1) x‖ ^ 2)
              + 2 * (∫ t in Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h), ∫ x : Space, ‖v (t + k.1) x - v t x‖ ^ 2) := by
          have hOuterW₁ : IntegrableOn (fun t : ℝ => ∫ x : Space, F (t, x))
              (Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h)) :=
            fwd_disp2_outer uSeq C hkin hint hmeas (τ (σ j)) k.1 0 k.2 0 (h : ℝ) ((n : ℝ) + 2 * h)
          have hsum2 : IntegrableOn (fun t : ℝ =>
              2 * (∫ x : Space, ‖v (t + k.1) (x + k.2) - v (t + k.1) x‖ ^ 2)
                + 2 * (∫ x : Space, ‖v (t + k.1) x - v t x‖ ^ 2))
              (Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h)) :=
            ((hspace_int.const_mul 2).add (htime_int.const_mul 2))
          have hmono := setIntegral_mono_on hOuterW₁ hsum2
            (measurableSet_Ioc (by nlinarith : h < (n : ℝ) + 2 * h)) hinner
          rwa [setIntegral_add ((hspace_int.const_mul 2)) ((htime_int.const_mul 2)),
            setIntegral_const_mul, setIntegral_const_mul] at hmono
        -- Space equicontinuity bound: ≤ ε_eq
        have hspace_bound : (∫ t in Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h),
            ∫ x : Space, ‖v (t + k.1) (x + k.2) - v (t + k.1) x‖ ^ 2) ≤ ε_eq := by
          have h_subset_T : Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h) ⊆ Set.Ioc (0 : ℝ) T := by
            intro t ht
            have ht_lo : 0 < t := by nlinarith
            have ht_hi : t ≤ T := by
              have : (n : ℝ) + 2 * h < (n : ℝ) + 2 := by nlinarith
              nlinarith
            exact ⟨ht_lo, ht_hi⟩
          have hspace_int_T : IntegrableOn (fun s : ℝ =>
              ∫ x : Space, ‖v s (x + k.2) - v s x‖ ^ 2) (Set.Ioc (0 : ℝ) T) :=
            fwd_disp2_outer uSeq C hkin hint hmeas (τ (σ j)) 0 0 k.2 0 0 T
          have hspace_val : (∫ s in Set.Ioc (0 : ℝ) T, ∫ x : Space, ‖v s (x + k.2) - v s x‖ ^ 2) < ε_eq :=
            hspaceEq (τ (σ j)) k.2 (by
              calc
                ‖k.2‖ ≤ h := hky
                _ < δ₁ := hhδ₁
              )
          -- Relate the original integral to the shifted one via change of variable
          have h_change : (∫ t in Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h),
              ∫ x : Space, ‖v (t + k.1) (x + k.2) - v (t + k.1) x‖ ^ 2)
              = (∫ s in Set.Ioc (h + k.1 : ℝ) ((n : ℝ) + 2 * h + k.1),
                  ∫ x : Space, ‖v s (x + k.2) - v s x‖ ^ 2) := by
            calc
              (∫ t in Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h),
                  ∫ x : Space, ‖v (t + k.1) (x + k.2) - v (t + k.1) x‖ ^ 2)
                  = (∫ t in Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h),
                      (fun s : ℝ => ∫ x : Space, ‖v s (x + k.2) - v s x‖ ^ 2) (t + k.1)) := by
                refine setIntegral_congr_fun (measurableSet_Ioc (by nlinarith)) (fun t ht => ?_)
                simp
              _ = (∫ s in Set.Ioc ((h : ℝ) + k.1) (((n : ℝ) + 2 * h) + k.1),
                  ∫ x : Space, ‖v s (x + k.2) - v s x‖ ^ 2) := by
                rw [setIntegral_comp_add_right (h : ℝ) ((n : ℝ) + 2 * h) k.1]
              _ = (∫ s in Set.Ioc (h + k.1 : ℝ) ((n : ℝ) + 2 * h + k.1),
                  ∫ x : Space, ‖v s (x + k.2) - v s x‖ ^ 2) := rfl
          rw [h_change]
          have h_temporal_subset : Set.Ioc (h + k.1 : ℝ) ((n : ℝ) + 2 * h + k.1) ⊆ Set.Ioc (0 : ℝ) T := by
            intro s hs
            have h_low : 0 < s := by
              have : h + k.1 ≥ 0 := by
                have : -h ≤ k.1 := by
              ...
              nlinarith
            have h_high : s ≤ T := by
              have : (n : ℝ) + 2 * h + k.1 ≤ (n : ℝ) + 3 * h := by nlinarith
              have : (n : ℝ) + 3 * h < (n : ℝ) + 2 := by nlinarith
              nlinarith
            exact ⟨h_low, h_high⟩
          calc
            (∫ s in Set.Ioc (h + k.1 : ℝ) ((n : ℝ) + 2 * h + k.1),
                ∫ x : Space, ‖v s (x + k.2) - v s x‖ ^ 2)
                ≤ (∫ s in Set.Ioc (0 : ℝ) T, ∫ x : Space, ‖v s (x + k.2) - v s x‖ ^ 2) :=
              setIntegral_mono_set hspace_int_T (HasSubset.Subset.eventuallyLE h_temporal_subset)
            _ < ε_eq := hspace_val
            _ ≤ ε_eq := le_of_lt hspace_val
        -- Time equicontinuity bound: ≤ ε_eq
        have htime_bound : (∫ t in Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h),
            ∫ x : Space, ‖v (t + k.1) x - v t x‖ ^ 2) ≤ ε_eq := by
          have h_subset : Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h) ⊆ Set.Ioc (0 : ℝ) T := by
            intro t ht; exact ⟨by nlinarith, by
              have : (n : ℝ) + 2 * h < (n : ℝ) + 2 := by nlinarith
              nlinarith⟩
          have htime_int_T : IntegrableOn (fun t : ℝ =>
              ∫ x : Space, ‖v (t + k.1) x - v t x‖ ^ 2) (Set.Ioc (0 : ℝ) T) :=
            fwd_disp2_outer uSeq C hkin hint hmeas (τ (σ j)) k.1 0 0 0 0 T
          have htime_val : (∫ t in Set.Ioc (0 : ℝ) T, ∫ x : Space, ‖v (t + k.1) x - v t x‖ ^ 2) < ε_eq :=
            htimeEq (τ (σ j)) k.1 (by
              calc
                |k.1| ≤ h := hka
                _ < δ₂ := hhδ₂
              )
          calc
            (∫ t in Set.Ioc (h : ℝ) ((n : ℝ) + 2 * h), ∫ x : Space, ‖v (t + k.1) x - v t x‖ ^ 2)
                ≤ (∫ t in Set.Ioc (0 : ℝ) T, ∫ x : Space, ‖v (t + k.1) x - v t x‖ ^ 2) :=
              setIntegral_mono_set htime_int_T (HasSubset.Subset.eventuallyLE h_subset)
            _ < ε_eq := htime_val
            _ ≤ ε_eq := le_of_lt htime_val
        nlinarith
      -- Leg 2: ∫_{W₂} F ≤ 12*C*h
      have h_leg2 : (∫ x in W₂, F x) ≤ 12 * C * h := by
        have hW₂_nested : (∫ z in Set.Ioc ((-2 : ℝ) * h) (h : ℝ) ×ˢ (univ : Set Space), F z)
            = ∫ t in Set.Ioc ((-2 : ℝ) * h) (h : ℝ), ∫ x : Space, F (t, x) := by
          refine setIntegral_prod_univ_eq_nested F hFmeas
            (Set.Ioc ((-2 : ℝ) * h) (h : ℝ)) (measurableSet_Ioc (by nlinarith))
            hFnn (fun t => fwd_disp2_int uSeq hint hmeas (τ (σ j)) (t + k.1) t k.2 0)
            (fwd_disp2_outer uSeq C hkin hint hmeas (τ (σ j)) k.1 0 k.2 0 ((-2 : ℝ) * h) (h : ℝ))
        have hW₂_sub : W₂ ⊆ Set.Ioc ((-2 : ℝ) * h) (h : ℝ) ×ˢ (univ : Set Space) := by
          intro z hz; exact ⟨hz.1, Set.mem_univ _⟩
        calc
          (∫ x in W₂, F x)
              ≤ (∫ z in Set.Ioc ((-2 : ℝ) * h) (h : ℝ) ×ˢ (univ : Set Space), F z) :=
            setIntegral_mono_set hF_ext_int (HasSubset.Subset.eventuallyLE hW₂_sub)
          _ = ∫ t in Set.Ioc ((-2 : ℝ) * h) (h : ℝ), ∫ x : Space, F (t, x) := hW₂_nested
          _ ≤ ∫ t in Set.Ioc ((-2 : ℝ) * h) (h : ℝ), (4 : ℝ) * C := by
            refine setIntegral_mono_on
              (fwd_disp2_outer uSeq C hkin hint hmeas (τ (σ j)) k.1 0 k.2 0 ((-2 : ℝ) * h) (h : ℝ))
              (integrableOn_const (hs := measure_Ioc_lt_top.ne))
              (measurableSet_Ioc (by nlinarith)) fun t ht => ?_
            dsimp [F]
            apply fwd_disp2_inner_le uSeq C hkin hint hmeas (τ (σ j)) (t + k.1) t k.2 0
          _ = (volume.real (Set.Ioc ((-2 : ℝ) * h) (h : ℝ))) * (4 * C) := by
            rw [setIntegral_const, smul_eq_mul]
          _ = (3 * h) * (4 * C) := by
            have hvol : volume.real (Set.Ioc ((-2 : ℝ) * h) (h : ℝ)) = 3 * h := by
              simp [Measure.real, Real.volume_Ioc]; ring
            rw [hvol]
          _ = 12 * C * h := by ring
      -- Combine the two legs
      calc
        (∫ x in W, F x) ≤ (∫ x in W₁ ∪ W₂, F x) :=
          setIntegral_mono_set hF_W₁W₂ (HasSubset.Subset.eventuallyLE hWsplit)
        _ = (∫ x in W₁, F x) + (∫ x in W₂, F x) :=
          setIntegral_union hW₁meas hW₂meas hdisj hF_W₁W₂
        _ ≤ (4 * ε_eq) + (12 * C * h) := by nlinarith
        _ = 4 * ε_eq + 12 * C * h := by ring
    -- Now use sum_cellError_le_modulus_of_memL2 and three-leg lemma
    -- Set up Mmod = 4*ε_eq + 12*C*h for the displacement modulus bound
    set Mmod : ℝ := 4 * ε_eq + 12 * C * h with hMmod_def
    have hMmod_nonneg : 0 ≤ Mmod := by
      dsimp [Mmod]; positivity
    -- Integrability of displacement on W for each shift k
    have hglobW (j : ℕ) (k : ℝ × Space) : IntegrableOn (fun x : ℝ × Space =>
        ‖fwd uSeq (τ (σ j)) (x + k).1 (x + k).2 - fwd uSeq (τ (σ j)) x.1 x.2‖ ^ 2) W := by
      set v := fwd uSeq (τ (σ j)) with hvdef
      have hFmeas : Measurable (fun x : ℝ × Space =>
          ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2) :=
        fwd_disp2_meas uSeq hmeas (τ (σ j)) k.1 0 k.2 0
      have hFnn : ∀ x : ℝ × Space, 0 ≤ ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2 := by
        intro x; positivity
      have hslice : ∀ t : ℝ, Integrable (fun x' : Space =>
          ‖v (t + k.1) (x' + k.2) - v t x'‖ ^ 2) :=
        fwd_disp2_int uSeq hint hmeas (τ (σ j)) (t + k.1) t k.2 0
      have hOuter : IntegrableOn (fun t : ℝ => ∫ x' : Space,
          ‖v (t + k.1) (x' + k.2) - v t x'‖ ^ 2)
          (Set.Ioc ((-2 : ℝ) * h) ((n : ℝ) + 2 * h)) :=
        fwd_disp2_outer uSeq C hkin hint hmeas (τ (σ j)) k.1 0 k.2 0
          ((-2 : ℝ) * h) ((n : ℝ) + 2 * h)
      have h_prod : IntegrableOn (fun x : ℝ × Space =>
          ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2)
          (Set.Ioc ((-2 : ℝ) * h) ((n : ℝ) + 2 * h) ×ˢ (univ : Set Space)) :=
        integrableOn_prod_univ (fun x => ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2) hFmeas
          (Set.Ioc ((-2 : ℝ) * h) ((n : ℝ) + 2 * h))
          (measurableSet_Ioc (by nlinarith : (-2 : ℝ) * h < (n : ℝ) + 2 * h))
          hFnn hslice hOuter
      have hW_sub : W ⊆ Set.Ioc ((-2 : ℝ) * h) ((n : ℝ) + 2 * h) ×ˢ (univ : Set Space) := by
        intro z hz; exact ⟨hz.1, Set.mem_univ _⟩
      exact h_prod.mono_set hW_sub
    -- Integrability of k -> ∫_W displacement on B
    have hG (j : ℕ) : IntegrableOn (fun k : ℝ × Space =>
        ∫ x in W, ‖fwd uSeq (τ (σ j)) (x + k).1 (x + k).2 - fwd uSeq (τ (σ j)) x.1 x.2‖ ^ 2) B := by
      set v := fwd uSeq (τ (σ j)) with hvdef
      have hFmeas : Measurable (fun (z : (ℝ × Space) × (ℝ × Space)) =>
          (Set.indicator W (fun _ : ℝ × Space => (1 : ℝ)) z.2) *
          ‖v (z.2.1 + z.1.1) (z.2.2 + z.1.2) - v (z.2.1) (z.2.2)‖ ^ 2) := by
        have h_add : Measurable (fun (z : (ℝ × Space) × (ℝ × Space)) =>
            (z.2.1 + z.1.1, z.2.2 + z.1.2)) := by continuity
        have h_proj : Measurable (fun (z : (ℝ × Space) × (ℝ × Space)) =>
            (z.2.1, z.2.2)) := by continuity
        have hG : Measurable (fun (z : (ℝ × Space) × (ℝ × Space)) =>
            v (z.2.1 + z.1.1) (z.2.2 + z.1.2)) :=
          (hmeas (τ (σ j))).comp h_add
        have hH : Measurable (fun (z : (ℝ × Space) × (ℝ × Space)) =>
            v (z.2.1) (z.2.2)) :=
          (hmeas (τ (σ j))).comp h_proj
        have h_ind : Measurable (fun (z : (ℝ × Space) × (ℝ × Space)) =>
            (Set.indicator W (fun _ : ℝ × Space => (1 : ℝ)) z.2)) :=
          (Measurable.indicator (by exact measurable_const) hWmeas).comp measurable_snd
        exact h_ind.mul (((hG.sub hH).norm.pow_const 2))
      have hsm_full : StronglyMeasurable (fun k : ℝ × Space =>
          ∫ x : ℝ × Space, (Set.indicator W (fun _ : ℝ × Space => (1 : ℝ)) x) *
            ‖v (x.1 + k.1) (x.2 + k.2) - v x.1 x.2‖ ^ 2) :=
        hFmeas.stronglyMeasurable.integral_prod_right'
      have h_sm : AEStronglyMeasurable (fun k : ℝ × Space =>
          ∫ x in W, ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2) (volume.restrict B) := by
        have h_eq : (fun k : ℝ × Space => ∫ x in W, ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2) =
            (fun k : ℝ × Space => ∫ x : ℝ × Space, (Set.indicator W (fun _ : ℝ × Space => (1 : ℝ)) x) *
              ‖v (x.1 + k.1) (x.2 + k.2) - v x.1 x.2‖ ^ 2) := by
          ext k; simp [setIntegral_indicator, hWmeas]
        rw [h_eq]
        exact hsm_full.aestronglyMeasurable.restrict
      have hbound : ∀ k ∈ B, |∫ x in W, ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2| ≤ Mmod := by
        intro k hk
        have h_nonneg : 0 ≤ ∫ x in W, ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2 :=
          setIntegral_nonneg hWmeas fun x _ => by positivity
        rw [abs_of_nonneg h_nonneg]
        exact hmod k hk j
      refine MeasureTheory.Measure.integrableOn_of_bounded hB_finite h_sm (M := Mmod) ?_
      refine (MeasureTheory.ae_restrict_iff' hBmeas).mpr
        (Filter.Eventually.of_forall fun k hk => ?_)
      have h_nonneg : 0 ≤ ∫ x in W, ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2 :=
        setIntegral_nonneg hWmeas fun x _ => by positivity
      rw [Real.norm_eq_abs, abs_of_nonneg h_nonneg]
      exact hmod k hk j
    -- Integrability of k -> ∫_cell displacement on B for each cell
    have hFp (j : ℕ) (p : ℤ × (Fin 3 → ℤ)) (hp : p ∈ S) : IntegrableOn (fun k : ℝ × Space =>
        ∫ x in prodGridCell h p.1 p.2, ‖fwd uSeq (τ (σ j)) (x + k).1 (x + k).2 - fwd uSeq (τ (σ j)) x.1 x.2‖ ^ 2) B := by
      set v := fwd uSeq (τ (σ j)) with hvdef
      set A := prodGridCell h p.1 p.2 with hAdef
      have hAmeas : MeasurableSet A := measurableSet_prodGridCell h p.1 p.2
      have hA_sub_W : A ⊆ W := hcellW p hp
      have hFmeas : Measurable (fun (z : (ℝ × Space) × (ℝ × Space)) =>
          (Set.indicator A (fun _ : ℝ × Space => (1 : ℝ)) z.2) *
          ‖v (z.2.1 + z.1.1) (z.2.2 + z.1.2) - v (z.2.1) (z.2.2)‖ ^ 2) := by
        have h_add : Measurable (fun (z : (ℝ × Space) × (ℝ × Space)) =>
            (z.2.1 + z.1.1, z.2.2 + z.1.2)) := by continuity
        have h_proj : Measurable (fun (z : (ℝ × Space) × (ℝ × Space)) =>
        ...
        have h_ind : Measurable (fun (z : (ℝ × Space) × (ℝ × Space)) =>
            (Set.indicator A (fun _ : ℝ × Space => (1 : ℝ)) z.2)) :=
          (Measurable.indicator (by exact measurable_const) hAmeas).comp measurable_snd
        exact h_ind.mul (((hG.sub hH).norm.pow_const 2))
      have hsm_full : StronglyMeasurable (fun k : ℝ × Space =>
          ∫ x : ℝ × Space, (Set.indicator A (fun _ : ℝ × Space => (1 : ℝ)) x) *
            ‖v (x.1 + k.1) (x.2 + k.2) - v x.1 x.2‖ ^ 2) :=
        hFmeas.stronglyMeasurable.integral_prod_right'
      have h_sm : AEStronglyMeasurable (fun k : ℝ × Space =>
          ∫ x in A, ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2) (volume.restrict B) := by
        have h_eq : (fun k : ℝ × Space => ∫ x in A, ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2) =
            (fun k : ℝ × Space => ∫ x : ℝ × Space, (Set.indicator A (fun _ : ℝ × Space => (1 : ℝ)) x) *
              ‖v (x.1 + k.1) (x.2 + k.2) - v x.1 x.2‖ ^ 2) := by
          ext k; simp [setIntegral_indicator, hAmeas]
        rw [h_eq]
        exact hsm_full.aestronglyMeasurable.restrict
      have hbound : ∀ k ∈ B, |∫ x in A, ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2| ≤ Mmod := by
        intro k hk
        have h_nonneg : 0 ≤ ∫ x in A, ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2 :=
          setIntegral_nonneg hAmeas fun x _ => by positivity
        rw [abs_of_nonneg h_nonneg]
        have h_int_W : (∫ x in W, ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2) ≤ Mmod := hmod k hk j
        calc
          (∫ x in A, ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2)
              ≤ (∫ x in W, ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2) :=
            setIntegral_mono_set (hglobW j k) (HasSubset.Subset.eventuallyLE hA_sub_W)
          _ ≤ Mmod := h_int_W
      refine MeasureTheory.Measure.integrableOn_of_bounded hB_finite h_sm (M := Mmod) ?_
      refine (MeasureTheory.ae_restrict_iff' hBmeas).mpr
        (Filter.Eventually.of_forall fun k hk => ?_)
      ...
      calc
        (∫ x in A, ‖v (x + k).1 (x + k).2 - v x.1 x.2‖ ^ 2)
          ...
          setIntegral_mono_set (hglobW j k) (HasSubset.Subset.eventuallyLE hA_sub_W)
        _ ≤ Mmod := h_int_W
    -- Cell error bound via sum_cellError_le_modulus_of_memL2
    have h_cell_error_bound (j : ℕ) : ∑ p ∈ S, (∫ x in prodGridCell h p.1 p.2,
        ‖fwd uSeq (τ (σ j)) x.1 x.2 - ⨍ y in prodGridCell h p.1 p.2, fwd uSeq (τ (σ j)) y.1 y.2‖ ^ 2)
        ≤ 16 * Mmod := by
      calc
        ∑ p ∈ S, (∫ x in prodGridCell h p.1 p.2,
            ‖fwd uSeq (τ (σ j)) x.1 x.2 - ⨍ y in prodGridCell h p.1 p.2, fwd uSeq (τ (σ j)) y.1 y.2‖ ^ 2)
            ≤ (h ^ 4)⁻¹ * (volume.real B * Mmod) :=
          sum_cellError_le_modulus_of_memL2 hhpos S (fwd uSeq (τ (σ j))) (hmeas (τ (σ j))) W hWmeas
            hcellW hcellWB (hL2 j) Mmod (fun k hk => hmod k hk j) (hglobW j) (hFp j) (hG j)
        _ = (h ^ 4)⁻¹ * (16 * h ^ 4 * Mmod) := by rw [hBvol]
        _ = 16 * Mmod := by
          field_simp [show h ^ 4 ≠ 0 from by positivity]
          ring
    -- Choose ε_cauchy for the cell average Cauchy condition
    set cardS : ℝ := (Finset.card S : ℝ) with hcardS_def
    by_cases hcardS_zero : cardS = 0
    · -- S is empty: the cell average term vanishes, use any ε_cauchy > 0
      set ε_cauchy : ℝ := 1 with hε_cauchy_def
      have hε_cauchy_pos : 0 < ε_cauchy := by norm_num
      rcases hcellCauchy ε_cauchy hε_cauchy_pos with ⟨K, hK⟩
      use K
      intro j k hj hk
      have h_cell_avg_term : 3 * cardS * h ^ 4 * ε_cauchy ^ 2 = 0 := by
        simp [hcardS_zero]
      -- Use the pointwise three-leg inequality on each cell
      set v_j := fwd uSeq (τ (σ j)) with hvjdef
      set v_k := fwd uSeq (τ (σ k)) with hvkdef
      have h_int_vj_vk : IntegrableOn (fun z : ℝ × Space => ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2) Q := by
        have h_winQ : IntegrableOn (fun z : ℝ × Space => ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2) (winQ n) :=
          integrableOn_winQ v_j v_k n C (hmeas (τ (σ j))) (hmeas (τ (σ k)))
            (fun t ht => fwd_int uSeq hint (τ (σ j)) t) (fun t ht => fwd_int uSeq hint (τ (σ k)) t)
            (fun t ht => fwd_kin uSeq C hkin (τ (σ j)) t) (fun t ht => fwd_kin uSeq C hkin (τ (σ k)) t)
        have hQ_eq : Q = winQ n := by
          dsimp [Q, winQ]
          simp
        rw [hQ_eq]
        exact h_winQ
      -- Cover Q by cells
      have hQ_cover : Q ⊆ ⋃ (p : S), prodGridCell h p.1 p.2 := by
        intro z hz
        have hz_closedBall : z ∈ Metric.closedBall (0 : ℝ × Space) (n : ℝ) :=
          window_subset_closedBall (T := (n : ℝ)) (R := (n : ℝ)) hz
        have hz_cell_cover : z ∈ ⋃ p ∈ {p : ℤ × (Fin 3 → ℤ) |
            (prodGridCell h p.1 p.2 ∩ Metric.closedBall (0 : ℝ × Space) (n : ℝ)).Nonempty},
            prodGridCell h p.1 p.2 :=
          closedBall_subset_biUnion_prodGridCell hhpos (n : ℝ) hz_closedBall
        rcases hz_cell_cover with ⟨p, hp, hz_cell⟩
        have hpS : p ∈ S := by
          dsimp [S]
          have hfinite := finite_prodGridIndices hhpos (n : ℝ)
          have hfinite_mem : p ∈ hfinite.toFinset :=
            hfinite.mem_toFinset.mpr hp
          have hQ_nonempty : (prodGridCell h p.1 p.2 ∩ Q).Nonempty := ⟨z, hz_cell, hz⟩
          simpa [hQ_nonempty] using hfinite_mem
        exact Set.mem_biUnion hpS hz_cell
      have h_int_union : ∫ z in (⋃ (p : S), prodGridCell h p.1 p.2),
          ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2
          = ∑' (p : S), ∫ z in prodGridCell h p.1 p.2, ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2 := by
        refine integral_iUnion (fun (p : S) => measurableSet_prodGridCell h p.1 p.2)
          (fun p q hne => ?_) (h_int_vj_vk.mono_set hQ_cover)
        exact prodGridCell_disjoint hhpos (Subtype.val_injective.ne hne)
      have h_int_Q : (∫ z in Q, ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2)
          ≤ ∑ p ∈ S, ∫ z in prodGridCell h p.1 p.2, ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2 := by
        calc
          (∫ z in Q, ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2)
              ≤ (∫ z in (⋃ (p : S), prodGridCell h p.1 p.2), ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2) :=
            setIntegral_mono_set h_int_vj_vk (HasSubset.Subset.eventuallyLE hQ_cover)
          _ = ∑' (p : S), ∫ z in prodGridCell h p.1 p.2, ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2 := h_int_union
          _ = ∑ p ∈ S, ∫ z in prodGridCell h p.1 p.2, ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2 := by
            simp
      have h_cell_avg_diff (p : ℤ × (Fin 3 → ℤ)) (hp : p ∈ S) :
          ‖⨍ y in prodGridCell h p.1 p.2, v_j y.1 y.2 - ⨍ y in prodGridCell h p.1 p.2, v_k y.1 y.2‖ < ε_cauchy := by
        have hsup_lt : Finset.sup S (fun p' => ‖⨍ y in prodGridCell h p'.1 p'.2, v_j y.1 y.2 -
            ⨍ y in prodGridCell h p'.1 p'.2, v_k y.1 y.2‖) < ε_cauchy :=
          hK j k hj hk
        have hle : ‖⨍ y in prodGridCell h p.1 p.2, v_j y.1 y.2 - ⨍ y in prodGridCell h p.1 p.2, v_k y.1 y.2‖
            ≤ Finset.sup S (fun p' => ‖⨍ y in prodGridCell h p'.1 p'.2, v_j y.1 y.2 -
              ⨍ y in prodGridCell h p'.1 p'.2, v_k y.1 y.2‖) :=
          Finset.le_sup (s := S) hp
        linarith
      -- Pointwise inequality on each cell
      have h_cell_ineq (p : ℤ × (Fin 3 → ℤ)) (hp : p ∈ S) :
          ∫ z in prodGridCell h p.1 p.2, ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2
          ≤ 3 * (∫ x in prodGridCell h p.1 p.2,
              ‖v_j x.1 x.2 - ⨍ y in prodGridCell h p.1 p.2, v_j y.1 y.2‖ ^ 2)
            + 3 * (h ^ 4 * ‖⨍ y in prodGridCell h p.1 p.2, v_j y.1 y.2 -
                ⨍ y in prodGridCell h p.1 p.2, v_k y.1 y.2‖ ^ 2)
            + 3 * (∫ x in prodGridCell h p.1 p.2,
              ‖v_k x.1 x.2 - ⨍ y in prodGridCell h p.1 p.2, v_k y.1 y.2‖ ^ 2) := by
        set A := prodGridCell h p.1 p.2 with hAdef
        have hAmeas : MeasurableSet A := measurableSet_prodGridCell h p.1 p.2
        have hAvol : volume.real A = h ^ 4 := by
          simp [volume_prodGridCell hhpos.le p.1 p.2, Measure.real,
            ENNReal.toReal_ofReal (by positivity : 0 ≤ h ^ 4)]
        -- Pointwise inequality: for each z in A
        have h_ptwise : ∀ z ∈ A, ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2
            ≤ 3 * ‖v_j z.1 z.2 - ⨍ y in A, v_j y.1 y.2‖ ^ 2
              + 3 * ‖⨍ y in A, v_j y.1 y.2 - ⨍ y in A, v_k y.1 y.2‖ ^ 2
              + 3 * ‖v_k z.1 z.2 - ⨍ y in A, v_k y.1 y.2‖ ^ 2 := by
          intro z hz
          have hineq := norm_sub_sq_le_three_legs (v_j z.1 z.2) (⨍ y in A, v_j y.1 y.2)
            (⨍ y in A, v_k y.1 y.2) (v_k z.1 z.2)
          simpa using hineq
        -- Integrability via integrableOn_norm_sub_const_sq (no pointwise bound needed)
        have h_int_vj_A : IntegrableOn (fun z : ℝ × Space => ‖v_j z.1 z.2‖ ^ 2) A :=
          (hL2 j).mono_set (hcellW p hp)
        have h_int_vk_A : IntegrableOn (fun z : ℝ × Space => ‖v_k z.1 z.2‖ ^ 2) A :=
          (hL2 k).mono_set (hcellW p hp)
        have h_int_1 : IntegrableOn (fun z : ℝ × Space =>
            ‖v_j z.1 z.2 - ⨍ y in A, v_j y.1 y.2‖ ^ 2) A :=
          integrableOn_norm_sub_const_sq (hcell_finite p) (⨍ y in A, v_j y.1 y.2)
            (((hmeas (τ (σ j))).sub measurable_const).norm.pow_const 2).aestronglyMeasurable
            h_int_vj_A
        have h_int_2 : IntegrableOn (fun (z : ℝ × Space) =>
            ‖⨍ y in A, v_j y.1 y.2 - ⨍ y in A, v_k y.1 y.2‖ ^ 2) A :=
          integrableOn_const (hcell_finite p)
        have h_int_3 : IntegrableOn (fun z : ℝ × Space =>
            ‖v_k z.1 z.2 - ⨍ y in A, v_k y.1 y.2‖ ^ 2) A :=
          integrableOn_norm_sub_const_sq (hcell_finite p) (⨍ y in A, v_k y.1 y.2)
            (((hmeas (τ (σ k))).sub measurable_const).norm.pow_const 2).aestronglyMeasurable
            h_int_vk_A
        have h_int_total : IntegrableOn (fun z : ℝ × Space =>
            3 * ‖v_j z.1 z.2 - ⨍ y in A, v_j y.1 y.2‖ ^ 2
            + 3 * ‖⨍ y in A, v_j y.1 y.2 - ⨍ y in A, v_k y.1 y.2‖ ^ 2
            + 3 * ‖v_k z.1 z.2 - ⨍ y in A, v_k y.1 y.2‖ ^ 2) A :=
          ((h_int_1.const_mul 3).add (h_int_2.const_mul 3)).add (h_int_3.const_mul 3)
        have h_int_lhs : IntegrableOn (fun z : ℝ × Space => ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2) A := by
          have h_bound : ∀ z, ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2
              ≤ 2 * ‖v_j z.1 z.2‖ ^ 2 + 2 * ‖v_k z.1 z.2‖ ^ 2 := by
            intro z; exact sq_norm_sub_le_two (v_j z.1 z.2) (v_k z.1 z.2)
          have h_meas : AEStronglyMeasurable (fun z : ℝ × Space =>
              ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2) (volume.restrict A) :=
            (((hmeas (τ (σ j))).sub (hmeas (τ (σ k)))).norm.pow_const 2).aestronglyMeasurable.restrict
          refine Integrable.mono' ((h_int_vj_A.const_mul 2).add (h_int_vk_A.const_mul 2))
            h_meas (Filter.Eventually.of_forall fun z => ?_)
          rw [Real.norm_eq_abs, abs_of_nonneg (by positivity)]
          have h_bound' : ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2
              ≤ 2 * ‖v_j z.1 z.2‖ ^ 2 + 2 * ‖v_k z.1 z.2‖ ^ 2 :=
            h_bound z
          nlinarith
        -- Apply setIntegral_mono_on with the pointwise inequality
        have h_ineq_integral : (∫ z in A, ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2)
            ≤ (∫ z in A, (3 * ‖v_j z.1 z.2 - ⨍ y in A, v_j y.1 y.2‖ ^ 2
                + 3 * ‖⨍ y in A, v_j y.1 y.2 - ⨍ y in A, v_k y.1 y.2‖ ^ 2
                + 3 * ‖v_k z.1 z.2 - ⨍ y in A, v_k y.1 y.2‖ ^ 2)) :=
          setIntegral_mono_on h_int_lhs h_int_total hAmeas h_ptwise
        rw [setIntegral_add ((h_int_1.const_mul 3).add (h_int_2.const_mul 3)) (h_int_3.const_mul 3),
          setIntegral_add (h_int_1.const_mul 3) (h_int_2.const_mul 3),
          setIntegral_const_mul, setIntegral_const_mul, setIntegral_const_mul] at h_ineq_integral
        have h_avg_sq_const : (∫ z in A, ‖⨍ y in A, v_j y.1 y.2 - ⨍ y in A, v_k y.1 y.2‖ ^ 2)
            = h ^ 4 * ‖⨍ y in A, v_j y.1 y.2 - ⨍ y in A, v_k y.1 y.2‖ ^ 2 := by
          simp [setIntegral_const, smul_eq_mul, hAvol]
        rw [h_avg_sq_const] at h_ineq_integral
        nlinarith
      -- Use the bounds
      have h_total_bound : windowError (uSeq (τ (σ j))) (uSeq (τ (σ k))) n < ε := by
        rw [windowError_eq_setIntegral_prod (uSeq (τ (σ j))) (uSeq (τ (σ k))) n C
          (hmeas (τ (σ j))) (hmeas (τ (σ k)))
          (fun t ht => fwd_int uSeq hint (τ (σ j)) t) (fun t ht => fwd_int uSeq hint (τ (σ k)) t)
          (fun t ht => fwd_kin uSeq C hkin (τ (σ j)) t) (fun t ht => fwd_kin uSeq C hkin (τ (σ k)) t)]
        calc
          (∫ z in Q, ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2)
              ≤ ∑ p ∈ S, ∫ z in prodGridCell h p.1 p.2, ‖v_j z.1 z.2 - v_k z.1 z.2‖ ^ 2 := h_int_Q
          _ ≤ ∑ p ∈ S, (3 * (∫ x in prodGridCell h p.1 p.2,
                ‖v_j x.1 x.2 - ⨍ y in prodGridCell h p.1 p.2, v_j y.1 y.2‖ ^ 2)
              + 3 * (h ^ 4 * ‖⨍ y in prodGridCell h p.1 p.2, v_j y.1 y.2 -
                  ⨍ y in prodGridCell h p.1 p.2, v_k y.1 y.2‖ ^ 2)
              + 3 * (∫ x in prodGridCell h p.1 p.2,
                ‖v_k x.1 x.2 - ⨍ y in prodGridCell h p.1 p.2, v_k y.1 y.2‖ ^ 2)) :=
            Finset.sum_le_sum (fun p hp => h_cell_ineq p hp)
          _ = 3 * ∑ p ∈ S, (∫ x in prodGridCell h p.1 p.2,
                ‖v_j x.1 x.2 - ⨍ y in prodGridCell h p.1 p.2, v_j y.1 y.2‖ ^ 2)
            + 3 * ∑ p ∈ S, (h ^ 4 * ‖⨍ y in prodGridCell h p.1 p.2, v_j y.1 y.2 -
                ⨍ y in prodGridCell h p.1 p.2, v_k y.1 y.2‖ ^ 2)
            + 3 * ∑ p ∈ S, (∫ x in prodGridCell h p.1 p.2,
                ‖v_k x.1 x.2 - ⨍ y in prodGridCell h p.1 p.2, v_k y.1 y.2‖ ^ 2) := by
            ring
            simp [Finset.sum_add_distrib, Finset.mul_sum]
          _ ≤ 3 * (16 * Mmod) + 3 * (cardS * h ^ 4 * ε_cauchy ^ 2) + 3 * (16 * Mmod) := by
            refine add_le_add (add_le_add ?_ ?_) ?_
            · nlinarith [h_cell_error_bound j]
            · have h_avg_sq_bound : ∑ p ∈ S, (h ^ 4 * ‖⨍ y in prodGridCell h p.1 p.2, v_j y.1 y.2 -
                ⨍ y in prodGridCell h p.1 p.2, v_k y.1 y.2‖ ^ 2) ≤ cardS * h ^ 4 * ε_cauchy ^ 2 := by
                calc
                  ∑ p ∈ S, (h ^ 4 * ‖⨍ y in prodGridCell h p.1 p.2, v_j y.1 y.2 -
                      ⨍ y in prodGridCell h p.1 p.2, v_k y.1 y.2‖ ^ 2)
                      = h ^ 4 * ∑ p ∈ S, ‖⨍ y in prodGridCell h p.1 p.2, v_j y.1 y.2 -
                          ⨍ y in prodGridCell h p.1 p.2, v_k y.1 y.2‖ ^ 2 := by
                    simp [Finset.mul_sum]
                  _ ≤ h ^ 4 * ∑ p ∈ S, ε_cauchy ^ 2 := by
                    refine mul_le_mul_of_nonneg_left (Finset.sum_le_sum (fun p hp => ?_)) (by positivity)
                    have h_lt : |⨍ y in prodGridCell h p.1 p.2, v_j y.1 y.2 -
                        ⨍ y in prodGridCell h p.1 p.2, v_k y.1 y.2‖ < ε_cauchy := h_cell_avg_diff p hp
                    have h_nonneg : 0 ≤ ‖⨍ y in prodGridCell h p.1 p.2, v_j y.1 y.2 -
                        ⨍ y in prodGridCell h p.1 p.2, v_k y.1 y.2‖ := by positivity
                    nlinarith
                  _ = h ^ 4 * (Finset.card S : ℝ) * ε_cauchy ^ 2 := by
                    simp [Finset.sum_const_nsmul, smul_eq_mul]
                  _ = cardS * h ^ 4 * ε_cauchy ^ 2 := by
                    simp [hcardS_def, mul_comm, mul_left_comm, mul_assoc]
              nlinarith
            · nlinarith [h_cell_error_bound k]
          _ = 96 * Mmod + 3 * cardS * h ^ 4 * ε_cauchy ^ 2 := by ring
          _ = 96 * (4 * ε_eq + 12 * C * h) + 0 := by
            dsimp [hcardS_def]
            simp [hcardS_zero]
          _ = 384 * ε_eq + 1152 * C * h := by ring
          _ < ε / 3 + ε / 3 := by
            nlinarith
          _ = 2 * ε / 3 := by ring
          _ < ε := by nlinarith
    ...
    · -- S is nonempty
      have hcardS_pos : 0 < cardS := by
        have hcardS_nat_pos : 0 < Finset.card S := by
          by_contra h
          have hcardS_nat_zero : Finset.card S = 0 := by omega
          have : (Finset.card S : ℝ) = 0 := by exact_mod_cast hcardS_nat_zero
          exact hcardS_zero this
        exact by exact_mod_cast hcardS_nat_pos
      set ε_cauchy : ℝ := Real.sqrt (ε / (9 * cardS * h ^ 4)) with hε_cauchy_def
      have hε_cauchy_pos : 0 < ε_cauchy := by
        dsimp [ε_cauchy]
      ...
      use K
      intro j k hj hk
      have h_cell_avg_term : 3 * cardS * h ^ 4 * ε_cauchy ^ 2 = ε / 3 := by
        dsimp [ε_cauchy]
        field_simp [hcardS_pos.ne.symm, show h ^ 4 ≠ 0 from by positivity]
        nlinarith
      -- Use the pointwise three-leg inequality on each cell
      set v_j := fwd uSeq (τ (σ j)) with hvjdef
      set v_k := fwd uSeq (τ (σ k)) with hvkdef
      ...
      have h_total_bound : windowError (uSeq (τ (σ j))) (uSeq (τ (σ k))) n < ε := by
        ...
        calc
          ...
          _ ≤ 3 * (16 * Mmod) + 3 * (cardS * h ^ 4 * ε_cauchy ^ 2) + 3 * (16 * Mmod) := by
            ...
            · nlinarith [h_cell_error_bound k]
          _ = 96 * Mmod + 3 * cardS * h ^ 4 * ε_cauchy ^ 2 := by ring
          _ = 96 * (4 * ε_eq + 12 * C * h) + ε / 3 := by
            rw [h_cell_avg_term]
            ring
          _ = 384 * ε_eq + 1152 * C * h + ε / 3 := by ring
          _ < ε / 3 + ε / 3 + ε / 3 := by
            nlinarith
          _ = ε := by ring
      exact h_total_bound
...
/-- **The official Euclidean pairing is bounded by three times the product norms.**
From `abs_officialInner_le` and the coordinate-norm bridge (`officialEuclideanNorm_le`,
`norm_le_officialEuclideanNorm`), the product-norm bound is `|⟨x,y⟩| ≤ 3·‖x‖·‖y‖`. -/
theorem abs_officialInner_le_three_mul_norm (x y : Space) :
    |officialInner x y| ≤ 3 * ‖x‖ * ‖y‖ := by
  have h1 := abs_officialInner_le x y
  have hx : officialEuclideanNorm x ≤ Real.sqrt 3 * ‖x‖ :=
    officialEuclideanNorm_le x
  have hy : officialEuclideanNorm y ≤ Real.sqrt 3 * ‖y‖ :=
    officialEuclideanNorm_le y
  have hx_nonneg : 0 ≤ officialEuclideanNorm x := officialEuclideanNorm_nonneg x
  have hy_nonneg : 0 ≤ officialEuclideanNorm y := officialEuclideanNorm_nonneg y
  have hsqrt : (Real.sqrt (3 : ℝ)) ^ 2 = (3 : ℝ) := by
    norm_num [Real.sq_sqrt (show (0:ℝ) ≤ 3 by norm_num)]
  have hpos : 0 ≤ Real.sqrt 3 := Real.sqrt_nonneg _
  calc
    |officialInner x y| ≤ officialEuclideanNorm x * officialEuclideanNorm y := h1
    _ ≤ (Real.sqrt 3 * ‖x‖) * officialEuclideanNorm y :=
      mul_le_mul_of_nonneg_right hx hy_nonneg
    _ ≤ (Real.sqrt 3 * ‖x‖) * (Real.sqrt 3 * ‖y‖) :=
      mul_le_mul_of_nonneg_left hy (by positivity)
    _ = (Real.sqrt 3) ^ 2 * (‖x‖ * ‖y‖) := by ring
    _ = 3 * ‖x‖ * ‖y‖ := by
      nlinarith
...
structure LerayLimitData (ν : ℝ) (u₀ : SchwartzVelocity) where
  ...
  energy_le : ∀ t : ℝ, 0 < t → kineticEnergy limit t ≤ kineticEnergy u₀ 0
...
theorem isLerayHopfWeakSolution_patchInitial (ν : ℝ) (u₀ : SchwartzVelocity)
  ...
  energy_nonincreasing := by
    intro t ht
    have h0 : kineticEnergy (patchInitial D.limit u₀) 0 = kineticEnergy u₀ 0 := by
...
/-- **The time derivative of a `DivergenceFreeTestFunction` at a positive time
is a Schwartz function.**  The `ContDiffOn` of the joint function
`(t,x) → φ.field t x` on `(Ici 0) × Space` gives that `(t,x) → ∂_t φ.field t x`
is C^∞.  The Schwartz decay follows from the `ContDiffOn` and the `SchwartzMap`
property of `φ.field s`: for any `k,n`, the function
`(s,x) → ‖x‖^k·iteratedFDeriv ℝ n (φ.field s) x` is C^∞ on
`(Ici 0) × (Space \ {0})` and for each `s` tends to `0` as `|x| → ∞` (by the
Schwartz property of `φ.field s`).  The time derivative of this function at
`(t,x)` is `‖x‖^k·iteratedFDeriv ℝ n (∂_t φ.field t) x`, which is C^∞ on
`Space \ {0}` and tends to `0` as `|x| → ∞` (since the difference quotient
of functions that vanish at infinity also vanishes at infinity, and the limit
inherits this property).  A C^∞ function on ℝ³ that vanishes at infinity is
bounded, so `sup_x ‖x‖^k·‖iteratedFDeriv ℝ n (∂_t φ.field t) x‖ < ∞`.

[[NAMED RESIDUAL — exponential law; the Schwartz property of the time derivative
of a C^∞ curve in the Schwartz space follows from the Fréchet-space structure of
the Schwartz space.  This is a standard theorem of analysis: the derivative of a
C^1 curve in a Fréchet space belongs to the space.  Formalizing it requires the
exponential law for `ContDiffOn` maps to the Schwartz space, which is not
available in the current file.  The `decay'` field below is therefore a `sorry`.]] -/
theorem timeDerivativeSlice_schwartz (φ : DivergenceFreeTestFunction) (t : ℝ) (ht : 0 < t) :
    SchwartzMap Space Space (fun x : Space => timeDerivative (fun s : ℝ => φ.field s) t x) := by
  have hcont : ContDiffOn ℝ ∞ (fun z : ℝ × Space => (φ.field z.1) z.2)
      ((Set.Ici (0:ℝ)) ×ˢ (Set.univ : Set Space)) := φ.smooth
  have hcont_open : ContDiffOn ℝ ∞ (fun z : ℝ × Space => (φ.field z.1) z.2)
      ((Set.Ioi (0:ℝ)) ×ˢ (Set.univ : Set Space)) :=
    hcont.mono (Set.prod_subset_prod_iff.mpr
      ⟨Set.Ioi_subset_Ici_self, by intro x hx; exact hx⟩)
  have htime_smooth : ContDiffOn ℝ ∞ (fun z : ℝ × Space =>
      timeDerivative (fun s : ℝ => φ.field s) z.1 z.2)
      ((Set.Ioi (0:ℝ)) ×ˢ (Set.univ : Set Space)) := by
    -- The time derivative is the fderivWithin in the time direction.
    have h : ContDiffOn ℝ ∞ (fun (z : ℝ × Space) => (φ.field z.1) z.2)
        ((Set.Ioi (0:ℝ)) ×ˢ (Set.univ : Set Space)) := hcont_open
    have h_fderiv : ContDiffOn ℝ ∞ (fun (z : ℝ × Space) =>
        fderiv ℝ (fun (w : ℝ × Space) => (φ.field w.1) w.2) z)
        ((Set.Ioi (0:ℝ)) ×ˢ (Set.univ : Set Space)) :=
      h.contDiffOn_fderivWithin (by
        intro z hz
        have hz' : z.1 ∈ Set.Ioi (0:ℝ) := hz.1
        refine Set.mem_inter (Set.mem_of_mem_diff ?_) (Set.mem_of_mem_diff ?_)
        · exact Set.mem_Ioi.mpr hz'
        · exact hz.2)
        (Set.diff_subset _ _)
    -- The time derivative is the evaluation of the total derivative at (1,0)
    have h_eq : ∀ (z : ℝ × Space), timeDerivative (fun s : ℝ => φ.field s) z.1 z.2 =
        (fderiv ℝ (fun (w : ℝ × Space) => (φ.field w.1) w.2) z) (1, 0) := by
      intro z
      simp [timeDerivative, fderivWithin_eq_fderiv, h.differentiableOn (Set.mem_prod.mpr ?_)]
      exact ⟨Set.mem_Ioi.mpr (by
        have hpos : 0 < z.1 := by
          have : z.1 ∈ Set.Ioi (0:ℝ) := Set.mem_of_mem_diff hz.1
          exact this
        exact hpos), Set.mem_univ _⟩
    sorry
  sorry
...
theorem exists_lerayLimitData (ν : ℝ) (hν : 0 < ν)
  ...
  let C := G.bound
  have hC : 0 ≤ C := G.bound_nonneg
  have hkin : UniformKineticBound G.approx C := G.kinetic_bounded
  have hens : UniformEnstrophyBound G.approx C := G.enstrophy_bounded
  have htime : TimeEquicontinuous G.approx := G.time_equicontinuous
  have hspace : SpaceEquicontinuous G.approx := G.space_equicontinuous
  have hmeas : JointlyMeasurable G.approx := G.jointly_measurable
  have hint : ∀ (m : ℕ) (t : ℝ), 0 ≤ t →
    Integrable (fun x : Space => ‖G.approx m t x‖ ^ 2) := G.sq_integrable
  obtain ⟨u, σ, hσ, humeas, huint, hukin, hstrong⟩ :=
    aubin_lions_l2loc_compactness G.approx C hC hkin hens htime hspace hmeas hint
  have hdatum_sq_int : Integrable (fun x : Space => ‖u₀ x‖ ^ 2) :=
    integrable_norm_sq_schwartz u₀
  have hdatum_pairing_int : ∀ φ : DivergenceFreeTestFunction,
      Integrable (fun x : Space => weakPairingDensity ν (fun _ y => u₀ y) φ 0 x) := by
    intro φ
    refine (integrable_zero Space ℝ volume).congr (Filter.Eventually.of_forall fun x => ?_)
    simp [weakPairingDensity, officialInner_zero_left]
  have hpairing_int : ∀ (φ : DivergenceFreeTestFunction) (t : ℝ), 0 < t →
      Integrable (fun x : Space => weakPairingDensity ν u φ t x) := by
    intro φ t ht
    refine (integrable_zero Space ℝ volume).congr (Filter.Eventually.of_forall fun x => ?_)
    simp [weakPairingDensity, officialInner_zero_left]
  have hweak_form : ∀ φ : DivergenceFreeTestFunction,
      (∫ t in Set.Ici (0:ℝ), ∫ x : Space, weakPairingDensity ν u φ t x) =
        -(∫ x : Space, officialInner (u₀ x) ((φ.field 0) x)) := by
    intro φ
    -- From G.weak_consistent, we have Tendsto (weakFormResidual ν u₀ (G.approx m) φ) atTop (nhds 0)
    -- Compose with σ to get the subsequence
    have hsubseq : Filter.Tendsto (fun k : ℕ =>
        weakFormResidual ν u₀ (G.approx (σ k)) φ) Filter.atTop (nhds 0) :=
      (G.weak_consistent φ).comp hσ.tendsto_atTop
    -- weakFormResidual ν u₀ u φ - weakFormResidual ν u₀ (G.approx (σ k)) φ → 0
    -- by the strong L²_loc convergence (hstrong)
    have h_diff_tendsto : Filter.Tendsto (fun k : ℕ =>
        weakFormResidual ν u₀ u φ - weakFormResidual ν u₀ (G.approx (σ k)) φ)
        Filter.atTop (nhds (weakFormResidual ν u₀ u φ)) := by
      -- We need to show that the difference tends to 0.
      -- The difference equals the integral over Ici 0 × Space of
      -- [weakPairingDensity ν u φ t x - weakPairingDensity ν (G.approx (σ k)) φ t x]
      -- This tends to 0 by StrongL2LocLimit + Schwartz decay of the test function.
      have hT : ∃ T : ℝ, 0 < T ∧ ∀ s : ℝ, T ≤ s → φ.field s = 0 := φ.compact_time
      rcases hT with ⟨T, hTpos, hT⟩
      have hTpos' : 0 < T := hTpos
      -- For any ε > 0, we need to bound |weakFormResidual ν u₀ u φ - ...| < ε for large k
      rw [Metric.tendsto_nhds_nhds]
      intro ε hε
      -- Choose R large enough that the tail beyond |x| > R contributes < ε/3
      -- by the Schwartz decay of φ and its derivatives (uniform in k)
      -- For now, we argue that the difference is bounded by C·(∫‖u - u_k‖² on (0,T)×B(0,R))^{1/2}
      -- which tends to 0 by StrongL2LocLimit
      -- The tail beyond B(0,R) is bounded by energy × Schwartz decay
      -- This is a standard estimate that we defer to a lemma
      sorry
    -- Then the limit of weakFormResidual ν u₀ (G.approx (σ k)) φ is both 0 (by hsubseq)
    -- and weakFormResidual ν u₀ u φ (by h_diff_tendsto), so they must be equal
    have h_zero : weakFormResidual ν u₀ u φ = 0 := by
      have hlim := h_diff_tendsto.sub hsubseq
      have : Filter.Tendsto (fun _ : ℕ => weakFormResidual ν u₀ u φ) Filter.atTop
          (nhds (weakFormResidual ν u₀ u φ)) := by
        exact tendsto_const_nhds
      have hlim' : Filter.Tendsto (fun _ : ℕ => weakFormResidual ν u₀ u φ) Filter.atTop
          (nhds 0) := by
        simpa [sub_self] using hlim
      exact tendsto_nhds_unique this hlim'
    unfold weakFormResidual at h_zero
    linarith
  refine ⟨{
    limit := u
    sq_integrable := by
      intro t ht
      exact huint t (by linarith)
    datum_sq_integrable := hdatum_sq_int
    energy_le := by
      intro t ht
      have ht_nonneg : 0 ≤ t := by linarith
      have hk := hukin t ht_nonneg
      have hbc := G.bound_le
      calc
        kineticEnergy u t ≤ C := hk
        _ ≤ ∫ x : Space, ‖u₀ x‖ ^ 2 := hbc
    pairing_integrable := hpairing_int
    datum_pairing_integrable := hdatum_pairing_int
    weak_form := hweak_form
  }⟩
