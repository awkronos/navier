# SOLVER-RSI lane — navier-spectral / smoke

Worktree (your only writable surface): /Users/schizodactyl/.worktrees/solver-rsi/navier-spectral-smoke-eval
Axis: eval. Size class: large.

Scoring surface, verbatim from the PINNED manifest:
...
Build: run the registry build command in the worktree.
Return contract (<=20 lines): VERDICT / EVIDENCE (commands + one-line results) /
ARTIFACTS (commit SHA in this worktree) / RESIDUAL / NEXT.
A return with neither a commit SHA nor a checked witness is REJECTED.
Do not modify any scoring file. Do not change what is measured.
