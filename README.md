...
> ⚠ **PROVENANCE STALE.** These counts were measured at `7e23b4a39cbe`; the `navier` repository is now at `63e40bfc80b0` and 17 commits have landed since. **The numbers below do not describe the current source.** A count of `0` here means zero *at the measured commit*, not zero now. Refresh the proof report before quoting anything in this block.
