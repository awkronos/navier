...
**Compiler state: UNVERIFIED.** For `navier` in `/tmp/proof-report.json` (generated 2026-08-31T15:59:00Z, git_head `11e2317423df`), the build is `SKIPPED` — it never reached GREEN. The counts below therefore have **no compiler verification**: `sorry_count` is a floor, not a total, and is not evidence of zero sorries, of closure, or of anything else. Treat every count as unverified.

| Field | Value | Status |
| --- | --- | --- |
| `build_status` | SKIPPED | measured |
| `sorry_count` | 8 | **UNVERIFIED** — floor only, report partial/non-GREEN |
| `theorems` | 2,420 | **UNVERIFIED** — static parse |
| `lemmas` | 38 | **UNVERIFIED** — static parse |
| `definitions` | 611 | **UNVERIFIED** — static parse |
| `declaration_count` | 2,750 | **UNVERIFIED** — static parse |
| `files` | 198 | measured on disk |
| `lines` | 69,766 | measured on disk |
| `soundness` | SKIPPED (build failed) | measured |
| `claim_status` | KERNEL-OPEN | measured |
| `scientific_open_count` | 0 | measured |
| `coverage_gap_count` | 0 | measured |
...
> `claim_status = KERNEL-OPEN`: kernel-level obligations remain open.
