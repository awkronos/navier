...
| 3 | `sobolevEmbeddingDomination` | BKMLogBootstrap | H³↪L∞; M–B Lemma 3.2. REDUCED in-repo to the single core `FourierMajorant.exists_fourierSpectralData` (SobolevEmbedding + FourierMajorant assembly kernel-clean: Cauchy–Schwarz half, spacePlancherel n=0 transport). **n=1,2,3 weighted-Plancherel rungs ESTABLISHED** (`FourierWeightedPlancherel.{sum_integral_normSq_lineDeriv_eq, sum_integral_normSq_pd_two_eq, sum_integral_normSq_pd_three_eq}` on ES=EuclideanSpace ℝ (Fin 3): ∑∫‖Dⁿf‖²=(2π)^{2n}∫‖ξ‖^{2n}‖𝓕f‖² for n=1,2,3, `7d44ed8`, all axioms [propext, Classical.choice, Quot.sound]). Reroute win: n=2/3 walled on a deterministic whnf timeout (2M hb) in the `calc` chain over the weighted derivative-transform integrand ‖ξ‖ᵏ‖𝓕(∂ⱼf)ξ‖²; isolated to the calc transitivity machinery over-normalising 𝓕 on 𝓢(ES,ℂ) (each step ~0.5-1s in isolation), rerouted to independent `have`s + final syntactic `rw`. To FIRE exists_fourierSpectralData (RED-with-spec, ~250-350 LOC, INDEPENDENT of the rungs): (a) vector transport 𝓢(Space,Space)→3 complexified scalar components on ES (~60 LOC, `toEuclid`/compCLMOfCLE pattern); (b) line-deriv↔iteratedFDeriv HS-norm bridge (my rungs use ∂_{eⱼ} line derivs; sobolevH3NormSq uses ‖iteratedFDeriv ℝ n‖ — need ∑ⱼ‖∂ⱼ..‖²=‖fderiv..‖²_HS per order); (c) inversion-domination ∀x ‖u x‖≤∫‖û‖ (~90 LOC, fourier inversion + norm_integral_le_integral_norm + Schwartz-decay MemLp); (d) binomial assembly: spectralMajorant F=∫‖û‖²(1+|ξ|²)³, sobWeightInv_eq gives (1+|ξ|²)³=1+3|ξ|²+3|ξ|⁴+|ξ|⁶, sum spacePlancherel(n=0)+rungs(n=1,2,3). Mathlib recon 2026-07-22: Distribution/Sobolev.lean ships MemSobolev.fourier_memL1 (qualitative 𝓕∈L¹, 2s>d) + SchwartzMap.memSobolev. Step-0e Gaussian-verified. | ~250-350 remaining (rungs done) |
