...
| 6 | `enstrophyDifferentialInequality` | Enstrophy | M–B §3.3. **Pointwise tower COMPLETE & axiom-clean** (`e856148`,`917525d`,`8b72a4b`): EnstrophyPointwise (∂_v|w|²=2⟨w,∂_v w⟩, Bochner Δ|w|²=2⟨w,Δw⟩+2∑|∂ᵢw|², viscous sign) → `local_enstrophy_balance` (EXACT integrand identity 2⟨ω,∂ₜω⟩=2⟨ω,(ω·∇)u⟩−(u·∇)|ω|²+νΔ|ω|²−2ν|∇ω|²) → `pointwise_enstrophy_differential_inequality` (2⟨ω,∂ₜω⟩ ≤ 2G|ω|²−(u·∇)|ω|²+νΔ|ω|²). RESIDUAL = integral layer only: (a) cutoff diff-under-integral E_R'=∫χ_R·(integrand) via `hasDerivAt_integral_of_dominated_loc_of_deriv_le` over supp χ_R; (b) integration-by-parts ∫χ_R(u·∇)|ω|²=−∫(u·∇χ_R)|ω|², ∫χ_R Δ|ω|²=∫(Δχ_R)|ω|²; (c) R→∞ interchange under the NAMED Pattern-A hypothesis: ∃ g∈L¹ dominating |ω(t,·)|² AND ∂ₜ|ω(t,·)|² uniformly for t in each compact ⊂[0,T) (automatic for H^m/Schwartz solutions; NOT carried by IsClassicalSolution). | integral layer ~250 |
