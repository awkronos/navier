...
| 3 | `sobolevEmbeddingDomination` | BKMLogBootstrap | H³↪L∞; M–B Lemma 3.2. REDUCED in-repo to the single core `FourierMajorant.exists_fourierSpectralData` (SobolevEmbedding + FourierMajorant assembly kernel-clean: Cauchy–Schwarz half, spacePlancherel n=0 transport). **n=1 weighted-Plancherel rung ESTABLISHED** (`FourierWeightedPlancherel.sum_integral_normSq_lineDeriv_eq`, ∑ⱼ∫‖∂ⱼf‖²=(2π)²∫‖ξ‖²‖𝓕f‖², axioms [propext, Classical.choice, Quot.sound]); n=2,3 iterate the same Schwartz-level multiplier identity on ∂ⱼf; then binomial assembly (sobWeightInv_eq) + inversion-domination substep. Mathlib recon 2026-07-22: Distribution/Sobolev.lean ships MemSobolev.fourier_memL1 (qualitative 𝓕∈L¹, 2s>d) + SchwartzMap.memSobolev. Step-0e Gaussian-verified. | ~200 remaining |
